...
1. Clone the `mlcommons/training` repository to acquire preprocessing scripts:
...
```bash
cd training/language_model/tensorflow/bert/cleanup_scripts
...
5. Follow the instructions at [Download and preprocess
   datasets](https://github.com/mlcommons/training/tree/master/language_model/tensorflow/bert#download-and-preprocess-datasets).
   The following commands can execute `create_pretraining_data.py` 500 times.
...
6. The above step can take over 36 hours. However, in parallel, you can create
...
7. Now switch to the `Model-References/MLPERF` directory and make a directory
   for the packed dataset and the results:

```bash
cd $ROOT/Model-References/MLPERF/Habana/benchmarks/bert
...
8. Prepare the packed dataset. After this, you will have several files
