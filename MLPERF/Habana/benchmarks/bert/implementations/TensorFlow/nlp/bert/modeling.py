...
class BertModel(object):
  ...
  def __init__(self,
               ...
               positions: Union[Tensor, type(None)] = None,
               next_sentence_positions: Union[Tensor, type(None)] = None,
               use_habana_layer_norm: bool = False):
    ...
    Args:
      ...
      enable_packed_data_mode: Used only when is_training==True.
                               Controls whether to work on packed data mode.
      positions: Used only when enable_packed_data_mode==True.
      next_sentence_positions: Used only when enable_packed_data_mode==True.
      use_habana_layer_norm: Whether to use habana layer_norm.
    ...
    with tf.compat.v1.variable_scope(scope, default_name="bert", custom_getter=get_custom_getter(compute_type)):
      with tf.compat.v1.variable_scope("embeddings"):
        ...
        self.embedding_output = embedding_postprocessor(
            ...
            positions=positions,
            use_habana_layer_norm=use_habana_layer_norm)
      ...
      with tf.compat.v1.variable_scope("encoder"):
        ...
        attention_mask = create_attention_mask_from_input_mask(
            input_ids, input_mask, is_training=is_training, enable_packed_data_mode=enable_packed_data_mode)
      ...
      with tf.compat.v1.variable_scope("pooler"):
        ...
        if enable_packed_data_mode and is_training: # only training should work in packed mode
          # When packed data mode is used, the "pooler"s output is tensor of shape [batch_size, 3, 2].
...
def layer_norm(input_tensor, use_habana_layer_norm: bool = False):
  ...
  if input_tensor.dtype == tf.float16:
    try:
      ...
      if use_habana_layer_norm:
        ...
        return tf.keras.layers.LayerNormalization(axis=-1, name='LayerNorm', epsilon=1e-12)(input_tensor)
  else:
    if use_habana_layer_norm:
      from habana_frameworks.tensorflow.ops.layer_norm import HabanaLayerNormalization
      return HabanaLayerNormalization(axis=-1, name='LayerNorm', epsilon=1e-12)(input_tensor)
...
def layer_norm_and_dropout(input_tensor, dropout_prob, use_habana_layer_norm: bool = False):
  """Runs layer normalization followed by dropout."""
  output_tensor = layer_norm(input_tensor, use_habana_layer_norm)
...
def embedding_postprocessor(input_tensor,
  ...
  output = layer_norm_and_dropout(output, dropout_prob, use_habana_layer_norm)
  return output


def create_attention_mask_from_input_mask(from_tensor, to_mask, is_training: bool = False, enable_packed_data_mode: bool = False):
    ...
    with tf.compat.v1.variable_scope("layer_%d" % layer_idx):
      ...
      with tf.compat.v1.variable_scope("attention"):
        ...
        with tf.compat.v1.variable_scope("output"):
          ...
          attention_output = layer_norm(attention_output + layer_input, use_habana_layer_norm)
      ...
      with tf.compat.v1.variable_scope("output"):
        ...
        layer_output = layer_norm(layer_output + attention_output, use_habana_layer_norm)
