...
"""MLPERF Hook for asynchronous checkpointing.

This hook dispatches checkpoint writing operations in a separate thread to
allow execution to continue on the main thread.

It is a slightly modified AsyncCheckpointSaverHook (tensorflow/python/tpu/async_checkpoint.py). Main differences:
- checkpoints are not stored at step 0
- pre_save method is called before spawning thread for checkpoint saving, pre_save method is intended to cache TF variables
  in separate set of variables, cached variables are further used by checkpoint saving thread
- when saving is in progress and next saving is triggered then execution will be stall until previous checkpoint is stored
"""
...
class AsyncCheckpointSaverHook(basic_session_run_hooks.CheckpointSaverHook):
  ...
  def __init__(self,
    ...
    # update timer to avoid storing checkpoints after first step
  ...
  def _save(self, session, step, asynchronous=True):
    ...
    if not asynchronous:
      ...
      self._get_saver().pre_save(session)
      _save_fn()
      return
