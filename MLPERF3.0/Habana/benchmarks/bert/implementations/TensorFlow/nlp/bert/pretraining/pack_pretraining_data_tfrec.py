...
###############################################################################
# Changes:
# - Added logging/progress information.
# - Changed the input-glob to input-dir argument to match other MLPerf scripts.
# - Added jobs parameter for setting ProcessPoolExecutor process limit.
...
def slice_examples(examples_by_length, slicing, strategy_set, repeat_counts):
    # Divide the work, firstly between the strategies and then into chunks
    slices = []
    strategies = []
    part_idx = []
...
def total_size(o, handlers={}, verbose=False):
    """ Returns the approximate memory footprint of the object and all of its contents.
...
def parallel_data_loader(path, filename):
    ...
    print(f"loading data from {path + filename}", flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-dir", help="A glob expression for the input files to read in and pack", required=True, type=str)
    ...
    parser.add_argument("--jobs", help="Number of jobs for parallel data processing.", default=25, type=int)
    ...
    # Input files
    print("Looping through dataset to collect sequence length...")
    input_files = np.random.choice(os.listdir(args.input_dir), size=args.max_files, replace=False)
    sequence_lengths = []
    examples_by_length = defaultdict(list)

    with ProcessPoolExecutor(args.jobs) as executor:
        work = repeat(args.input_dir), input_files.tolist()
    ...
    print('Done extracting sequence length.')
    ...
    gc.collect()
    print('Done slicing examples.')

    del examples_by_length; del slicing; del strategy_set; del mixture
    gc.collect()

    chunk_count = len(strategies) // 500 + 1
    print(f"Splitting work into {chunk_count} chunks.")
    start = time.time()
    for rr in range(chunk_count):
        print(f"Packing chunk {rr + 1} of {chunk_count}.")
