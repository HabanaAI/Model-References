...
class GPTDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(sample) != (self.seq_length + self.add_extra_token):
            ...
            sample = np.pad(sample, (0, self.seq_length + self.add_extra_token -
                            len(sample)), mode='constant', constant_values=-1)
...
def _build_index_mappings(name, data_prefix, documents, sizes,
    ...
    if (os.path.isfile(doc_idx_filename)) and \
        (os.path.isfile(sample_idx_filename)) and \
        ...
        doc_idx = _build_doc_idx(documents, num_epochs, np_rng,
                                 separate_last_epoch)
        print_rank_0(' > elasped time to build doc-idx mapping '
        ...
        #                              num_epochs, tokens_per_epoch,
        #                              num_samples < 0, add_extra_token)
        print_rank_0(' > elasped time to build sample-idx mapping '
                     '(seconds): {:4f}'.format(time.time() - start_time))
        # shuffle-idx.
        start_time = time.time()
        # -1 is due to data structure used to retieve the index:
        ...
        print_rank_0(' > elasped time to build shuffle-idx mapping'
                     ' (seconds): {:4f}'.format(time.time() - start_time))
