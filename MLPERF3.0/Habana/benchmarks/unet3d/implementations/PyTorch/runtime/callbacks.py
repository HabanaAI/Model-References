...
from runtime.distributed_utils import get_rank
...
def get_profiler(flags):
    if flags.profile_steps is not None:
        ...
        profiling_activities = [torch.profiler.ProfilerActivity.CPU]
        if flags.profile_hpu_device:
            profiling_activities += [torch.profiler.ProfilerActivity.HPU]

        profiler = torch.profiler.profile(
            activities=profiling_activities,
            schedule=torch.profiler.schedule(wait=0, warmup=profile_steps[0], active=profiling_duration, repeat=1),
            on_trace_ready=torch.profiler.tensorboard_trace_handler(flags.log_dir,
                                                                    worker_name=f"worker_{get_rank()}",
                                                                    use_gzip=True),
            record_shapes=True,
            with_stack=True)
        profiler.start()
