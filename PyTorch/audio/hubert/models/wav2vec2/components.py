...
class FeatureExtractor(Module):
    ...
    def forward(
        ...
        if self.use_conv2d:
            x = x.reshape(x.shape[0], 1, 1, -1) #(batch, 1, 1, frame)
        else:
            x = x.unsqueeze(1)  # (batch, channel==1, frame)


        for layer in self.conv_layers:
            x, length = layer(x, length)  # (batch, feature, frame) or (batch, feature, 1, frame)

        if self.use_conv2d:
            x = x.squeeze(2) #(batch, feature, 1, frame) [5, 512, 1, 781] => [5, 512, 781]
...
class SelfAttention(Module):
    ...
    def __init__(
        ...
        self.use_max_sub_softmax_opt = use_max_sub_softmax_opt
    ...
    def forward(
        ...
        # the following line is fused in softmax
        if self.use_max_sub_softmax_opt == False:
...
class FeedForward(Module):
    ...
    def forward(self, x):
        ...
        x = torch.nn.functional.gelu(x)
        if self.intermediate_dropout.p > 0.0:
            x = self.intermediate_dropout(x)

        x = self.output_dense(x)
        if self.output_dropout.p > 0.0:
...
class Transformer(Module):
    def __init__(
        ...
        layer_norm_first: bool,
        layer_drop: float,
        static_layerdrop: bool,
        recompilation_optimization: bool
    ):
        super().__init__()
        ...
        self.dropout = nn.Dropout(dropout)
        self.layers = layers
        self.static_layerdrop = static_layerdrop
        self.recompilation_optimization = recompilation_optimization
    ...
    def forward(
        ...
        if self.training and self.static_layerdrop and 'hpu' in str(x.device) and self.layer_drop > 0 and self.recompilation_optimization:
            htcore.mark_step()
        for layer in self.layers:
            if self.layer_drop <= 0:
                x, position_bias = layer(x, attention_mask, position_bias=position_bias)
            else:
                if self.static_layerdrop:
                    layer_x, position_bias = layer(x, attention_mask, position_bias=position_bias)
                    mask = torch.rand(1).item() <= self.layer_drop
                    mask = (mask * torch.ones_like(x)).type(torch.bool).to(x.device)
                    if self.training:
                        x = torch.where(mask,x,layer_x)
                        if 'hpu' in str(x.device) and self.recompilation_optimization:
                            htcore.mark_step()
                    else:
                        x = layer_x
                else:
...
class Encoder(Module):
    def __init__(
        ...
        transformer: Module,
        static_indexing: bool
    ):
        super().__init__()
        self.feature_projection = feature_projection
        self.transformer = transformer
        self.static_indexing = static_indexing
    ...
    def _preprocess(
        ...
        if lengths is not None:
            ...
            if self.static_indexing:
                x = torch.where(mask.unsqueeze(-1).expand(batch_size, max_len, x.shape[2]),torch.zeros_like(x, device=x.device),x)
            else:
                x[mask] = 0.0
...
def _get_feature_extractor(
    ...
    for i, (out_channels, kernel_size, stride) in enumerate(shapes):
        ...
        if norm_mode == "group_norm" and i == 0:
            if use_instancenorm: #TODO: add check for group == out_channels
                if use_conv2d:
                    print("USE INSTANCENORM 2D")
                    normalization = nn.InstanceNorm2d(out_channels, affine=True, track_running_stats=False)
                else:
                    normalization = nn.InstanceNorm1d(out_channels, affine=True, track_running_stats=False)
    ...
    return FeatureExtractor(nn.ModuleList(blocks), use_conv2d)
...
def _get_encoder(
    ...
    Args:
        ...
        layer_drop (float):
            ...
            Expected values are 0.1 for both Base and Large arch.
        static_indexing(bool): Use static torch.where instead of boolean indexing
        static_layerdrop (bool):
        use_max_sub_softmax_opt (bool):
    See Also:
        * "encoder_embed_dim"
    ...
    for _ in range(num_layers):
        attention = SelfAttention(
            ...
            use_max_sub_softmax_opt=use_max_sub_softmax_opt,
        )
    ...
    transformer = Transformer(
        ...
        static_layerdrop=static_layerdrop,
        recompilation_optimization=recompilation_optimization
    )
    return Encoder(feature_projection, transformer, static_indexing)
...
def _compute_mask_indices(
    ...
    for i in range(batch_size):
        if padding_mask is not None:
            if 'hpu' in str(padding_mask.device):
...
class MaskGenerator(Module):
    ...
    def forward(self, x: Tensor, padding_mask: Optional[Tensor]) -> Tensor:
        ...
        if self.mask_prob > 0:
            ...
            if self.static_indexing:
                expanded_mask_indices = mask_indices.unsqueeze(-1).expand(x.shape)
                expanded_mask_embedding = self.mask_embedding.unsqueeze(0).unsqueeze(0).expand(x.shape)
                x = torch.where(expanded_mask_indices, expanded_mask_embedding, x).to(x.dtype)
            else:
                #print("NO STATIC INDEX")
...
def compute_pos_is_neg(pos, label_embeddings, proj_x_size):
    label_embeddings0 = label_embeddings[:250,:]
    label_embeddings1 = label_embeddings[250:,:]
    def helper(emb):
        negs = emb.unsqueeze(1).expand(-1, proj_x_size, -1) # [250, 4984, 768]
        neg_is_pos = (pos == negs).all(-1)
        return neg_is_pos, negs
    neg_is_pos0, negs0 = helper(label_embeddings0)
    neg_is_pos1, negs1 = helper(label_embeddings1)
    return torch.cat([neg_is_pos0, neg_is_pos1]), negs0, negs1


def compute_logits_from_posneg(pos, negs0, negs1, proj_x):
    targets = torch.cat([pos, negs0], dim=0)
    logits0 = torch.cosine_similarity(proj_x.float(), targets.float(), dim=-1).type_as(proj_x)

    logits1 = torch.cosine_similarity(proj_x.float(), negs1.float(), dim=-1).type_as(proj_x)
    return torch.cat([logits0, logits1])
...
def _compute_logits(
    ...
    """
    logit_temp = 0.1
    #[TODO sasarkar] getting backward error with index_select
    #SW-110495
    #pos = torch.index_select(label_embeddings, 0, target.long())
    idx = torch.broadcast_to(target.unsqueeze(1), (target.shape[0], label_embeddings.shape[1]))
    pos = torch.gather(label_embeddings, 0, idx)
    if split_logits:
        neg_is_pos, negs0, negs1 = compute_pos_is_neg(pos, label_embeddings, proj_x.size(0))
        pos = pos.unsqueeze(0)
        logits = compute_logits_from_posneg(pos, negs0, negs1, proj_x)
    else:
    ...
    logits /= logit_temp
    if static:
        #Workaround to remove .any and boolean indx
        pos_mask = torch.zeros((pos.shape[0], pos.shape[1]), dtype=torch.bool, device=pos.device)
        pos_neg_mask = torch.cat([pos_mask,neg_is_pos], dim=0)
        logits = torch.where(pos_neg_mask, float("-inf"), logits)
    else:
        if neg_is_pos.any():
...
class LogitGenerator(Module):
    ...
    def forward(self, x: Tensor, label: Tensor, mask_m: Tensor, mask_u: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        if self.static:
            proj_x_reshaped = proj_x.reshape((-1, proj_x.shape[-1]))
            label_reshaped = label.flatten()
            return _compute_logits(proj_x_reshaped, label_reshaped, self.label_embeddings, self.static, self.split_logits)
        else:
            if self.skip_masked:
                logit_m = None
            else:
                ...
                logit_m = _compute_logits(proj_x_m, label_m, self.label_embeddings, self.static)
            ...
            if self.skip_nomask:
                ...
                logit_u = _compute_logits(proj_x_u, label_u, self.label_embeddings, self.static)
