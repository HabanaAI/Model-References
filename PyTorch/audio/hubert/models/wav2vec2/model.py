...
class HuBERTPretrainModel(Module):
    ...
    def __init__(
        ...
        use_conv2d: Optional[bool] = False,
        use_instancenorm: Optional[bool] = False,
    ):
        super().__init__()
    ...
    def forward(
        ...
        if padding_mask is not None:
            ...
            mask_m = mask
            mask_u = ~mask_m
        if self.logit_generator.static:
            logits = self.logit_generator(x, labels, mask_m, mask_u)
            return logits, mask_m, mask_u, features_pen
        else:
...
def wav2vec2_model(
    ...
    feature_extractor = components._get_feature_extractor(
        extractor_mode, extractor_conv_layer_config, extractor_conv_bias, use_conv2d=False, use_instancenorm=False
    )
    encoder = components._get_encoder(
        ...
        static_indexing=False,
        static_layerdrop=False,
        use_max_sub_softmax_opt=False,
        recompilation_optimization=False
    )
    aux = None
...
def _init_hubert_pretrain_model(module):
    if isinstance(module, components.ConvLayerBlock):
        ...
        std = math.sqrt(4.0 / (module.embed_dim * kernel))
...
def hubert_pretrain_model(
    ...
    final_dim: int,
    feature_grad_mult: Optional[float],
    static_logit_generator: bool,
    static_indexing: bool,
    static_layerdrop: bool,
    use_conv2d: Optional[bool],
    use_instancenorm: Optional[bool],
    use_max_sub_softmax_opt: Optional[bool],
    ...
    Args:
        ...
        static_logit_generator (Bool):
            Use static implementation for logitgenerator
    ...
    feature_extractor = components._get_feature_extractor(
        extractor_mode, extractor_conv_layer_config, extractor_conv_bias, use_conv2d, use_instancenorm,
    )
    encoder = components._get_encoder(
        in_features=extractor_conv_layer_config[-1][0],
        embed_dim=encoder_embed_dim,
        ...
        layer_norm_first=encoder_layer_norm_first,
        layer_drop=encoder_layer_drop,
        static_indexing=static_indexing,
...
def hubert_pretrain_base(
    ...
    return hubert_pretrain_model(
        ...
        final_dim=256,
        feature_grad_mult=feature_grad_mult,
        static_logit_generator=static_logit_generator,
        static_indexing=static_indexing,
