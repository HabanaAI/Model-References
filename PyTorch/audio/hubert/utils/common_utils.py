...
def create_tsv(
    ...
    with open(out_dir / f"{dataset}_train.tsv", "w") as train_f:
        ...
        lst = [i for i in root_dir.glob(f"**/*.{extension}")]
        for idx, fname in enumerate(lst):
            if idx % 1000 == 0:
                print(f'create_tsv: {idx}/{len(lst)}')
...
def hpu_wav2vec2_params_reshape(state, conv2d_to_1d):
    ...
    conv_param_name = ["model.wav2vec2.feature_extractor.conv_layers.{}.conv.weight".format(i) for i in range(0, 7)]

    for name, param in state.items():
        if name in conv_param_name:
            if conv2d_to_1d and param.data.shape in conv2d_weight_shapes:
                #print("CHANGE PARAM 2D->1D", name)
                param.data = param.data.squeeze(2)
            elif conv2d_to_1d == False and param.data.shape in conv1d_weight_shapes:
                #print("CHANGE PARAM 1D->2D", name)
