...
from .common_utils import _get_feat_lens_paths, hpu_wav2vec2_params_reshape
import models
...
def _load_state(model: Module, checkpoint_path: Path, device=_DEFAULT_DEVICE) -> Module:
    ...
    hpu_wav2vec2_params_reshape(state_dict["state_dict"], True) #change conv2d weights to conv1d if needed
...
def dump_features(
    ...
    if feature_type == "hubert":
        from models import hubert_pretrain_base
