...
class ResNet50Argparser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument('--dl-worker-type', default='MT', type=lambda x: x.upper(),
    ...
    self.add_argument('-j', '--workers', metavar='<workers>', type=int, default=8, help='Training device size')
    ...
    self.add_argument('--hls-type', default='HLS1', metavar='<hls_type>', help='hls type to differentiate HLS1 and HLS1_H',
                      choices=["HLS1", "HLS1-H"])
...
def main():
    ...
    if args.world_size == 1:
        ...
        training_runner = TrainingRunner([command],env_vars,args.world_size,args.dist,False,mpi_runner,
                                         map_by='slot', hls_type=args.hls_type,
                                         multi_hls=args.multi_hls)
