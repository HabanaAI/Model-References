...
def init_distributed_mode(args):
    ...
    if world_size  > 1:
        os.environ["ID"] = str(args.local_rank % process_per_node )
        # extend the default HCL timeout
        os.environ["MAX_WAIT_ATTEMPTS"] = "50"
        #not used currently
        os.environ["LOCAL_RANK"] = str(args.local_rank % process_per_node )
        import habana_frameworks.torch.core.hccl
        dist.init_process_group('hccl', rank=args.local_rank, world_size=world_size)
    else:
...
def save_model(args, model, optimizer, scheduler):
    ...
    if (str(args.device) == 'hpu'):
        permute_params(model_to_save, False, args.run_lazy_mode)
        ...
        for state in optimizer.state.values():
            for k, v in state.items():
                if isinstance(v, torch.Tensor):
                    state[k] = v.to('hpu')
        permute_params(model_to_save, True, args.run_lazy_mode)
...
def main():
    ...
    if (str(args.device) == 'hpu'):
        if not args.run_lazy_mode:
            os.environ["PT_HPU_LAZY_MODE"] = "2" # environment must be set before load_habana_module due to caching
