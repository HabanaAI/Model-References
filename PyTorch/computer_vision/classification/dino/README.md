...
* Some temporary workarounds have been applied in scripts in order to enable the model on HPU device. It includes:
  * setting SynapseAI environment variables in order to support bicubic interpolation mode;
  ...
  * replacing `index_copy_` with `scatter_` operator, because `index_copy_` is not yet implemented on HPU device;
...
# Known issues
* Running self-supervised part (`main_dino.py`) with `--batch_size_per_device=64` and float32 precision on HPU device crashes with an out of memory error.
  Due to this fact `--batch_size_per_device=32` is recommended for float32 precision.