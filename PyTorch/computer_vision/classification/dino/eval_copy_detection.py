...
# - removed jpegqual and crops blocks as they're unavailable for download
...
def extract_features(image_list, model, args):
    ...
    data_loader = torch.utils.data.DataLoader(tempdataset, batch_size=args.batch_size_per_device,
    ...
    for samples, index in utils.MetricLogger(global_batch_size, delimiter="  ").log_every(data_loader, 10):
        samples, index = samples.to(args.device, non_blocking=True), index.to(args.device, non_blocking=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--batch_size_per_device', default=16, type=int, help='Per-device batch-size')
