...
if __name__ == '__main__':
    ...
    train_features = extract_features(model, data_loader_train, global_batch_size, args.device, args.store_on_device, multiscale=args.multiscale)
    query_features = extract_features(model, data_loader_query, global_batch_size, args.device, args.store_on_device, multiscale=args.multiscale)
    ...
    if utils.get_rank() == 0:  # only rank 0 will work from now on
        ...
        # Moving sim to CPU due to argsort issues on HPU
        ranks = torch.argsort(-sim.cpu(), dim=0).numpy()
