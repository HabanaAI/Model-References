...
# - replace index_copy_ with scatter_ operator as it is not yet implemented on HPU device
...
def extract_feature_pipeline(args, create_summary_writer_fn):
    ...
    train_features = extract_features(model, data_loader_train, global_batch_size, args.store_on_device, summary_writer=create_summary_writer_fn('train_features'), max_batches=args.max_batches)
    print("Extracting features for val set...")
    test_features = extract_features(model, data_loader_val, global_batch_size, args.store_on_device, summary_writer=create_summary_writer_fn('test_features'), max_batches=args.max_batches)
...
def extract_features(model,
    ...
    for batch_num, (samples, index) in enumerate(metric_logger.log_every(data_loader, 10)):
        if max_batches is not None and batch_num >= max_batches:
            ...
            break
        samples = samples.to(device=args.device, non_blocking=True)
        index = index.to(device=args.device, non_blocking=True)
        if multiscale:
        ...
        if dist.get_rank() == 0 and features is None:
            ...
            if store_on_device:
                features = features.to(device=args.device, non_blocking=True)
        ...
        if dist.get_rank() == 0:
            if store_on_device:
                # Temporary replace index_copy_ with scatter_ operator
                # as the index_copy_ is not yet implemented on HPU device
                index_all = torch.unsqueeze(index_all, 1).repeat(1, feats.size(1))
                features.scatter_(0, index_all, torch.cat(output_l))
            else:
