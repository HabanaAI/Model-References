...
def restrict_neighborhood(h, w):
    ...
    return mask.to(args.device, non_blocking=True)
...
def label_propagation(args, model, frame_tar, list_frame_feats, list_segs, mask_neighborhood=None):
    ...
    list_segs = [s.to(args.device) for s in list_segs]
...
def extract_feature(model, frame, return_h_w=False):
    ...
    habana_compat.mark_step()
    out = model.get_intermediate_layers(frame.unsqueeze(0).to(args.device), n=1)[0]
