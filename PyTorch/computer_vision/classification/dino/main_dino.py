...
# - added support for habana mixed precision mode for hpu device
# - added possibility to limit dataset size by `data_limit` argument
# - renamed batch_size_per_gpu parameter to batch_size_per_device
# - changed default value for batch_size_per_device to 32
...
def get_args_parser():
    ...
    parser.add_argument('--use_fp16', type=utils.bool_flag, default=True, help="""Whether or not
        to use half precision for training. This feature is enabled only on `cuda` devices.
        Improves training time and memory requirements, but can provoke instability
        and slight decay of performance. We recommend disabling mixed precision if the loss is unstable,
    ...
    parser.add_argument('--batch_size_per_device', default=32, type=int,
...
def train_dino(args):
    ...
    dataset = utils.limit_dataset(dataset, args.data_limit)
    ...
    lr_schedule = utils.cosine_scheduler(
        args.lr * (args.batch_size_per_device * utils.get_world_size()) / 256.,  # linear scaling rule
        args.min_lr,
    ...
    summary_writer = SummaryWriter(args.output_dir) if utils.is_main_process() else None
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
                    ...
                    fp16_scaler, summary_writer, args):
    metric_logger = utils.MetricLogger(args.batch_size_per_device * utils.get_world_size(), "  ", summary_writer)
    header = 'Epoch: [{}/{}]'.format(epoch, args.epochs)
    for it, (images, _) in enumerate(metric_logger.log_every(data_loader, 10, header, epoch)):
    ...
    metric_logger.synchronize_between_processes(args.device)
