...
```
ii. Example: ResNet50, lazy mode: bf16 mixed precision, Batch size 256, world-size 1, custom learning rate, include dataloading time in throughput computation,
**use habana_dataloader with hardware decode support on Gaudi2**
```python
PT_HPU_POOL_STRATEGY=3 $PYTHON -u demo_resnet.py  --world-size 1 --batch-size 256 --model resnet50 --device hpu --print-freq 1 --channels-last True --deterministic --data-path /root/software/data/pytorch/imagenet/ILSVRC2012 --mode lazy --epochs 90 --data-type bf16  --custom-lr-values 0.1,0.01,0.001,0.0001 --custom-lr-milestones 0,30,60,80 --dl-time-exclude=False --dl-worker-type "HABANA"
```
iii. Example: ResNext101 lazy mode, bf16 mixed precision, Batch size 128, Custom learning rate
```python
...
```
iv. Example: ResNet152, lazy mode, bf16 mixed precision, Batch Size 128, Custom learning rate
```python
...
```
vi. Example: GoogLeNet, Batch Size 256, bf16 precision, lazy mode, World Size 1 on a single HLS1 machine.
```python
...
```
vii. Example: GoogLeNet, Batch Size 128, fp32 precision, lazy mode, World Size 1 on a single HLS1 machine.
```python
...
# Changelog
## 1.4.1
 - Added support for habana_dataloader with hardware decode support on Gaudi2
## 1.4.0
...
- Habana_dataloader with hardware decode support only supports training with 1 card on Gaudi2
...
2. Dataloader related changes
    ...
    d. Added habana_dataloader with hardware decode support for Gaudi2. It needs to be enabled with PT_HPU_POOL_STRATEGY=3.
