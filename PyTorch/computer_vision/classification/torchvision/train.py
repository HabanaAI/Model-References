...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, print_freq, apex=False):
    ...
    for image, target in metric_logger.log_every(data_loader, print_freq, header):
        ...
        mark_step_model_list = ['resnext101_32x4d', 'resnet152']
        if (args.run_lazy_mode and args.model in mark_step_model_list):
            htcore.mark_step()
        ...
        if args.run_lazy_mode:
            if ('mobilenet_v2' in args.model and not args.distributed):
                htcore.mark_step()
        ...
        if args.run_lazy_mode and not args.distributed:
            htcore.mark_step()
...
def main(args):
    ...
    if args.distributed:
        if args.device == 'hpu':
            ...
            bucket_size_mb = 200 if 'resnext101' or 'resnet152' in args.model else 100
            grad_view_model_list = ['mobilenet_v2', 'resnet152']
            is_grad_view = True if args.model in grad_view_model_list else False
