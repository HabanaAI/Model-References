...
Single Shot MultiBox Detector (SSD) is an object detection network. For an input image, the network
 outputs a set of bounding boxes around the detected objects, along with their classes. For example:
...
```
Two scripts are provided to run the workload. Primary one is demo_ssd.py
which is a wrapper over the base topology training script train.py.
Commands are mentioned here wrt demo_ssd.py with few examples using train.py.

Please refer to the following command for available training parameters:
```python
$PYTHON demo_ssd.py --help
```
i. Example: lazy mode, bf16 mixed precision, Batch Size 64, 12 data loader workers
```python
$PYTHON demo_ssd.py --world_size 1 --batch-size 64 --epochs 50 --data /root/software/data/pytorch/coco
 --log-interval 50 --val-interval 10 --num-workers 12 --device hpu --mode lazy
 --data-type bfloat16 --nosave-checkpoint
```
```python
$PYTHON train.py --batch-size 64 --num-workers 12 --epochs 50 --log-interval 50 --val-interval 10
...
$PYTHON demo_ssd.py --world_size 1 --batch-size 64 --epochs 50 --data /root/software/data/pytorch/coco
 --log-interval 50 --val-interval 10 --num-workers 12 --device hpu --mode lazy
 --data-type float32 --nosave-checkpoint
```
...
```python
$PYTHON demo_ssd.py --world_size 8 --bind-launchpath bind_launch.py --batch-size 32
 --epochs 52 --data /root/software/lfs/data/pytorch/coco --log-interval 100
 --val-interval 10 --num-workers 12 --device hpu --mode lazy --data-type bfloat16 --nosave-checkpoint
```
```python
MAX_WAIT_ATTEMPTS=90 $PYTHON -m bind_launch --nsockets_per_node 1 --ncores_per_socket 32
 --nproc_per_node 8 train.py -d /root/software/data/pytorch/coco/ --batch-size 32 --log-interval 100
 --val-interval 10 --use-hpu --hpu-lazy-mode --hmp --hmp-bf16 ops_bf16_ssdrn34.txt
 --hmp-fp32 ops_fp32_ssdrn34.txt --num-workers 12
```
ii. Example: lazy mode, fp32, Batch Size 32, 12 data loader workers
```python
$PYTHON demo_ssd.py --world_size 8 --bind-launchpath bind_launch.py --batch-size 32
 --epochs 52 --data /root/software/lfs/data/pytorch/coco --log-interval 100
 --val-interval 10 --num-workers 12 --device hpu --mode lazy --data-type float32 --nosave-checkpoint
```
