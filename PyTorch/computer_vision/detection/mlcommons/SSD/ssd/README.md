...
- [Directions](#1.-directions)
  ...
  - [Steps to launch training](#steps-to-launch-training)
    - [Single HPU Training](#single-hpu-training)
...
- [Training Script Modifications](#7.-training-script-modifications)
...
```
i. Example: lazy mode, bf16 mixed precision, Batch Size 128, 12 data loader workers
```python
...
```
ii. Example: lazy mode, fp32, Batch Size 128, 12 data loader workers
```python
$PYTHON train.py --batch-size 128 --num-workers 12 --epochs 50 --log-interval 100 --val-interval 5
 --data /root/software/data/pytorch/coco/ --use-hpu --hpu-lazy-mode
```
iii. Example: lazy mode, bf16 mixed precision, Batch Size 128, 12 Habana data loader workers
```python
$PYTHON train.py --batch-size 128 --num-workers 12 --epochs 50 --log-interval 100 --val-interval 5
 --data /root/software/data/pytorch/coco/ --use-hpu --hpu-lazy-mode
 --hmp --hmp-bf16 ops_bf16_ssdrn34.txt --hmp-fp32 ops_fp32_ssdrn34.txt --use-habana-dl
 ```
...
mpirun -n 8 --bind-to core --map-by socket:PE=6 --rank-by core --report-bindings
 --allow-run-as-root $PYTHON train.py -d /root/software/data/pytorch/coco/ --batch-size 128
 --log-interval 100 --val-interval 10 --use-hpu --hpu-lazy-mode --hmp --warmup 2.619685
 --hmp-bf16 ops_bf16_ssdrn34.txt --hmp-fp32 ops_fp32_ssdrn34.txt --num-workers 12 --use-habana-dl
```
...
- Using Habana Dataloader provides faster training, but currently having accuracy issue preventing
 convergence to SOTA.
- Evaluation using Habana Dataloader is not supported at this stage.

# 7. Training Script Modifications
