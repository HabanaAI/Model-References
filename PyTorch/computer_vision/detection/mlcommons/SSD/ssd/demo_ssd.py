...
class SSDPArgparser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument('--bind-launchpath',metavar='<distributed launch script>', type=str,
                      help="script to launch distributed")
...
def build_command(args, path, train_script):
    ...
    bind_launchpath = os.path.join(os.path.abspath(os.path.dirname(__file__)),
                                       "bind_launch.py")
    if args.world_size > 1:
        if args.bind_launchpath:
            bind_launchpath = args.bind_launchpath
        # Using a dummpy TrainRunner object just
        # to read the sockets and cores_per_socket info
        trainrunner_temp = TrainingRunner(world_size=args.world_size)
        try:
            _, sockets, cores_per_socket = trainrunner_temp.get_peval()
        except Exception:
            print("Error: Reading lscpu! Assigning default " \
                       "values for Cores per socket and Sockets per node")
            sockets = 1
            cores_per_socket = 32

        command = f" {bind_launchpath}" \
                  f" --nsockets_per_node {sockets}" \
                  f" --ncores_per_socket {cores_per_socket}" \
...
def main():
    ...
    training_runner = TrainingRunner([command], env_vars,
                                     world_size=args.world_size,
                                     dist=False, map_by='slot', use_mpi=False)
