    ...
    for nbatch, (img, img_id, img_size, bbox, label) in enumerate(val_dataloader):
        with torch.no_grad():
            ...
            try:
                if use_hpu:
                    if is_hmp:
                        results = encoder.decode_batch(ploc.float().cpu(), plabel.float().cpu(),
                                                       overlap_threshold,
                                                       nms_max_detections,
                                                       nms_valid_thresh=nms_valid_thresh)
                    else:
                        results = encoder.decode_batch(ploc.cpu(), plabel.cpu(),
                                                       overlap_threshold,
                                                       nms_max_detections,
                                                       nms_valid_thresh=nms_valid_thresh)
                else:
                ...
                if use_hpu:
                    ...
                    if is_hmp:
                        with hmp.disable_casts():
                            ...
                            trans_bbox = fbbox.to(device).transpose(1,2).contiguous()
