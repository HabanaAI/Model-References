...
# - Added BF16 support via --lowp/--BF16
...
def parse_args():
    ...
    parser.add_argument('--use-habana-dl', action='store_true',
                        help='use habana dataloader instance')
    parser.add_argument('--hpu-channels-last', action='store_true',
    ...
    parser.add_argument('--lowp', '--BF16', action='store_true', help='cast to BF16')  # amorgenstern
...
def coco_eval(model, val_dataloader, cocoGt, encoder, inv_map, threshold,
              ...
              hpu_channels_last=False, hpu_lazy_mode=False,
              nms_valid_thresh=0.05, N_gpu=1, local_rank=-1):
...
def train300_mlperf_coco(args):
    ...
    use_habana_dl = args.use_habana_dl
    hpu_channels_last = args.hpu_channels_last
    hpu_lazy_mode = args.hpu_lazy_mode
    is_hmp = args.is_hmp
    device = torch.device('cpu')
    if use_habana_dl:
        from habana_dataloader import HabanaDataloaderWrapper
        data_loader_type = HabanaDataloaderWrapper
    else:
