...
def init_dist(launcher, bucket_cap_mb, backend='nccl', **kwargs):
    if mp.get_start_method(allow_none=True) is None:
        mp.set_start_method('spawn')
    if launcher == 'pytorch':
        _init_dist_pytorch(backend, bucket_cap_mb, **kwargs)
    elif launcher == 'mpi':
...
def _init_dist_pytorch(backend, bucket_cap_mb, **kwargs):
    if is_hpu_enabled():
