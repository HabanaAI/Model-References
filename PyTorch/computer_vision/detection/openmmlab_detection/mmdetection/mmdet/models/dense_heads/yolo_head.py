...
class YOLOV3Head(BaseDenseHead, BBoxTestMixin):
    ...
    def get_bboxes(self,
        ...
        if rescale:
            flatten_bboxes /= flatten_bboxes.new_tensor(
                numpy.array(scale_factors)).unsqueeze(1)
        ...
        if is_hpu_enabled() and groundtruth_processing_on_cpu():
            flatten_bboxes = move_to_device(flatten_bboxes, 'cpu')
            flatten_cls_scores = move_to_device(flatten_cls_scores, 'cpu')
            flatten_objectness = move_to_device(flatten_objectness, 'cpu')
    ...
    def loss_single(self, pred_map, target_map, neg_map):
        ...
        #if torch.max(pos_and_neg_mask) > 1. #remove to avoid d2h copy as dataset has no condition
        # for if torch.min(pos_and_neg_mask) < 0. and  torch.max(pos_and_neg_mask) < 1.
    ...
    def _get_targets_single(self, anchors, responsible_flags, gt_bboxes,
        ...
        if is_hpu_enabled() and not groundtruth_processing_on_cpu():
            assert 'hpu' in str(anchor_strides.device)
        ...
        neg_map[sampling_result.neg_inds.long()] = 1
