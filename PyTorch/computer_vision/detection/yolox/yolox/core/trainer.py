...
class Trainer:
    ...
    def train_one_iter(self):
        ...
        if torch.cuda.is_available() and not self.args.disable_prefetcher:
            inps_cpu, targets = self.prefetcher.next()
        else:
        ...
        if self.args.hpu:
            ...
            inps = inps_cpu.to(self.data_type)
        ...
        if self.rank == 0:
            self.tblogger.add_scalar("train/loss", loss, self.progress_in_iter)

        if torch.cuda.is_available() and self.scaler is not None:  # support for fp32 w/o scaler
        ...
        self.epoch_data_time += (data_end_time - iter_start_time)
        self.epoch_iter_time += (iter_end_time - iter_start_time)
        self.num_iters += 1
    ...
    def before_train(self):
        ...
        if self.max_epoch == 1 and 0 < self.steps < total_iters_epoch:
            self.max_iter = self.steps
        else:
            self.max_iter = total_iters_epoch
    ...
    def before_epoch(self):
        ...
        self.epoch_data_time = 0
        self.epoch_iter_time = 0
        self.epoch_time = 0
        self.num_iters = 0
    ...
    def after_epoch(self):
        ...
        if (self.epoch + 1) % self.exp.eval_interval == 0:
            all_reduce_norm(self.model, self.device)
            self.evaluate_and_save_model()

        logger.info("average data loading time (per batch): {}".format(
            datetime.timedelta(seconds=(self.epoch_data_time / self.num_iters))))
        avg_iter_time = self.epoch_iter_time / self.num_iters
        logger.info("average iteration time (per batch): {}".format(
            datetime.timedelta(seconds=avg_iter_time)))
        logger.info("average throughput for epoch_{}: {:0.2f} img/sec".format(
            (self.epoch + 1), self.args.batch_size / avg_iter_time))
        logger.info("data loading time (per epoch): {:0.2f} min".format(self.epoch_data_time / 60))
        logger.info("epoch_{} time: {:0.2f} min".format((self.epoch + 1), self.epoch_time / 60))
    ...
    def after_iter(self):
        ...
        if (self.iter + 1) % self.exp.print_interval == 0:
            ...
            time_list = ["{}: {:.3f}s".format(k, v.avg) for k, v in time_meter.items()]
            time_list.append("global_avg_time: {:.3f}s".format(self.meter["iter_time"].global_avg))
            ...
            else: # remove GPU usage in progress
                logger.info(
                    "{}, {}, {}, lr: {:.3e}".format(
                        progress_str,
                        time_str,
