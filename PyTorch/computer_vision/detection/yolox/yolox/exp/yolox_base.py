...
class Exp(BaseExp):
    ...
    def get_data_loader(
            self, batch_size, is_distributed, no_aug=False, cache_img=False, use_hpu=True
    ):
        ...
        pin_memory =  pin_memory_device != None

        if use_hpu:
            torch.cuda.current_device = lambda: None
            torch.cuda.set_device = lambda x: None

        dataloader_kwargs = {"num_workers": self.data_num_workers, "pin_memory": pin_memory}
        if pin_memory:
    ...
    def get_optimizer(self, batch_size, use_hpu):
        if "optimizer" not in self.__dict__:
            ...
            if use_hpu:  # use Habana FusedSGD
                from habana_frameworks.torch.hpex.optimizers import FusedSGD
                optimizer = FusedSGD(pg0, lr=lr, momentum=self.momentum, nesterov=True)
            else:
    ...
    def get_eval_loader(self, batch_size, is_distributed, testdev=False, legacy=False, use_hpu=True):
        ...
        dataloader_kwargs = {"num_workers": self.data_num_workers, "pin_memory": pin_memory, "sampler": sampler}
        if pin_memory:
            dataloader_kwargs["pin_memory_device"] = pin_memory_device
    ...
    def get_evaluator(self, batch_size, is_distributed, testdev=False, legacy=False, use_hpu=False, inferece_only=False):
