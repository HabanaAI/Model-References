...
```
vi. Example: Unet3D in Lazy mode, bf16 mixed precision, Batch Size 2 Fold 0, reduced validation frequency to reduce time to train
```python
$PYTHON -u main.py --results /tmp/Unet/results/fold_0 --task 01 --logname res_log --fold 0 --hpus 1 --gpus 0 --data /root/software/data/pytorch/unet/01_3d --seed 1 --num_workers 8 --affinity disabled --norm instance --dim 3 --optimizer fusedadamw --exec_mode train --learning_rate 0.001 --run_lazy_mode --hmp --hmp-bf16 ./config/ops_bf16_unet.txt --hmp-fp32 ./config/ops_fp32_unet.txt --deep_supervision --batch_size 2 --val_batch_size 2 --val_every_n_epoch=10 --patience=11
```
vii. Example: Unet3D in Lazy mode, bf16 mixed precision, Batch Size 2, Benchmarking
```python
...
```
vi. Example: Unet3D in Lazy mode, bf16 mixed precision, Batch Size 2, world-size 8, reduced validation frequency to reduce time to train
```python
$PYTHON -u main.py --results /tmp/Unet/results/fold_0 --task 01 --logname res_log --fold 0 --hpus 8 --gpus 0 --data /root/software/data/pytorch/unet/01_3d --seed 1 --num_workers 8 --affinity disabled --norm instance --dim 3 --optimizer fusedadamw --exec_mode train --learning_rate 0.001 --run_lazy_mode --hmp --hmp-bf16 ./config/ops_bf16_unet.txt --hmp-fp32 ./config/ops_fp32_unet.txt --deep_supervision --batch_size 2 --val_batch_size 2 --val_every_n_epoch=10 --patience=11
```

vii. Example: Unet3D in Lazy mode, bf16 mixed precision, Batch Size 2, world-size 8, benchmarking
```python
...
# Changelog
## 1.2.0
 - Added a new command line argument in the script to control the number of epochs after which validation is performed
...
- Time to train for Unet3D will be improved in a subsequent release.
...
4. To improve performance
   ...
   f. Added monai_sliding_window_inference.py with code from monai package. Certain ops are performed on CPU in the validation path.
