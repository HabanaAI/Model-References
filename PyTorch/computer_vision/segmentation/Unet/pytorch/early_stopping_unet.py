...
class EarlyStopping:
    ...
    def __init__(
        ...
        #self.first_epoch = True # PL 1.6 - added to enable loading ckpt
    ...
    def _should_skip_check(self, trainer: "Trainer") -> bool:
        ...
        #    return True
        #return trainer.state.fn != TrainerFn.FITTING
        return False

    def on_train_epoch_end(self, trainer: "Trainer",  dice_sum) -> None:
        if not self._check_on_train_epoch_end:
            return
        self._run_early_stopping_check(trainer, dice_sum)

    def on_validation_end(self, trainer: "Trainer",  dice_sum) -> None:
        #if self._check_on_train_epoch_end:
        #    return
        self._run_early_stopping_check(trainer, dice_sum)

    def _run_early_stopping_check(self, trainer: "Trainer", dice_sum) -> None:
        ...
        current = dice_sum.squeeze()
        should_stop, reason = self._evaluate_stopping_criteria(current)

        # stop every ddp process if any world process decides to stop
        #should_stop = trainer.strategy.reduce_boolean_decision(should_stop)
        should_stop = should_stop # will replaced by ddp reduce_boolean_decision
        trainer.should_stop = trainer.should_stop or should_stop
        if should_stop:
            print("EarlyStop kicks in===========>")
    ...
    @staticmethod
    def _log_info(trainer: Optional["Trainer"], message: str) -> None:
