...
class LightningWrapper(DALIGenericIterator):
    ...
    def reset(self):
        # workaround to Dali loader not supporting manual reset before epoch is finished
        while not (self._counter >= self._size or self._size < 0):
            print("WARNING: DALIGenericIterator cannot be reset before finish epoch. skip remaining batches")
            super().__next__()
        super().reset()
