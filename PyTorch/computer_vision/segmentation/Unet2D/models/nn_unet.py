...
class NNUnet(pl.LightningModule):
    ...
    def inference2d(self, image):
        ...
        if self.args.hpus:
            ...
            for start in range(0, image.shape[0] - self.args.val_batch_size + 1, self.args.val_batch_size):
                ...
                preds = pred if preds==None else torch.cat((preds, pred), dim=0)
                mark_step(self.args.run_lazy_mode)
            if batch_modulo != 0:
    ...
    def validation_epoch_end(self, outputs):
        # workaround to pl 1.4 mismatch DaliLoader
        # val loader doesn't reach StopIteration so doesn't resets itself
    ...
    def configure_optimizers(self):
        ...
        if self.args.optimizer.lower() == 'sgd':
            ...
            optimizer = torch.optim.AdamW(self.parameters(), lr=self.learning_rate, weight_decay=self.args.weight_decay)
