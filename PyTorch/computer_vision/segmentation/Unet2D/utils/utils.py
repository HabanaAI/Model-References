...
def permute_4d_tensor(tensor, to_filters_last):
    if (tensor.ndim == 4):
        if to_filters_last:
...
def permute_params(model, to_filters_last, lazy_mode):
    with torch.no_grad():
        for name, param in model.named_parameters():
            param.data = permute_4d_tensor(param.data, to_filters_last)
    mark_step(lazy_mode)
...
def get_main_args(strings=None):
    ...
    arg("--bucket_cap_mb", type=positive_int, default=90, help="Size in MB for the gradient reduction bucket size")
    arg(
        '--channels_last',
        default='True',
    ...
    if args.is_hmp:
        ...
        if not args.hmp_bf16:
            args.hmp_bf16 = os.path.join(path, "../config/ops_bf16_unet2d.txt")
        if not args.hmp_fp32:
            args.hmp_fp32 = os.path.join(path, "../config/ops_fp32_unet2d.txt")
    return args
