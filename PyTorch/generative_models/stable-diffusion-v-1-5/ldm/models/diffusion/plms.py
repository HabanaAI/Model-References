...
class PLMSSampler(object):
    ...
    def plms_sampling(self, cond, shape,
        ...
        for i, step in enumerate(timesteps):
            ...
            ts_next_list.append(torch.full((b,), time_range[min(i + 1, len(time_range) - 1)], device=device, dtype=torch.long))
        ts_list = torch.stack([ts for ts in ts_list])
        ts_next_list = torch.stack([ts_next for ts_next in ts_next_list])

        for i, step in enumerate(iterator):
            index = total_steps - i - 1
            ts_list = torch.roll(ts_list, shifts=1, dims=0)
            ts = ts_list[0]
            ts_next_list = torch.roll(ts_next_list, shifts=1, dims=0)
            ts_next = ts_next_list[0]
            ...
            outs = self.p_sample_plms(img, cond, ts, total_steps=total_steps, use_original_steps=ddim_use_original_steps,
    ...
    @torch.no_grad()
    def p_sample_plms(self, x, c, t, total_steps, repeat_noise=False, use_original_steps=False, quantize_denoised=False,
                      temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None,
        ...
        def get_x_prev_and_pred_x0(e_t, b, use_original_steps, device, total_steps):
        ...
        if len(old_eps) == 0:
            ...
            x_prev, pred_x0 = get_x_prev_and_pred_x0(e_t, b, use_original_steps, device, total_steps)
        ...
        x_prev, pred_x0 = get_x_prev_and_pred_x0(e_t_prime, b, use_original_steps, device, total_steps)
