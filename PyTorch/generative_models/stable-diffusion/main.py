...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="path to first_stage checkpoint",
    )
    parser.add_argument(
        "--dataset_path",
        type=str,
...
def worker_init_fn(_):
    ...
    if isinstance(dataset, Txt2ImgIterableBaseDataset):
        split_size = dataset.num_records
        if worker_info.num_workers > 1:
            split_size = split_size // worker_info.num_workers
...
class SetupCallback(Callback):
    def __init__(self, resume, now, logdir, ckptdir, cfgdir, config,
                 lightning_config, debug, no_ckpt):
        super().__init__()
        self.resume = resume
        self.now = now
        self.logdir = logdir
    ...
    def on_keyboard_interrupt(self, trainer, pl_module):
        if not self.debug and not self.no_ckpt and trainer.global_rank == 0:
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not cpu and "gpus" in trainer_config:
        ...
        def melk(*args, **kwargs):
            ...
            if trainer.global_rank == 0 and not opt.no_ckpt:
                print("Summoning checkpoint.")
