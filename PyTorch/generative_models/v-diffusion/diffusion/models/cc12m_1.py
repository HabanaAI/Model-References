...
class ResModConvBlock(ResidualBlock):
    def __init__(self, state, feats_in, c_in, c_mid, c_out, w, is_last=False):
        skip = None if c_in == c_out else nn.Conv2d(c_in, c_out, 1, bias=False)
        super().__init__([
            nn.Conv2d(c_in, c_mid, 3, padding=1),
            nn.LayerNorm([c_mid, w, w], elementwise_affine=False),
            Modulation2d(state, feats_in, c_mid),
            nn.ReLU(inplace=True),
            nn.Conv2d(c_mid, c_out, 3, padding=1),
            nn.LayerNorm([c_out, w, w], elementwise_affine=False) if not is_last else nn.Identity(),
...
class CC12M1Model(nn.Module):
    def __init__(self):
        ...
        self.net = nn.Sequential(   # 256x256
            conv_block(3 + 16, cs[0], cs[0], 256),
            conv_block(cs[0], cs[0], cs[0], 256),
            conv_block(cs[0], cs[0], cs[0], 256),
            conv_block(cs[0], cs[0], cs[0], 256),
            SkipBlock([
                self.down,  # 128x128
                conv_block(cs[0], cs[1], cs[1], 128),
                conv_block(cs[1], cs[1], cs[1], 128),
                conv_block(cs[1], cs[1], cs[1], 128),
                conv_block(cs[1], cs[1], cs[1], 128),
                SkipBlock([
                    self.down,  # 64x64
                    conv_block(cs[1], cs[2], cs[2], 64),
                    conv_block(cs[2], cs[2], cs[2], 64),
                    conv_block(cs[2], cs[2], cs[2], 64),
                    conv_block(cs[2], cs[2], cs[2], 64),
                    SkipBlock([
                        self.down,  # 32x32
                        conv_block(cs[2], cs[3], cs[3], 32),
                        conv_block(cs[3], cs[3], cs[3], 32),
                        conv_block(cs[3], cs[3], cs[3], 32),
                        conv_block(cs[3], cs[3], cs[3], 32),
                        SkipBlock([
                            self.down,  # 16x16
                            conv_block(cs[3], cs[4], cs[4], 16),
                            SelfAttention2d(cs[4], cs[4] // 64),
                            conv_block(cs[4], cs[4], cs[4], 16),
                            SelfAttention2d(cs[4], cs[4] // 64),
                            conv_block(cs[4], cs[4], cs[4], 16),
                            SelfAttention2d(cs[4], cs[4] // 64),
                            conv_block(cs[4], cs[4], cs[4], 16),
                            ...
                            SkipBlock([
                                self.down,  # 8x8
                                conv_block(cs[4], cs[5], cs[5], 8),
                                SelfAttention2d(cs[5], cs[5] // 64),
                                conv_block(cs[5], cs[5], cs[5], 8),
                                SelfAttention2d(cs[5], cs[5] // 64),
                                conv_block(cs[5], cs[5], cs[5], 8),
                                SelfAttention2d(cs[5], cs[5] // 64),
                                conv_block(cs[5], cs[5], cs[5], 8),
                                ...
                                SkipBlock([
                                    self.down,  # 4x4
                                    conv_block(cs[5], cs[6], cs[6], 4),
                                    SelfAttention2d(cs[6], cs[6] // 64),
                                    conv_block(cs[6], cs[6], cs[6], 4),
                                    SelfAttention2d(cs[6], cs[6] // 64),
                                    conv_block(cs[6], cs[6], cs[6], 4),
                                    SelfAttention2d(cs[6], cs[6] // 64),
                                    conv_block(cs[6], cs[6], cs[6], 4),
                                    ...
                                    conv_block(cs[6], cs[6], cs[6], 4),
                                    SelfAttention2d(cs[6], cs[6] // 64),
                                    conv_block(cs[6], cs[6], cs[5], 4),
                                    SelfAttention2d(cs[5], cs[5] // 64),
                                    self.up,
                                ]),
                                conv_block(cs[5] * 2, cs[5], cs[5], 8),
                                SelfAttention2d(cs[5], cs[5] // 64),
                                conv_block(cs[5], cs[5], cs[5], 8),
                                SelfAttention2d(cs[5], cs[5] // 64),
                                conv_block(cs[5], cs[5], cs[5], 8),
                                SelfAttention2d(cs[5], cs[5] // 64),
                                conv_block(cs[5], cs[5], cs[4], 8),
                                SelfAttention2d(cs[4], cs[4] // 64),
                                self.up,
                            ]),
                            conv_block(cs[4] * 2, cs[4], cs[4], 16),
                            SelfAttention2d(cs[4], cs[4] // 64),
                            conv_block(cs[4], cs[4], cs[4], 16),
                            SelfAttention2d(cs[4], cs[4] // 64),
                            conv_block(cs[4], cs[4], cs[4], 16),
                            SelfAttention2d(cs[4], cs[4] // 64),
                            conv_block(cs[4], cs[4], cs[3], 16),
                            SelfAttention2d(cs[3], cs[3] // 64),
                            self.up,
                        ]),
                        conv_block(cs[3] * 2, cs[3], cs[3], 32),
                        conv_block(cs[3], cs[3], cs[3], 32),
                        conv_block(cs[3], cs[3], cs[3], 32),
                        conv_block(cs[3], cs[3], cs[2], 32),
                        self.up,
                    ]),
                    conv_block(cs[2] * 2, cs[2], cs[2], 64),
                    conv_block(cs[2], cs[2], cs[2], 64),
                    conv_block(cs[2], cs[2], cs[2], 64),
                    conv_block(cs[2], cs[2], cs[1], 64),
                    self.up,
                ]),
                conv_block(cs[1] * 2, cs[1], cs[1], 128),
                conv_block(cs[1], cs[1], cs[1], 128),
                conv_block(cs[1], cs[1], cs[1], 128),
                conv_block(cs[1], cs[1], cs[0], 128),
                self.up,
            ]),
            conv_block(cs[0] * 2, cs[0], cs[0], 256),
            conv_block(cs[0], cs[0], cs[0], 256),
            conv_block(cs[0], cs[0], cs[0], 256),
            conv_block(cs[0], cs[0], 3, 256, is_last=True),
        )
