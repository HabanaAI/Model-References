...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    if train_phase == 1:
        ...
        print(f'Create dataset {raw_dataset.dataset_name_clean} for {train_phase=} size={len(chosen_dataset)}')
        ...
        for i, tmp_data in enumerate(current_dataset):
            ...
            if prompt is not None:
                ...
                if prompt_token["input_ids"].size()[-1] <= max_seq_len:
                    ...
                    filtered += 1
        print(f'Create dataset {raw_dataset.dataset_name_clean} '
...
def create_prompt_dataset(local_rank,
    ...
    buf_create_cache = get_accelerator().ByteTensor([not cache_found]).to(device)
    torch.distributed.all_reduce(buf_create_cache)

    if local_rank <= 0 and (buf_create_cache.item() != 0 or reload):
        print(f'Creating dataset {data_path}, {reload=}')
