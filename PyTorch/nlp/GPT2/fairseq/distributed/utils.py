...
def distributed_init(cfg: FairseqConfig):
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            # Todo: SW-78027 - Remove the brute "hccl" assignment once ticket is resolved (keep only the import part)
            if cfg.common.hpu:
                cfg.distributed_training.distributed_backend = "hccl"

            if cfg.distributed_training.distributed_backend is "hccl":
    ...
    do_model_parallel_init = cfg.common.model_parallel_size > 1 or cfg.common.force_model_parallel_init
    if do_model_parallel_init:
        try:
            from fairseq.model_parallel.megatron.mpu import (
                initialize_model_parallel,
                model_parallel_cuda_manual_seed,
                model_parallel_hpu_manual_seed,
            )
        except ImportError:
            raise ImportError(
        ...
        if cfg.common.hpu or cfg.common.cpu:
            model_parallel_hpu_manual_seed(cfg.common.seed)
        else:
...
def distributed_main(i, main, cfg: FairseqConfig, kwargs):
    ...
    # TODO (SW-72036): Temp WA until when solved
...
def call_main(cfg: FairseqConfig, main, **kwargs):
    if cfg.distributed_training.distributed_init_method is None:
        infer_init_method(cfg.distributed_training, force_distributed=cfg.common.force_model_parallel_init)
