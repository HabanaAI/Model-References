...
from fairseq.models.transformer import (
    ...
    TransformerModel,
    TransformerConfig,
    handle_transformer_checkpoint_activations,
)
...
class ModelParallelTransformerEncoder(TransformerEncoder):
    ...
    def build_encoder_layer(self, args):
        layer = ModelParallelTransformerEncoderLayer(args)
        cfg = TransformerConfig.from_namespace(args)
        layer = handle_transformer_checkpoint_activations(layer, cfg, use_rng_tracker=True)
        # TODO: [SW-63547] [TFTF Fairseq] support FSDP for model parallel architecture
        # layer = handle_transformer_fsdp_wrapping(layer, cfg)
        return layer
...
class ModelParallelTransformerDecoder(TransformerDecoder):
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        layer = ModelParallelTransformerDecoderLayer(args, no_encoder_attn)
        cfg = TransformerConfig.from_namespace(args)
        layer = handle_transformer_checkpoint_activations(layer, cfg, use_rng_tracker=True)
        # TODO: [SW-63547] [TFTF Fairseq] support FSDP for model parallel architecture
        # layer = handle_transformer_fsdp_wrapping(layer, cfg)
