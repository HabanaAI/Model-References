...
class ModelParallelMultiheadAttention(nn.Module):
    ...
    def __init__(
        ...
        def init_method(tensor, gain=1/math.sqrt(2)):
            return nn.init.xavier_uniform_(tensor, gain)

        self.k_proj = ColumnParallelLinear(
            self.kdim, embed_dim, bias=bias, gather_output=False, init_method=init_method
        )
        self.v_proj = ColumnParallelLinear(
            self.vdim, embed_dim, bias=bias, gather_output=False, init_method=init_method
        )
        self.q_proj = ColumnParallelLinear(
            embed_dim, embed_dim, bias=bias, gather_output=False, init_method=init_method
        )
        self.out_proj = RowParallelLinear(
            embed_dim, embed_dim, bias=bias, input_is_parallel=True, init_method=init_method
        )
    ...
    def forward(
        ...
        **unused_kwargs,
        ...
        if query.device.type == "hpu" or query.device.type == "cpu":
            with get_hpu_rng_tracker().fork():
