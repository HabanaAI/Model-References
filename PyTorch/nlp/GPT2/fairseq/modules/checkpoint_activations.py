...
def checkpoint_wrapper(m, offload_to_cpu=False, use_rng_tracker=False):
    """
...
def _checkpointed_forward(original_forward, offload_to_cpu, use_rng_tracker, *args, **kwargs):
    ...
    parent_ctx_dict = {
        ...
        "use_rng_tracker": use_rng_tracker
    }
...
class CheckpointFunction(torch.autograd.Function):
    ...
    def forward(ctx, run_function, parent_ctx_dict, kwarg_keys, *args):
        ...
        ctx.fwd_rng_state = utils.get_rng_state(use_rng_tracker=parent_ctx_dict["use_rng_tracker"])
    ...
    def backward(ctx, *args):
        ...
        # In case of activation checkpointing and lazy-mode1, need to flush the graph before the recompute of the fwd
        # in order to ensure lower memory footprint
        # TODO: SW-76953 - currently with try/except in order to fit non-HPU PT device types. need to make it cleaner.
        try:
