...
def move_to_hpu(sample, device):
    def _move_to_hpu(tensor):
        return tensor.to(device)

    return apply_to_sample(_move_to_hpu, sample)
...
def _verify_has_megatron():
    global get_cuda_rng_tracker
    global get_hpu_rng_tracker

    if get_cuda_rng_tracker is None or get_hpu_rng_tracker is None:
        try:
            from fairseq.model_parallel.megatron.mpu import get_cuda_rng_tracker as get_cuda_rng_tracker
            from fairseq.model_parallel.megatron.mpu import get_hpu_rng_tracker as get_hpu_rng_tracker
...
def get_rng_state(use_rng_tracker=False):
...
def set_rng_state(state):
    ...
    if torch.cuda.is_available():
        ...
        if "cuda_rng_tracker_states" in state:
            _verify_has_megatron()
            get_cuda_rng_tracker().set_states(state["cuda_rng_tracker_states"])
    if hpu is not None and "hpu_rng_tracker_states" in state:
        _verify_has_megatron()
