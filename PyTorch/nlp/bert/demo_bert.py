...
def args_bert_common(params: BertParams):
    ...
    params.add_argument('--mode', type=str, default=None, choices=['eager', 'graph'], help='Execution mode')
    params.add_argument('-d', '--device', type=str, default='habana', help='Device on which to run on')
...
def args_bert_pretraining(params: BertParams):
    ...
    params.add_argument('--init-checkpoint', nargs=2, type=str, default=[None, None], help='Initial checkpoints')
    params.add_argument('--resume_training', action='store_true', help='Enable resuming training from checkpoint')
    ...
    params.add_argument('--all_reduce_post_accumulation', action='store_true',
                        help='Enable allreduces during gradient accumulation steps')
    params.add_argument('--all_reduce_post_accumulation_fp16', action='store_true',
...
def build_bert_envs(params, paths):
    # common env vars for bert
    params.add_env('PT_HABANA_LOG_MOD_MASK', 'FFFF')
    params.add_env('PT_HABANA_LOG_TYPE_MASK', 1)
    params.add_env('RUN_TPC_FUSER', 1)
    params.add_env('HCL_CPU_AFFINITY', 1)
    # env vars for graph mode
    if params.get('mode') == 'graph':
        params.add_env('PT_HABANA_ENABLE_GRAPHMODE_LAYERNORM_FUSION', 1)
        params.add_env('HABANA_GRAPH_FUSION_OPS_FILE',  f"{paths['demo_config_path']}/BERT_Fusion_Ops.txt")
        params.add_env('HABANA_PGM_ENABLE_CACHE', 15)
    # add mpirun env vars
...
def bert_finetuning(params, paths):
    ...
    params += '--use_habana' if params.device == 'habana' else '--no_cuda'
...
def bert_pretraining(params, paths):
    ...
    p1_train_batch_size = 8192
    p2_train_batch_size = 4096
    ...
    if params.get('init-checkpoint'):
        p1_init_checkout = params.get('init-checkpoint')[0]
        p2_init_checkout = params.get('init-checkpoint')[1]
        if p1_init_checkout:
            p1_checkpoint = f"--resume_from_checkpoint --init_checkpoint={p1_init_checkout}"
        else:
            p1_checkpoint = ""
        if p2_init_checkout:
            p2_checkpoint = f"--resume_from_checkpoint --init_checkpoint={p2_init_checkout}"
        else:
    ...
    params += '--use_habana' if params.device == 'habana' else '--no_cuda'
    params += '--resume_from_checkpoint' if params.resume_training else None
    params += '--allreduce_post_accumulation' if params.all_reduce_post_accumulation else None
    params += '--allreduce_post_accumulation_fp16' if params.all_reduce_post_accumulation_fp16 else None
...
if __name__ == '__main__':
    ...
    if params.get_subparser() == 'finetuning':
        ...
        print('Invalid subtask: ', params.get_subparser())
        exit(1)
