...
# - added StaticMaxLengthCriteria
# - removed static decorator from _update_model_kwargs_for_generation
# - added token_idx tensor support to _update_model_kwargs_for_generation
# - added max_steps param to _get_stopping_criteria
# - added max_steps, ignore_eos, pre_token_hook, post_token_hook, search_on_cpu
#   params to generate, greedy_search and beam_search
# - added support for static shapes in generate, greedy_search and beam_search
...
class StaticMaxLengthCriteria(MaxLengthCriteria):
    def __init__(self, max_length: int, max_steps: int):
...
class GenerationMixin:
    ...
    def _get_stopping_criteria(
        self, max_length: Optional[int], max_time: Optional[float], max_steps: Optional[int], stopping_criteria: Optional[StoppingCriteriaList]
    ) -> StoppingCriteriaList:
        criteria = StoppingCriteriaList()
        if max_steps is not None:
            criteria.append(StaticMaxLengthCriteria(max_length=max_length, max_steps=max_steps))
        if max_length is not None and max_steps is None:
    ...
    def generate(
        ...
        pre_token_hook: [Callable[[], None]] = [],
        post_token_hook: [Callable[[], None]] = [],
        search_on_cpu: Optional[bool] = None,
        ...
        **model_kwargs,
        ...
        if input_ids_seq_length >= max_length and 'token_idx' not in model_kwargs:
        ...
        stopping_criteria = self._get_stopping_criteria(
            max_length=max_length, max_time=max_time, max_steps=max_steps, stopping_criteria=stopping_criteria
        )
        ...
        if is_greedy_gen_mode:
            ...
            return self.greedy_search(
                ...
                pre_token_hook=pre_token_hook,
                post_token_hook=post_token_hook,
                **model_kwargs,
            )
            ...
            beam_scorer = BeamSearchScorer(
                ...
                num_beams=num_beams,
                device='cpu' if search_on_cpu else inputs_tensor.device,
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            if token_idx is not None:
                input_ids.index_copy_(1, token_idx, next_tokens.unsqueeze(-1) if next_tokens.dim() == 1 else next_tokens)
            else:
    ...
    def beam_search(
        ...
        **model_kwargs,
        ...
        move_to_cpu_if_needed = (lambda x: x.cpu()) if search_on_cpu else (lambda x: x)
        this_peer_finished = False  # used by synced_gpus only
        while True:
            for clb in pre_token_hook:
                clb()
            ...
            if token_idx is not None:
                if outputs.logits.shape[-2] > 1:
                    next_token_logits = torch.index_select(outputs.logits, 1, token_idx-1).squeeze(-2)
                else:
                    next_token_logits = outputs.logits.squeeze(-2)
            else:
                next_token_logits = outputs.logits[:, -1, :]
            ...
            beam_outputs = beam_scorer.process(
                move_to_cpu_if_needed(input_ids),
                move_to_cpu_if_needed(next_token_scores),
                move_to_cpu_if_needed(next_tokens),
                move_to_cpu_if_needed(next_indices),
                pad_token_id=pad_token_id,
                eos_token_id=eos_token_id,
                beam_indices=beam_indices,
            )

            beam_scores = beam_outputs["next_beam_scores"].to(input_ids.device)
            beam_next_tokens = beam_outputs["next_beam_tokens"].to(input_ids.device)
            beam_idx = beam_outputs["next_beam_indices"].to(input_ids.device)

            if token_idx is not None:
                input_ids = input_ids[beam_idx, :]
                input_ids.index_copy_(1, token_idx, beam_next_tokens.unsqueeze(-1))
            else:
                input_ids = torch.cat([input_ids[beam_idx, :], beam_next_tokens.unsqueeze(-1).to(input_ids.device)], dim=-1)
        ...
        sequence_outputs = beam_scorer.finalize(
            move_to_cpu_if_needed(input_ids),
            move_to_cpu_if_needed(beam_scores),
            move_to_cpu_if_needed(next_tokens),
            move_to_cpu_if_needed(next_indices),
            pad_token_id=pad_token_id,
            eos_token_id=eos_token_id,
