...
def build_alibi_tensor(attention_mask: torch.Tensor, slopes: torch.Tensor, n_head: int, dtype, device) -> torch.Tensor:
    """
...
def dropout_add(x, residual, prob, training):
    ...
    Args:
        ...
        training (`bool`, *required*):
            training mode
    """
    out = nn.functional.dropout(x, p=prob, training=training) if training else x
    out = residual + out
    return out
    ...
    def forward(
        ...
        if layer_past is not None:
            ...
            if token_idx is not None:
                # HPU bug WA
                past_key.index_add_(1, token_idx-1, key_layer - torch.index_select(past_key, 1, token_idx-1))
        ...
        attention_probs = attention_probs * neg_attention_mask
...
class BloomModel(BloomPreTrainedModel):
    def __init__(self, config):
        ...
        self.register_buffer('alibi_slope', build_alibi_slope_tensor(self.n_head), persistent=False)
    ...
    def forward(
        ...
        alibi = build_alibi_tensor(attention_mask, self.alibi_slope, self.n_head, hidden_states.dtype, hidden_states.device)

        causal_mask = self._prepare_attn_mask(attention_mask, input_shape, inputs_embeds, past_key_values_length)
        neg_causal_mask = ~causal_mask.bool()
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past_key_values)):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                outputs = block(
                    ...
                    neg_attention_mask=neg_causal_mask,
