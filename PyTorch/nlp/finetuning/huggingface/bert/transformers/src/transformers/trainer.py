...
class Trainer:
    ...
    def __init__(
        ...
        # Disable WANDB reports by default
        os.environ['WANDB_DISABLED'] = "true"

        self.use_lazy_mode = args.use_lazy_mode
        if args.use_hpu:
        ...
        if self.args.use_fused_clip_norm:
            ...
            self.FusedNorm = FusedClipNorm(model.parameters(), self.args.max_grad_norm)
    ...
    def _wrap_model(self, model, training=True):
        ...
        if self.fsdp is not None:
            ...
            model = nn.parallel.DistributedDataParallel(
                model,
                device_ids=[self.args.local_rank] if self.args._n_gpu != 0 and not self.args.use_hpu else None,
                output_device=self.args.local_rank if self.args._n_gpu != 0 and not self.args.use_hpu else None,
                **kwargs,
            )
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    and (not (args.use_hpu or is_torch_tpu_available()))
                ):
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0 or (
                    ...
                    if args.max_grad_norm is not None and args.max_grad_norm > 0 and not self.deepspeed:
                        ...
                        elif self.args.use_fused_clip_norm:
                    ...
                    if self.deepspeed:
                        ...
                        if self.args.use_hpu and self.args.hmp and not(self.args.use_fused_adam):
                            from habana_frameworks.torch.hpex import hmp
                            with hmp.disable_casts():
                                self.optimizer.step()
                        else:
