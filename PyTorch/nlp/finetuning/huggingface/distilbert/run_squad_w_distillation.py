...
def train(args, train_dataset, model, tokenizer, teacher=None):
    ...
    for _ in train_iterator:
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            if args.hpu:
                batch = [b.type(torch.IntTensor) if b.dtype == torch.int64 else b for b in batch]
...
def main():
    ...
    if args.hpu:
        ...
        if args.local_rank != -1:
            try:
                ...
                assert False, "Could not import habana_frameworks.torch.distributed.hccl"
    ...
    if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.hpu:
            model_to_save = (model.module.to("cpu") if hasattr(model, "module") else model.to("cpu"))
        else:
            model_to_save = (
                model.module if hasattr(model, "module") else model
                )  # Take care of distributed/parallel training
