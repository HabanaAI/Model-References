...
from fairseq.dataclass.utils import convert_namespace_to_omegaconf,str_to_bool
...
class Trainer(object):
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        self.hpu = cfg.common.use_habana
        self.cfg.common.use_lazy_mode = str_to_bool(self.cfg.common.use_lazy_mode)
        ...
        if self.cfg.common.use_lazy_mode:
            try:
        ...
        if self.cfg.common.use_habana:
             self._criterion = self._criterion.to(device=self.device)
             self._model = self._model.to(device=self.device)

        if self.cfg.common.use_fused_clip_norm:
            try:
                from habana_frameworks.torch.hpex.normalization import FusedClipNorm
            except ImportError:
                raise ImportError("Please install habana_torch.")
            self.FusedNorm = FusedClipNorm(model.parameters(), self.cfg.optimization.clip_norm)
    ...
    def load_checkpoint(
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            if self.cfg.common.use_habana:
                if self.cfg.optimizer.use_fused_adam:
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("clip-grads"):
                # clip grads
                if self.cfg.common.use_fused_clip_norm:
                    grad_norm = self.FusedNorm.clip_norm(self.model.parameters())
                else:
                    grad_norm = self.clip_grad_norm(self.cfg.optimization.clip_norm)

            # check that grad norms are consistent across workers
            # on tpu check tensor is slow
            if not self.tpu and not self.hpu:
                if (
                    not self.cfg.optimization.use_bmuf
    ...
    def _prepare_sample(self, sample, is_dummy=False):
        ...
        elif self.cfg.common.use_habana:
