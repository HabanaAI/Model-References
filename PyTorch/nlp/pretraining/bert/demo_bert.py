...
def args_bert_common(params: BertParams):
    ...
    params.add_argument('--mode', type=str, default='lazy', choices=['eager', 'graph', 'lazy'], help='Execution mode')
...
def args_bert_pretraining(params: BertParams):
    ...
    params.add_argument('--enable_packed_data_mode', action='store_true', default=False,
                        help=' Use packed data')
    params.add_argument('--avg_seq_per_pack', nargs=2, type=float, default=[1.2, 2],
                        help='Average number of sequences per packed sample. should be 1.2/2 for phase1/2 for bookswiki')
    return params
...
def build_mode_args(pars):
    ...
    if pars.mode:
        pars += '--use_lazy_mode false' if pars.mode == 'eager' else None
...
def bert_pretraining(params, paths):
    ...
    if params.enable_packed_data_mode:
        params += ('--enable_packed_data_mode')
    ...
    if params.enable_packed_data_mode:
        ph1_params += ('--avg_seq_per_pack',params.avg_seq_per_pack[0])
        ph2_params += ('--avg_seq_per_pack',params.avg_seq_per_pack[1])
    ph2_params += '--phase2'
