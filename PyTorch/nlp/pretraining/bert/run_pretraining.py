            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if args.local_rank != -1 and not args.allreduce_post_accumulation \
                        ...
                        with model.no_sync():
                            loss = model(input_ids=input_ids, token_type_ids=segment_ids, attention_mask=input_mask, masked_lm_labels=masked_lm_labels, next_sentence_labels=next_sentence_labels)
                    else:
                        if args.local_rank != -1:
