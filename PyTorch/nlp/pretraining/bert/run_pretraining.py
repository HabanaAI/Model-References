...
def parse_arguments():
    ...
    parser.add_argument('--enable_packed_data_mode', action='store_true',default=False,
                        help='Whether to enable training with packed data. If enabled, packed data should be provided to --input_dir.')
    parser.add_argument('--avg_seq_per_pack', type=float, default=1.0,
                        help='Average number of sequences per packed sample.')
...
def setup_training(args):
    ...
    if args.use_habana:
        ...
        os.environ['PT_HPU_USE_PT_STORE_SYNC'] = 'true'
        if args.hmp:
            print(args.hmp_bf16)
    ...
    if args.enable_packed_data_mode:
        args.gradient_accumulation_steps = round(args.gradient_accumulation_steps / args.avg_seq_per_pack)
    else:
        args.avg_seq_per_pack = 1.0
...
def main():
    ...
    if args.use_habana:
        try:
            ...
            print("Warning ! set num_workers to zero, unsupported start method")
            pass
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if global_step >= args.steps_this_run or timeout_sent or training_steps % (args.log_freq * args.gradient_accumulation_steps) == 0:
                        ...
                        average_perf_per_step = args.train_batch_size*args.avg_seq_per_pack/average_training_time_per_step
...
if __name__ == "__main__":
    ...
    if is_main_process():
        ...
        training_perf = args.train_batch_size * args.gradient_accumulation_steps * pu_count * args.avg_seq_per_pack\
