...
from utils import is_main_process, format_step, get_world_size, get_rank, fix_tensor_numpy
...
OPTIMIZERS_ALL = ('nvlamb','nvlamb_experimental' 'adam', 'adamw') + OPTIMIZERS_CUDA_ONLY
...
def prepare_model_and_optimizer(args, device):
    ...
    if args.optimizer == 'fused_lamb':
        ...
        optimizer = optimizers.FusedLAMB(optimizer_grouped_parameters, lr=args.learning_rate)
    elif args.optimizer == 'ds_fused_lamb':
        print('Using DeepSpeed FusedLamb')
        optimizer = DeepSpeedFusedLamb(optimizer_grouped_parameters, lr=args.learning_rate)
    elif args.optimizer == 'nvlamb':
        print('Using NVLamb')
        optimizer = NVLAMB(optimizer_grouped_parameters, lr=args.learning_rate, adjust_step=args.zero_optimization)
    elif args.optimizer == 'nvlamb_experimental':
        print('Using NVLambExperimental')
        optimizer = NVLAMB_EXP(optimizer_grouped_parameters, lr=args.learning_rate, max_grad_norm=0., eps=1e-8,
                           adjust_step=args.zero_optimization, use_nvlamb=True, max_trust=10., min_trust=0.01)
    ...
    if args.do_train:
        ...
        while True:
            ...
            if not args.resume_from_checkpoint or epoch > 0 or (args.phase2 and global_step < 1):
                ...
                if 'file_start_id' in checkpoint.keys():
                    f_start_id = checkpoint['file_start_id']
                    files = checkpoint['files']
                    skip_steps = checkpoint['step'] + 1
                    print(f"Skipping {skip_steps} micro steps.")
                else:
                    f_start_id = checkpoint['files'][0]
                    files = checkpoint['files'][1:]
                    print(f"Legacy checkpoint format found: Start training from data file at offset 0.")
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if args.use_hpu and args.use_lazy_mode:
                        htcore.mark_step()
                    ...
                    if global_step >= args.steps_this_run or training_steps % (
                        ...
                        if not args.skip_checkpoint:
                            ...
                            checkpoint_dict ={}
                            if args.do_train:
                                checkpoint_dict = {'file_start_id': f_id_curr,
                                                   'files': files,
                                                   'epoch': epoch,
                                                   'step': step}
