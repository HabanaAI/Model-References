...
class SequenceGenerator(nn.Module):
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if avg_attn_scores is not None:
                if attn is None:
                    ...
                    attn = attn.fill_(0.0)
                attn = attn.to('cpu')
                avg_attn_scores = avg_attn_scores.to('cpu')
                attn[:, :, step + 1].copy_(avg_attn_scores)
                attn = attn.to('hpu')
                avg_attn_scores = avg_attn_scores.to('hpu')
    ...
    def finalize_hypos(
        ...
        eos_scores = eos_scores.to('cpu')
        pos_scores[:, step] = eos_scores
        # convert from cumulative to per-position scores
        pos_scores[:, 1:] = pos_scores[:, 1:] - pos_scores[:, :-1]
        eos_scores = eos_scores.to('hpu')
        pos_scores = pos_scores.to('hpu')
        ...
        for i in range(bbsz_idx.size()[0]):
            ...
            if len(finalized[sent]) < beam_size:
                if attn_clone is not None:
                    ...
                    attn_clone = attn_clone.to('cpu')
                    hypo_attn = attn_clone[i]
                    attn_clone = attn_clone.to('hpu')
                else:
