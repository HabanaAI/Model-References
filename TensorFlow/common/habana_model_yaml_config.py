...
class HabanaModelYamlConfig():
        ...
        try:
            with open(config_path, "r") as hb_config:
                ...
                if self.model != self.parsed_config['model']:
                    raise Exception(f"{self.args.hb_config} has a different model name \"{self.parsed_config['model']}\" than \"{self.model}\"")
    ...
    def get_parameter(self, key):
        return self.model_parameters[key] if self.model_parameters[key] is not None else None
    ...
    # Example usage: See bert_base_default.yaml and the filter 'dataset_parameters/mrpc/data_type_parameters/bf16'
