...
def get_lib_path(directory, lib):
    versionned_lib_path = os.path.abspath(os.path.join(directory, lib + '.' + tf.__version__))
    # TODO: Remove once TF-training switches to lib format with full TF version suffix
    short_versionned_lib_path = os.path.abspath(os.path.join(directory, lib + '.' + tf.__version__.rsplit('.', 1)[0]))

    return versionned_lib_path if os.path.isfile(versionned_lib_path) else short_versionned_lib_path
...
def load_habana_module():
    ...
    # WA to make smooth transition to Wheel.
    # In wheel libs are not auto-found by OS and need to be explicitly loaded
    ctypes.CDLL(os.path.join(habana_modules_directory, "libsynapse_helpers.so." + tf.__version__),
                ctypes.RTLD_GLOBAL)
