  ...
  def write_summaries(self, items, always_write=True):
    ...
    with self._summary_writer.as_default():
      for name, tensor in items.items():
        # explicit cast is required here because global_step might be an int32
        # due to some constraints with device placement of consts
        self._summary_fn(name, tensor, step=tf.cast(self._global_step, tf.int64))
    return True
