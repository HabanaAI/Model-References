...
class ResNet_Cifar(ModelDesc):
    ...
    def build_graph(self, image, label):
        ...
        pytorch_default_init = tf.compat.v1.variance_scaling_initializer(scale=1.0 / 3, mode='fan_in', distribution='uniform')
        with argscope([Conv2D, BatchNorm, GlobalAvgPooling], data_format='channels_first'), \
            ...
            for i, blocks_in_module in enumerate(MODULE_SIZES):
                for j in range(blocks_in_module):
                    ...
                    with tf.compat.v1.variable_scope("res%d.%d" % (i, j)):
                        net = preactivation_block(net, FILTER_SIZES[i], stride)
            net = GlobalAvgPooling('gap', net)
            logits = FullyConnected('linear', net, CLASS_NUM,
                                    kernel_initializer=tf.compat.v1.random_normal_initializer(stddev=1e-3))

        ce_cost = tf.nn.softmax_cross_entropy_with_logits(labels=tf.stop_gradient(label), logits=logits)
        ce_cost = tf.reduce_mean(input_tensor=ce_cost, name='cross_entropy_loss')

        single_label = tf.cast(tf.argmax(input=label, axis=1), tf.int32)
        wrong = tf.cast(tf.logical_not(tf.nn.in_top_k(predictions=logits, targets=single_label, k=1)), tf.float32, name='wrong_vector')
        # monitor training error
        add_moving_summary(tf.reduce_mean(input_tensor=wrong, name='train_error'), ce_cost)
