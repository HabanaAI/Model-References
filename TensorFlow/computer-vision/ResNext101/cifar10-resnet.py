    ...
    def build_graph(self, image, label):
        ...
        def residual(name, l, increase_dim=False, first=False):
            ...
            with tf.compat.v1.variable_scope(name):
                ...
                if increase_dim:
                    ...
                    l = tf.pad(tensor=l, paddings=[[0, 0], [in_channel // 2, in_channel // 2], [0, 0], [0, 0]])
        ...
        with argscope([Conv2D, AvgPooling, BatchNorm, GlobalAvgPooling], data_format='channels_first'), \
                argscope(Conv2D, use_bias=False, kernel_size=3,
                         kernel_initializer=tf.compat.v1.variance_scaling_initializer(scale=2.0, mode='fan_out')):
        ...
        cost = tf.reduce_mean(input_tensor=cost, name='cross_entropy_loss')

        wrong = tf.cast(tf.logical_not(tf.nn.in_top_k(predictions=logits, targets=label, k=1)), tf.float32, name='wrong_vector')
        # monitor training error
        add_moving_summary(tf.reduce_mean(input_tensor=wrong, name='train_error'))

        # weight decay on all W of fc layers
        wd_w = tf.compat.v1.train.exponential_decay(0.0002, get_global_step_var(),
                                          480000, 0.2, True)
