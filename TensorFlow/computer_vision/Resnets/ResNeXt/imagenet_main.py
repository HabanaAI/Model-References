...
def limit_cpus(flags_obj):
  """Limit the number of cpus to max_cpus if this flag is set."""

  if flags_obj.max_cpus is not None:
    affinity = os.sched_getaffinity(0)
    print(f"Host: {platform.node()}, CPU count = {os.cpu_count()}, affinity = {affinity}")
    if len(affinity) > flags_obj.max_cpus:
      new_affinity = set(list(affinity)[:flags_obj.max_cpus])
      print(f"Restricting CPUs to {new_affinity} because max_cpus={flags_obj.max_cpus}")
      os.sched_setaffinity(0, new_affinity)

def main(_):
  limit_cpus(flags.FLAGS)
...
if __name__ == '__main__':
  ...
  flags.DEFINE_integer(
    name='max_cpus', short_name='mc', default=None,
    help='Maximum amount of CPUs to use by the script')
