...
def resnet_main(
  ...
  if flags_obj.profiling:
    train_hooks.append(TensorBoardHook(output_dir=model_dir))
...
def define_resnet_flags(resnet_size_choices=None, dynamic_loss_scale=False,
  ...
  flags.DEFINE_boolean(
    name='profiling', default=False,
    help=flags_core.help_wrap(
        "turn on profiling (generate json for steps 20-24)"))
