...
# - changed default save_summary_steps to 100
...
from TensorFlow.computer_vision.Resnets.ResNeXt.utils import export
...
def process_record_dataset(dataset,
  ...
  if experimental_preloading:
    # In newest version of TF, optimization introduces implicit prefetching
    # This lead to copying image to device on the beginning of iteration
    # Copying image consume DMA throughput that could delay transfer other
    #  data which are needed for compute
    # By disabling optimization, we postpone image transfer which leaves DMA
    #  free for more critical data
    options = tf.data.Options()
...
def learning_rate_with_decay(
    ...
    decay_rates, base_lr=0.1, warmup=False, warmup_epochs=5.0, use_cosine_lr=False):
...
def resnet_model_fn(features, labels, mode, model_class,
  ...
  if not flags.FLAGS.enable_lars:
  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    if flags.FLAGS.enable_lars:
      ...
      optimizer = LARSOptimizer.LARSOptimizer(
          learning_rate,
          momentum=momentum,
...
def resnet_main(
  ...
  if flags_obj.disable_checkpoints:
    ...
    save_checkpoints_steps = steps_per_epoch * flags_obj.train_epochs
  ...
  def input_fn_train(num_epochs, input_context=None, is_training=True):
    return input_function(
        ...
        jpeg_data_dir=flags_obj.jpeg_data_dir,
  ...
  train_hooks.append(ExamplesPerSecondEstimatorHook(
    ...
    log_global_step=True, tags_to_print=['global_examples/sec', 'examples/sec']))
  ...
  if flags_obj.export_dir is not None:
    ...
    if flags_obj.image_bytes_as_serving_input:
      ...
      classifier.export_saved_model(flags_obj.export_dir, input_receiver_fn)
    else:
      features = tf.compat.v1.placeholder(
        dtype=export_dtype, shape=[flags_obj.batch_size] + shape, name='input_tensor')
      labels = tf.compat.v1.placeholder(
        dtype=tf.int32, shape=[flags_obj.batch_size], name='labels')
      input_receiver_fn_train = tf.estimator.experimental.build_raw_supervised_input_receiver_fn(features, labels)
      input_receiver_fn_predict = export.build_tensor_serving_input_receiver_fn(
          shape, batch_size=flags_obj.batch_size, dtype=export_dtype)
      input_receiver_fn_map={tf.estimator.ModeKeys.TRAIN : input_receiver_fn_train,
                            tf.estimator.ModeKeys.PREDICT : input_receiver_fn_predict}
      classifier.experimental_export_all_saved_models(flags_obj.export_dir, input_receiver_fn_map)
...
def define_resnet_flags(resnet_size_choices=None, dynamic_loss_scale=False,
  ...
  flags.DEFINE_float(
      name='warmup_epochs', default=8.0,
      help=flags_core.help_wrap(
  ...
  flags.DEFINE_enum(
      ...
      help=flags_core.help_wrap(
          'Residual model type.'))
  flags.DEFINE_integer(
      name='save_summary_steps', default=100,
      help=flags_core.help_wrap(
  ...
  flags.DEFINE_string(
    name='profile_steps', default="7,10",
    help=flags_core.help_wrap(
        "Steps range to profile (aka --profile_steps 7,10)"))
