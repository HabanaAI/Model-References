...
2. Follow [Training](#training) above on all servers
3. Configure ssh between servers (inside all dockers). For AWS users, ensure that the IP addresses of the servers are in the same security group and that all traffic is allowed between them. See https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/authorizing-access-to-an-instance.html for details:
Do the following in all server docker containers:
```
mkdir ~/.ssh
cd ~/.ssh
...
```
MPI_TCP_INCLUDE=interface_name MULTI_HLS_IPS=192.10.100.174,10.10.100.101 $PYTHON demo_resnet_keras.py -dt bf16 -dlit bf16 --use_horovod --num_workers_per_hls 8 -te 40 -ebe 40 --optimizer LARS --experimental_preloading --data_dir /data/tensorflow_datasets/imagenet/tf_records
```
...
$PYTHON demo_resnet_keras.py -dt bf16 -dlit bf16 --experimental_preloading -bs 256 -te 90 -ebe 90
...
- ResNet50, 8 Gaudis on 1 server, batch 256, 40 epochs, bf16 precision, LARS, experimental preloading
    ```bash
    $PYTHON demo_resnet_keras.py --dtype bf16 --data_loader_image_type bf16 --use_horovod --num_workers_per_hls 8 -te 40 -ebe 40 -bs 256 --optimizer LARS --experimental_preloading
...
socket:PE = floor((number of physical cores) / (number of gaudi devices per each node))*
    ```bash
    mpirun --allow-run-as-root --bind-to core -np 8 --map-by socket:PE=7 --merge-stderr-to-stdout $PYTHON demo_resnet_keras.py --dtype bf16 --data_loader_image_type bf16 --use_horovod --num_workers_per_hls 8 --kubernetes_run=True -te 40 -ebe 40 -bs 256 --optimizer LARS --experimental_preloading
    ```
...
- You can also run the community script (resnet_ctl_imagenet_main.py) directly with mpirun. The following command will be roughly equivalent to the one presented above*:
    ```bash
    mpirun --allow-run-as-root --bind-to core -np 8 --map-by socket:PE=7 --merge-stderr-to-stdout $PYTHON resnet_ctl_imagenet_main.py --dtype bf16 --data_loader_image_type bf16 --use_horovod -te 40 -ebe 40 -bs 256 --optimizer LARS --base_learning_rate 9.5 --warmup_epochs 3 --lr_schedule polynomial --label_smoothing 0.1 --weight_decay 0.0001 --single_l2_loss_op --experimental_preloading
    ```
...
- ResNet50, 8 Gaudis on 1 server, batch 256, 40 epochs, bf16 precision, LARS
    ```bash
    $PYTHON demo_resnet_keras.py -dt bf16 -dlit bf16 --batch_size 2048 --distribution_strategy=hpu --num_workers_per_hls 8 -te 40 -ebe 40 --optimizer LARS --experimental_preloading
    ```
Note:
...
- ResNet50, 16 Gaudis on 2 boxes, batch 256, 40 epochs, bf16 precision, LARS, experimental preloading enabled
    ...
    $PYTHON demo_resnet_keras.py -dt bf16 -dlit bf16 -bs 256 --use_horovod --num_workers_per_hls 8 -te 40 -ebe 40 --optimizer LARS --experimental_preloading --data_dir /data/tensorflow_datasets/imagenet/tf_records
    ```
Note that to run multi-server over host NICs, use `--horovod_hierarchical_allreduce` flag. This is required for AWS users.
...
- Resnet 50 training on 8 HPU 1 box - Horovod
    ```bash
    $PYTHON demo_resnet_keras.py -dt bf16 -dlit bf16 -te 40 -ebe 40 --steps_per_loop 1000 --experimental_preloading --optimizer LARS --use_horovod --num_workers_per_hls 8 -bs 256
    ```
