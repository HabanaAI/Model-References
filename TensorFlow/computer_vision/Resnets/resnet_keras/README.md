...
- Environment variable `HCCL_OVER_TCP=1` must be set to enable multi-server in HCCL.
...
- ResNet50, 16 Gaudis on 2 boxes, batch 256, 40 epochs, bf16 precision, LARS, experimental preloading enabled
    ...
    $PYTHON demo_resnet_keras.py -dt bf16 -dlit bf16 --distribution_strategy hpu --num_workers_per_hls 8 -bs 4096 --optimizer LARS
    ```
