...
###############################################################################
# List of changes:
# - added logic for saving full models after every trainig epoch in HDF5 format
...
from TensorFlow.computer_vision.Resnets.resnet_keras.orbit import utils
...
class Controller(object):
  ...
  def __init__(
      ...
      model_to_save: Optional[tf.keras.Model] = None,
      # summary related
    ...
    Args:
      ...
      model_to_save: An instance of `tf.keras.Model` used to save the model state.
  ...
  def _maybe_save_model(self):
    if self.model_to_save:
      self.model_to_save.save(self.checkpoint_manager.directory + "/resnet_model.h5", save_format='h5')
      logging.info("Saved HDF5 model under %s", self.checkpoint_manager.directory)
