  ...
  if mode == tf.estimator.ModeKeys.TRAIN:
    ...
    if flags.FLAGS.enable_lars:
      if model_type == 'resnext':
        raise flags.ValidationError("Cannot run ResNeXt with LARS optimizer. Unsupported configuration.")
...
def resnet_main(
  ...
  if flags_obj.dummy_epoch:
    ...
    is_mlperf_enabled = flags.FLAGS.is_mlperf_enabled
    flags.FLAGS.is_mlperf_enabled = False   # disable mlperf logging during dummy epoch
    dummy_classifier.train(
        input_fn=lambda input_context=None: input_fn_train(
            1, input_context=input_context, is_training=False),
        max_steps=2)
    flags.FLAGS.is_mlperf_enabled = is_mlperf_enabled
