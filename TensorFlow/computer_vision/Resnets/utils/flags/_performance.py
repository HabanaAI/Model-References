...
###############################################################################
# List of changes:
# - added the 'bf16_config_path' parameter
# - added some imports
# - disabled loss scaling for bf16
# - added training_dataset_cache=False to define_performance
...
def get_loss_scale(flags_obj, default_for_fp16):
  if flags_obj.loss_scale == "dynamic":
    ...
    return 1  # No loss scaling is needed for fp32 or bf16
  else:
...
def define_performance(num_parallel_calls=False, inter_op=False, intra_op=False,
                       synthetic_data=False, max_train_steps=False, max_eval_steps=False, dtype=False,
                       data_loader_image_type = False,
                       ...
                       force_v2_in_keras_compile=False, training_dataset_cache=False):
  ...
  Args:
    ...
    max_train_steps: Create a flags to allow specification of maximum number
      of training steps
    max_eval_steps: Create a flags to allow specification of maximum number
      of evaluation steps
  ...
  if max_eval_steps:
    flags.DEFINE_integer(
        name="max_eval_steps", short_name="mes", default=None, help=help_wrap(
            "The model will stop evaluation if the global_step reaches this "
            "value."
        ))

  if data_loader_image_type:
    flags.DEFINE_enum(
        name="data_loader_image_type", short_name="dlit", default="fp32",
        enum_values=DTYPE_MAP.keys(),
        help=help_wrap("data loader images output type"))
  ...
  if dtype:
    ...
    DEFAULT_BF16_CONFIG_PATH = os.fspath(Path(os.path.realpath(__file__)).parents[4].joinpath("common/bf16_config/full.json"))
