...
###############################################################################
# List of changes:
# - added support for HPU strategy
...
def get_distribution_strategy(distribution_strategy="mirrored",
  ...
  Args:
    ...
    num_hpus: Number of HPUs to run this model.
  ...
  if num_hpus < 0:
    raise ValueError("`num_hpus` can not be negative.")
  ...
  if distribution_strategy == "off":
    ...
    if num_hpus > 1:
      raise ValueError("When {} HPUs are specified, distribution_strategy "
                       "flag cannot be set to `off`.".format(num_hpus))
    return None
  ...
  if distribution_strategy == "mirrored":
    if num_gpus == 0 and num_hpus ==0:
      ...
      devices = ["device:HPU:%d" % i for i in range(num_hpus)]
    else:
