...
def run_experiment(
  ...
  hparams = {"batch_size" : params.task.train_data.global_batch_size,
  ...
  with distribution_strategy.scope():
    ...
    with dump_callback(params.runtime.dump_config) if params.runtime.dump_config else contextlib.ExitStack():
      if mode == 'train':
