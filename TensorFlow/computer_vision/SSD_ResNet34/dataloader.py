...
def transpose_labels(classes, boxes):
  sizes = [constants.FEATURE_SIZES[i] ** 2 * constants.NUM_DEFAULTS[i] for i in range(len(constants.FEATURE_SIZES))]
  ...
  for i in range(len(constants.FEATURE_SIZES)):
    cls_i = tf.reshape(classes_split[i], [constants.NUM_DEFAULTS[i], constants.FEATURE_SIZES[i], constants.FEATURE_SIZES[i], 1])
    cls_i = tf.transpose(cls_i, (1, 2, 0, 3))
    cls_i = tf.reshape(cls_i, [-1, 1])
    classes_t.append(cls_i)
    box_i = tf.reshape(boxes_split[i], [constants.NUM_DEFAULTS[i], constants.FEATURE_SIZES[i], constants.FEATURE_SIZES[i], 4])
