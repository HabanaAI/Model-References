...
def _localization_loss(pred_locs, gt_locs, gt_labels, num_matched_boxes, optimized = True):
    ...
    Args:
      ...
      optimized: use optimized implementation (reduce_sum on MME)
    Returns:
    ...
    for i, k in enumerate(keys):
        ...
        if optimized:
            reduce_sum_filter = tf.fill([1, 1, 4, 1], 1.0, name="reduce_sum_filter")
            huber = tf.reshape(huber, [gt_loc.shape[0], gt_loc.shape[1], 1, 4])
            smooth_l1 = tf.nn.conv2d(
                huber, reduce_sum_filter, strides=1, padding="VALID", name="smooth_l1")
            smooth_l1 = tf.reshape(smooth_l1, [gt_loc.shape[0], gt_loc.shape[1]])
        else:
            smooth_l1 = tf.reduce_sum(huber, axis=-1)
...
def _model_fn(features, labels, mode, params, model):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        update_weights = optimizer.apply_gradients(named_grads_and_vars, global_step=global_step)

        # Batch norm requires update_ops to be added as a train_op dependency.
        update_ops = tf.get_collection(tf.GraphKeys.UPDATE_OPS)
        train_op = tf.group(update_weights,
                            update_ops)
