...
def concat_outputs(cls_outputs, box_outputs, transpose):
  ...
  with tf.name_scope("concat_cls_outputs") as scope:
    for i, k in enumerate(keys):
      ...
      if transpose:
        cls_reshape = tf.reshape(cls_outputs_k, intermediate_shape, name="reshape_k"+str(k))
        cls_transpose = tf.transpose(cls_reshape, (0, 3, 1, 2, 4), name="transpose_k"+str(k))
        cls_reshape2 = tf.reshape(cls_transpose, final_shape, name="reshape2_k"+str(k))
      else:
        cls_reshape2 = tf.reshape(cls_outputs_k, final_shape, name="reshape_k"+str(k))
  ...
  with tf.name_scope("concat_box_outputs") as scope:
    for i, k in enumerate(keys):
      ...
      if transpose:
        box_reshape = tf.reshape(box_outputs_k, intermediate_shape, name="reshape_k"+str(k))
        box_transpose = tf.transpose(box_reshape, (0, 3, 1, 2, 4), name="transpose_k"+str(k))
        box_reshape2 = tf.reshape(box_transpose, final_shape, name="reshape2_k"+str(k))
      else:
        box_reshape2 = tf.reshape(box_outputs_k, final_shape, name="reshape_k"+str(k))
...
def _softmax_cross_entropy_mme(logits, label):
  ...
  with tf.name_scope("softmax_cross_entropy"):
    ...
    reduce_sum_filter = tf.fill([1, 1, num_classes, 1], 1.0, name="reduce_sum_filter")
    ...
    max_logits = tf.reshape(reduce_max, [1, batch_size, num_boxes, 1], name="max_logits")
...
def _classification_loss(pred_labels, gt_labels, num_matched_boxes):
  ...
  with tf.name_scope("class_loss"):
    ...
    neg_sorted_cross_indices = tf.cast(neg_sorted_cross_indices, tf.float32, name="cast_neg_sorted_cross_indices")
  ...
  return tf.reduce_mean(normalized_class_loss, name="class_loss")
...
def detection_loss(cls_outputs, box_outputs, labels):
  ...
  if isinstance(labels[constants.BOXES], dict):
    ...
    cls_outputs, box_outputs = concat_outputs(cls_outputs, box_outputs, False)
...
def learning_rate_schedule(params, global_step):
  ...
  with tf.colocate_with(global_step):
    ...
    learning_rate = tf.where(global_step < second_lr_drop_step, learning_rate,
                            adjusted_learning_rate * 0.01, name="learning_rate")
...
def _model_fn(features, labels, mode, params, model):
  ...
  if mode == tf.estimator.ModeKeys.PREDICT:
    flattened_cls, flattened_box = concat_outputs(cls_outputs, box_outputs, True)
