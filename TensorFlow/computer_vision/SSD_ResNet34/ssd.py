...
DEFAULT_VAL_JSON_PATH = DEFAULT_DATASET_PATH+"/raw-data/annotations/instances_val2017.json"
...
class SSDTrainingHook(tf.estimator.SessionRunHook):
    ...
    def after_run(self, run_context, run_values):
        ...
        if self.cur_step_in_epoch == self.steps_per_epoch or self.cur_step == self.total_steps:
            ...
            self.cur_epoch += 1
            self.cur_step_in_epoch = 0
            print("Epoch {} finished. Duration: {} IPS: {:.2f}".format(self.cur_epoch, self._format_duration(epoch_duration), ips))

        if self.cur_step == self.total_steps:
            train_time = cur_time - self.training_start_time
            print("Training finished. Total time: {}".format(self._format_duration(train_time)))
...
if __name__ == '__main__':
    ...
    if ARGS.mode == 'train':
        ...
        def input_fn():
            ...
            image = tf.reshape(image, [1, constants.IMAGE_SIZE, constants.IMAGE_SIZE, 3])
        ...
        for loc_, label_, prob_ in zip(loc, label, prob):
            ...
            color_neg = (255-((cl >> 4 ) & 3) * 85, 255-((cl >> 2) & 3) * 85, 255- (cl & 3) * 85)
            ...
            out_img = cv2.putText(out_img, text, text_pos, cv2.FONT_HERSHEY_SIMPLEX, 0.5, color_neg, 1, cv2.LINE_AA)

            print("{:1.2f}  ({:3}) {:16}  {} {}".format(prob_, cl, text, top_left, bottom_right))
