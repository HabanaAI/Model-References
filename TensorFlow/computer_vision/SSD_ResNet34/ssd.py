...
class TensorBoardHook(SessionRunHook):
  ...
  def before_run(self, run_context):
    if self.step == 20:
        tf2.profiler.experimental.start(self.output_dir)
    elif self.step == 25:
...
if __name__ == '__main__':
    ...
    if len(affinity) > ARGS.max_cpus:
        ...
        for cpu in affinity:
            ...
            if len(new_affinity) == ARGS.max_cpus:
                break
        print("Restricting CPUs to {} because max_cpus={}".format(new_affinity, ARGS.max_cpus))
    ...
    if not ARGS.no_hpu:
        ...
        if 'TF_DONT_CLUSTER' not in os.environ:
            os.environ['TF_DONT_CLUSTER'] = 'add_1' # In order to split clusters (decreases overhead)
    ...
    if ARGS.mode == 'train':
        ...
        if ARGS.profiling:
            train_hooks.append(TensorBoardHook(output_dir=ARGS.model_dir))
