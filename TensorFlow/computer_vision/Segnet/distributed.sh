...
if [ "$TRAINING_ENGINE" == "hpu" ]; then
    mpirun -np ${WORKERS} --allow-run-as-root $PYTHON -m keras_segmentation train --train_images="${DATAPATH}/images_prepped_train/" \
    ...
    horovodrun -np ${WORKERS} $PYTHON -m keras_segmentation train --train_images="${DATAPATH}/images_prepped_train/" \
        --train_annotations="${DATAPATH}/annotations_prepped_train/" --val_images="${DATAPATH}/images_prepped_test/" \
        --val_annotations="${DATAPATH}/annotations_prepped_test/" --n_classes=12 --input_height=320 \
