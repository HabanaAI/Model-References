...
# https://github.com/tensorflow/addons/blob/v0.12.0/tensorflow_addons/metrics/f_scores.py#L211-L283
class FBetaScore(tf.keras.metrics.Metric):
    ...
    >>> metric = FBetaScore(num_classes=3, beta=2.0, threshold=0.5)
    ...
    def update_state(self, y_true, y_pred, sample_weight=None):
        ...
        y_pred = tf.cast(y_pred, self.dtype) # B H W C
    ...
    def reset_states(self):
        # When disabling eager execution, must cast to np.zeros rather than tf.zeros
        reset_value = np.zeros(self.init_shape, dtype=self.dtype)
