...
class BaseDataset(ABC):
    ...
    def synth_dataset_fn(
        ...
        masks = tf.random.truncated_normal(mask_shape, dtype=tf.float32, mean=0.01, stddev=0.1, seed=seed, name='synth_masks')
        labels = tf.compat.v1.random_uniform([batch_size], minval=0, maxval=1, dtype=tf.int32, name='synthetic_labels')
        ...
        if use_hpu_prefetch:
            dataset.apply(tf.data.experimental.prefetch_to_device(device="/device:HPU:0", buffer_size=batch_size * 8))
