...
class DAGM2007_Dataset(BaseDataset):
    ...
    def dataset_fn(
        ...
        super(DAGM2007_Dataset, self).dataset_fn(
            ...
            use_hpu_prefetch=use_hpu_prefetch,
        ...
        if only_defective_images:
            dataset = dataset.filter(lambda line: tf.not_equal(tf.strings.substr(input=line, pos=-1, len=1), "0"))

        if horovod_enabled() and training:
            dataset = dataset.shard(hvd.size(), hvd.rank())

        def _load_dagm_data(line):

            input_image_name, image_mask_name, label = tf.io.decode_csv(
                records=line, record_defaults=[[""], [""], [0]], field_delim=','
            )
        ...
        def _augment_data(input_image, mask_image, label):
            ...
            if augment_data:
                ...
                horizontal_flip = tf.random.uniform(shape=(), seed=seed) > 0.5
                input_image = tf.cond(
                    pred=horizontal_flip, true_fn=lambda: tf.image.flip_left_right(input_image), false_fn=lambda: input_image
                )
                mask_image = tf.cond(pred=horizontal_flip, true_fn=lambda: tf.image.flip_left_right(mask_image), false_fn=lambda: mask_image)
                n_rots = tf.random.uniform(shape=(), dtype=tf.int32, minval=0, maxval=3, seed=seed)
        ...
        if use_hpu_prefetch:
            dataset.apply(tf.data.experimental.prefetch_to_device(device="/device:HPU:0", buffer_size=4))
...
if __name__ == "__main__":
    ...
    tf.compat.v1.logging.info("[*] Executing Benchmark in %s mode" % ("training" if FLAGS.training else "inference"))
    tf.compat.v1.logging.info("[*] Benchmark using %s data" % ("synthetic" if FLAGS.use_synthetic_data else "real"))

    print()
    tf.compat.v1.logging.info("[*] num_samples: %d" % _num_samples)
    tf.compat.v1.logging.info("[*] num_steps: %d" % _num_steps)
    tf.compat.v1.logging.info("[*] num_epochs: %d" % _num_epochs)
    ...
    input_images = tf.image.resize_with_crop_or_pad(input_images, target_height=512, target_width=512)
