...
def crelu(features, name='crelu', axis=-1):
    ...
    net = tf.nn.crelu(features=features, name=name, axis=axis)
...
def prelu(inputs, channel_shared=False, trainable=True, name='prelu'):
    ...
    def parametric_relu(_x):
        ...
        alphas = tf.compat.v1.get_variable(
            'alpha', w_shape, trainable=trainable, initializer=tf.compat.v1.initializers.truncated_normal(mean=-1.0, stddev=0.2)
        )
