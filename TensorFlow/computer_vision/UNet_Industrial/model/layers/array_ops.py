...
def flatten(inputs, name='flatten'):
    ...
    net = tf.compat.v1.layers.flatten(inputs, name=name)
...
def upscale_2d(inputs, size, is_scale=True, method=0, align_corners=True, data_format='NHWC', name='upsample2d_layer'):
    ...
    if len(inputs.get_shape()) == 3:
        ...
        if data_format == 'NCHW':
            inputs = tf.transpose(a=inputs, perm=[0, 2, 3, 1])  # NCHW => NHWC
    ...
    with tf.compat.v1.variable_scope(name):
        net = tf.compat.v1.image.resize_images(inputs, size=_size, method=method, align_corners=align_corners)

    if data_format == 'NCHW' and len(inputs.get_shape()) == 4:
        net = tf.transpose(a=net, perm=[0, 3, 1, 2])  # NHWC => NCHW
