...
class UNet_v1(object):
    ...
    def __init__(
        ...
        self.conv2d_hparams = hp.HParams(
            kernel_initializer=None, bias_initializer=tf.compat.v1.initializers.constant(0.0), activation_fn=activation_fn
        )

        if weight_init_method == "he_normal":
            self.conv2d_hparams.kernel_initializer = tf.compat.v1.initializers.variance_scaling(
            ...
            self.conv2d_hparams.kernel_initializer = tf.compat.v1.initializers.orthogonal(gain=1.0)
    ...
    def __call__(self, features, labels, mode, params):
        ...
        with tf.device("/device:HPU:0"):
            ...
            for threshold in [None, 0.05, 0.125, 0.25, 0.5, 0.75, 0.85, 0.95, 0.99]:
                ...
                tf.compat.v1.summary.image('output_sample_ths_%s' % threshold, binarize_img, 10)

            # ==============+ Evaluation Metrics ==================== #

            with tf.compat.v1.name_scope("IoU_Metrics"):
                ...
                for threshold in [0.05, 0.125, 0.25, 0.5, 0.75, 0.85, 0.95, 0.99]:
                    ...
                    tf.compat.v1.summary.scalar('iou_score_ths_%s' % threshold, iou_score)

                    if mode == tf.estimator.ModeKeys.EVAL:
                        eval_metrics["IoU_THS_%s" % threshold] = tf.compat.v1.metrics.mean(iou_score)

            ## for kibana report
            if mode == tf.estimator.ModeKeys.EVAL:
                eval_acc = metrics.iou_score(y_pred=y_pred, y_true=mask_image, threshold=0.99)
                tf.identity(eval_acc, name="eval_acc")
                tf.compat.v1.summary.scalar("eval_acc", eval_acc)
                eval_metrics["eval_acc"] = tf.compat.v1.metrics.mean(eval_acc)

            labels = tf.cast(labels, tf.float32)
            labels_preds = tf.reduce_max(input_tensor=y_pred, axis=(1, 2, 3))
            ...
            with tf.compat.v1.variable_scope("Confusion_Matrix") as scope:
                ...
                if mode == tf.estimator.ModeKeys.TRAIN:
                    local_vars = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.LOCAL_VARIABLES, scope=scope.name)
                    confusion_matrix_reset_op = tf.compat.v1.initializers.variables(local_vars, name='reset_op')
                ...
                tf.compat.v1.summary.scalar('true_positives', tp[3])  # For Ths = 0.5
                tf.compat.v1.summary.scalar('true_negatives', tn[3])  # For Ths = 0.5
                tf.compat.v1.summary.scalar('false_positives', fp[3])  # For Ths = 0.5
                tf.compat.v1.summary.scalar('false_negatives', fn[3])  # For Ths = 0.5
            ...
            tf.compat.v1.summary.image('sample_mask', binarized_mask, 10)

            ##########################

            mask_max_val = tf.reduce_max(input_tensor=mask_image)
            tf.identity(mask_max_val, name='mask_max_val_ref')

            mask_min_val = tf.reduce_min(input_tensor=mask_image)
            tf.identity(mask_min_val, name='mask_min_val_ref')

            mask_mean_val = tf.reduce_mean(input_tensor=mask_image)
            ...
            output_max_val = tf.reduce_max(input_tensor=y_pred)
            tf.identity(output_max_val, name='output_max_val_ref')

            output_min_val = tf.reduce_min(input_tensor=y_pred)
            tf.identity(output_min_val, name='output_min_val_ref')

            output_mean_val = tf.reduce_mean(input_tensor=y_pred)
            ...
            if mode == tf.estimator.ModeKeys.TRAIN:
                ...
                with tf.compat.v1.variable_scope("optimizers"):
                    ...
                    tf.compat.v1.summary.scalar('learning_rate_ref', learning_rate)
                    ...
                    if horovod_enabled():
                        opt = hvd.DistributedOptimizer(opt, device_dense='/device:HPU:0')
                    ...
                    train_op = tf.group(backprop_op, tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.UPDATE_OPS))
    ...
    def build_model(self, inputs, training=True, reuse=False, debug_verbosity=0):
        ...
        with tf.compat.v1.variable_scope(self.model_hparams.model_name, reuse=reuse):

            with tf.compat.v1.variable_scope("input_reshape"):

                with tf.compat.v1.variable_scope("initial_zero_padding"):
                    inputs = tf.image.resize_with_crop_or_pad(inputs, target_height=512, target_width=512)
                ...
                if self.model_hparams.input_format == 'NHWC' and self.model_hparams.compute_format == 'NCHW':
                    ...
                    net = tf.transpose(a=inputs, perm=[0, 3, 1, 2])
