...
class Runner(object):
    ...
    def __init__(
        ...
        run_config_additional = hp.HParams(
            dataset_hparams=dataset_hparams,
            model_dir=model_dir if not horovod_enabled() else "{}/worker_{}".format(model_dir, hvd.rank()),
            log_dir=log_dir if not horovod_enabled() or hvd.rank() == 0 else None,
            sample_dir=sample_dir if not horovod_enabled() or hvd.rank() == 0 else None,
            data_dir=data_dir,
        ...
        if self.run_hparams.seed is not None:
            ...
            tf.compat.v1.set_random_seed(self.run_hparams.seed)
        ...
        self.num_hpus = 1 if not horovod_enabled() else hvd.size()
    ...
    def _build_hparams(*args):
        ...
        for _hparams in args:
            if not isinstance(_hparams, hp.HParams):
    ...
    def train(
        ...
        global_batch_size = batch_size * self.num_hpus
        ...
        if not horovod_enabled() or hvd.rank() == 0:
            ...
            Logger.log(step=('PARAMETER'), data={"# HPUs": self.num_hpus}, verbosity=Logger.Verbosity.DEFAULT)
            Logger.log(step=('PARAMETER'), data={"HPU Batch Size": batch_size}, verbosity=Logger.Verbosity.DEFAULT)
        ...
        training_hooks.append(ExamplesPerSecondEstimatorHook(
            batch_size=batch_size, every_n_steps=10,
            output_dir=self.run_hparams.model_dir, extra_metrics={'global_examples/sec': batch_size*self.num_hpus},
