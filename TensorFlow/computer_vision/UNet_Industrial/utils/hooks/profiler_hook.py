...
class ProfilerHook(tf.estimator.SessionRunHook):
    ...
    def after_create_session(self, session, coord):
        ...
        params_count = tf.compat.v1.get_default_graph().get_tensor_by_name("trainable_parameters_count_ref:0")
    ...
    def before_run(self, run_context):
        ...
        if self._current_step % self._log_every == 0:
            ...
            additional_fetches = {
                'total_loss': tf.compat.v1.get_default_graph().get_tensor_by_name("losses/total_loss_ref:0"),
                'iou_scores': dict(),
            ...
            if self._is_training:
                ...
                additional_fetches["weight_decay"] = tf.compat.v1.get_default_graph().get_tensor_by_name("losses/l2_loss_ref:0")

                additional_fetches["reconstruction_loss"] = tf.compat.v1.get_default_graph(
                ).get_tensor_by_name("losses/reconstruction_loss_ref:0")

                additional_fetches["learning_rate"] = tf.compat.v1.get_default_graph(
            ...
            if self._sample_dir is not None and self._is_training:
                ...
                additional_fetches["samples"]["input_image"] = tf.compat.v1.get_default_graph(
                ).get_tensor_by_name("input_image_jpeg_ref:0")

                additional_fetches["samples"]["mask"] = tf.compat.v1.get_default_graph().get_tensor_by_name("mask_sample_ref:0")

                for threshold in [None, 0.05, 0.125, 0.25, 0.5, 0.75, 0.85, 0.95, 0.99]:

                    additional_fetches["samples"][str(threshold)] = tf.compat.v1.get_default_graph().get_tensor_by_name(
            ...
            for threshold in [0.05, 0.125, 0.25, 0.5, 0.75, 0.85, 0.95, 0.99]:
                ...
                if threshold is not None:
                    additional_fetches["iou_scores"][str(threshold)] = tf.compat.v1.get_default_graph().get_tensor_by_name(
                        "IoU_Metrics/iou_score_ths_%s_ref:0" % threshold
                    )

            additional_fetches["confusion_matrix"]["tp"] = tf.compat.v1.get_default_graph(
            ).get_tensor_by_name("Confusion_Matrix/true_positives_ref:0")

            additional_fetches["confusion_matrix"]["tn"] = tf.compat.v1.get_default_graph(
            ).get_tensor_by_name("Confusion_Matrix/true_negatives_ref:0")

            additional_fetches["confusion_matrix"]["fp"] = tf.compat.v1.get_default_graph(
            ).get_tensor_by_name("Confusion_Matrix/false_positives_ref:0")

            additional_fetches["confusion_matrix"]["fn"] = tf.compat.v1.get_default_graph(
        ...
        return tf.estimator.SessionRunArgs(fetches=request_fetches)
