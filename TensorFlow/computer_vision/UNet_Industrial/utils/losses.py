...
def regularization_l2loss(weight_decay):
    ...
    filtered_params = [tf.cast(v, tf.float32) for v in tf.compat.v1.trainable_variables() if loss_filter_fn(v.name)]
...
def reconstruction_l2loss(y_pred, y_true):
    ...
    return tf.reduce_mean(input_tensor=tf.nn.l2_loss(reconstruction_err), name='reconstruction_loss_l2_loss')
