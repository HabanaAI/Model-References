...
def load_pretrained(size, weights, pretrained_top, model):
    ...
    if comm_size() > 1:
        ...
        if comm_rank() == 0:
            tf.keras.utils.get_file(fname, origin, cache_subdir="weights", cache_dir=config.WEIGHTS_DIR)
        MPI.COMM_WORLD.Barrier()
    local_filepath = tf.keras.utils.get_file(fname, origin, cache_subdir="weights", cache_dir=config.WEIGHTS_DIR)
