...
# - Weights cache dir is no longer hardcoded
# - Only rank 0 download weights
# - Last layer must be fp32 - required for tf.keras.mixed_precision
...
def build_model(
    ...
    if include_top:
            y = tf.keras.layers.Dense(classes, name="head", activation=activation, dtype='float32')(y)
...
def load_pretrained(size, weights, pretrained_top, model):
    ...
    if comm_size() > 1:
        ...
        if comm_rank() == 0:
            tf.keras.utils.get_file(
                fname, origin, cache_subdir="weights", cache_dir=config.WEIGHTS_DIR)
        MPI.COMM_WORLD.Barrier()
    local_filepath = tf.keras.utils.get_file(
        fname, origin, cache_subdir="weights", cache_dir=config.WEIGHTS_DIR)

    # 'tf.keras.utils.get_file' checks for write access even if such is not required.
    # This can lead to another path being used. Warn if such behavior happens.
    if config.WEIGHTS_DIR is not None:
        import os
        import sys
        if not os.access(config.WEIGHTS_DIR, os.W_OK) and comm_rank() == 0:
            warnings.warn(
                f'No write access to {config.WEIGHTS_DIR}, path to weights currently used: {local_filepath}',
                UserWarning,
            )
            sys.stderr.flush()
    ...
    # Wait until all ranks have their weights loaded.
    if comm_size() > 1:
        MPI.COMM_WORLD.Barrier()
