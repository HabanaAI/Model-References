...
def get_optimizer(model_name, optim_name, initial_lr, epsilon=1e-2):
    ...
    """
    if optim_name == 'sgd':
        return tf.keras.optimizers.legacy.SGD(learning_rate=initial_lr, momentum=0.9, nesterov=True)
    elif optim_name == 'adam':
        return tf.keras.optimizers.legacy.Adam(learning_rate=initial_lr, epsilon=epsilon)
    elif optim_name == 'rmsprop':
        return tf.keras.optimizers.legacy.RMSprop(learning_rate=initial_lr, epsilon=epsilon,
                                           rho=0.9)
    else:
