...
import utils
from horovod_estimator import HorovodEstimator, hvd_info_rank0, is_rank0
try:
...
dataset_default_dirs = ['/data/tensorflow/coco2017/tf_records']
...
backbone_ckpt_default_dirs = ['/data/tensorflow/efficientdet/backbones/efficientnet-b0']
...
flags.DEFINE_string('backbone_ckpt', BACKBONE_CKPT_DEFAULT_DIR,
                    'Location of the EfficientNet checkpoint to use for model '
                    'initialization.')
...
def set_env(use_amp, use_fast_math=False):
    ...
    os.environ['TF_GPU_THREAD_COUNT'] = '1' if hvd is None or not hvd.is_initialized() else str(hvd.size())
...
def get_session_config(use_xla):
    ...
    elif hvd is not None and hvd.is_initialized():
...
def build_estimator_params(mode, config, num_shards):
...
def build_estimator_config(mode, config, num_shards, num_cores_per_replica,
...
def write_summary(eval_results, summary_dir, current_step):
    ...
    with tf.Graph().as_default():
        ...
        for metric in eval_results:
            try:
                summaries.append(tf.Summary.Value(tag=metric, simple_value=eval_results[metric]))
        ...
        try:
            tf_summaries = tf.Summary(value=summaries)
...
def main(argv):
  ...
  if FLAGS.dump_config or FLAGS.deterministic:
    set_deterministic()
  ...
  run_config = build_estimator_config('train', config, num_shards,
                      num_cores_per_replica, input_partition_dims)
  ...
  if FLAGS.mode == 'train':
    ...
    for cycle in range(FLAGS.num_epochs):
      ...
      train_estimator.train(
          ...
          max_steps=(cycle + 1) * int(FLAGS.num_examples_per_epoch / FLAGS.train_batch_size))
      ...
      if is_rank0():
        ...
        last_ckpt = tf.train.latest_checkpoint(str(checkpoint_path), latest_filename=None)
