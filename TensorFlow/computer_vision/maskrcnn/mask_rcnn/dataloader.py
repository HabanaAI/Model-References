...
class InputReader(object):
    ...
    def __call__(self, params, input_context=None):
        ...
        dataset = dataset.interleave(
            ...
            cycle_length=32,
            block_length=64,
            num_parallel_calls=tf.data.experimental.AUTOTUNE if not self._seed else 1,
        )
        ...
        available_memory_gb = meminfo['MemTotal'] / (1024 * 1024)
        # Disable cache and prefetch if there's less than 45G.
        # This value should be safe for coco2017.
        allow_cache_and_prefetch = available_memory_gb >= 45
        if not allow_cache_and_prefetch:
            logging.warning(
                f"Dataset cache and prefetching disabled due to limited amount of RAM ({available_memory_gb:.1f}G). It may decrease performance.")

        if allow_cache_and_prefetch:
        ...
        if allow_cache_and_prefetch:
            ...
            if not tf.distribute.has_strategy():
                # With Horovod the local device is always 0.
                device = f'/device:{params["device"]}:0'
        ...
        # tf.data.experimental.MapVectorizationOptions.* was removed in TF 2.6.0
        ver = tf.__version__.split('.')
        if int(ver[0]) <= 2 and int(ver[1]) <= 5:
