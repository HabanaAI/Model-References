...
def get_training_hooks(mode, model_dir, checkpoint_path=None,
                       skip_checkpoint_variables=None, batch_size=None,
                       save_summary_steps=None, profile_steps=''):
    ...
    worker_dir = os.path.join(model_dir, f'worker_{MPI_rank()}') if MPI_is_distributed() else model_dir
    if save_summary_steps is not None:
      training_hooks.append(
        ExamplesPerSecondEstimatorHook(
          batch_size,
          every_n_steps=save_summary_steps,
    ...
    if profile_steps:
      training_hooks.append(TensorBoardHook(worker_dir, profile_steps))
...
class BaseExecuter(object):
  ...
  def _save_config(self):
    ...
    model_dir = self._runtime_config.model_dir
    if MPI_is_distributed():
      model_dir = os.path.join(model_dir, f'worker_{MPI_rank()}')
  ...
  def get_last_checkpoint_path(self):
    ...
    return tf.train.latest_checkpoint(str(checkpoint_path), latest_filename=None)
