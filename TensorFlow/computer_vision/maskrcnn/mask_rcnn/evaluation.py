...
def compute_coco_eval_metric(predictor,
    ...
    while num_batches < 0 or batch_idx < num_batches:
        ...
        try:
            ...
            if MPI_is_distributed():
                ...
                MPI.COMM_WORLD.Barrier()  # Need to get time for all predictors for given batch_idx
    ...
    if MPI_is_distributed():
        ...
        MPI.COMM_WORLD.Barrier()  # FIXME: first gather is calling MPI_FINALIZE causing crash
        if MPI_rank() == 0:
            predictions.clear()
