...
def build_model_graph(features, labels, is_training, params):
    ...
    with tf.name_scope('KEEP_FP32_PRECISION'):
    ...
    if params['use_custom_box_proposals_op']:
        ...
        with tf.name_scope('KEEP_FP32_PRECISION'):
            rpn_box_scores, rpn_box_rois = roi_ops.multilevel_propose_rois(
                ...
                topk_before_nms=params['topk_before_nms'],
                nms_on_hpu=params['nms_on_hpu'],
    ...
    box_roi_features = spatial_transform_ops.multilevel_crop_and_resize(
        ...
        boxes=rpn_box_rois,
        output_size=7,
        custom_roi=params['pyramid_roi_impl'],
    ...
    if not is_training:
        ...
        with tf.name_scope('KEEP_FP32_PRECISION'):
            detections = generate_detections_fn(
                ...
                nms_multiclass_workaround=params['nms_multiclass_workaround']
            )
    ...
    MODELS["Mask_Head"] = heads.Mask_Head_Model(
        ...
        is_gpu_inference=is_gpu_inference,
        offload_post_proc=params['offload_maskhead_postproc'],
        trainable=is_training,
        name="mask_head"
    )
    ...
    if is_training:
        if params['device'] == 'HPU' and params['use_habana_get_mask_targets']:
            get_mask_targets_fn = training_ops.get_mask_targets_habana
        else:
            get_mask_targets_fn = training_ops.get_mask_targets

        mask_targets = get_mask_targets_fn(
...
def _model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
        ...
        # Add identifiable prefix to gradient. Used by SBS.
        if params['dump_config'] is not None:
            # 'gradients' is immutable, so new array have to be made.
            new_gradients = []
            for t in gradients:
                # Filter tensors that can be omitted.
                if 'AddN' in t.name or 'truediv' in t.name:
                    identity_name = 'SBS_' + t.name.split(':')[0]
                    new_gradients.append(tf.identity(t, name=identity_name))
                else:
                    new_gradients.append(t)
    ...
    if not isinstance(replica_id, tf.Tensor) or tf.get_static_value(replica_id) == 0:
        ...
        tf.compat.v1.summary.scalar("train/l2_loss", l2_regularization_loss)
        tf.compat.v1.summary.scalar("train/mask_loss", mask_loss)
        tf.compat.v1.summary.scalar("train/total_loss", total_loss)
        tf.compat.v1.summary.scalar("train/rpn_box_loss", rpn_box_loss)
        tf.compat.v1.summary.scalar("train/rpn_score_loss", rpn_score_loss)
        tf.compat.v1.summary.scalar("train/rpn_total_loss", total_rpn_loss)
        tf.compat.v1.summary.scalar("train/fastRCNN_class_loss", fast_rcnn_class_loss)
        tf.compat.v1.summary.scalar("train/fastRCNN_box_loss", fast_rcnn_box_loss)
        tf.compat.v1.summary.scalar("train/fastRCNN_total_loss", total_fast_rcnn_loss)
        tf.compat.v1.summary.scalar("train/learning_rate", learning_rate)
        tf.compat.v1.summary.scalar("loss", total_loss)  # for compatibility
