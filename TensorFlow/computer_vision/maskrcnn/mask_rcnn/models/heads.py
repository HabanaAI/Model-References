...
class Mask_Head_Model(tf.keras.Model):
    ...
    def __init__(
        ...
        self._offload_post_proc = offload_post_proc
        ...
        if self._offload_post_proc:
            ...
            with tf.name_scope('masks_post_processing'):
                ...
                if batch_size == 1:
                    ...
                    gather_indices = tf.stack([mask_indices, self._class_indices], axis=2)
                    gather_indices = tf.reshape(gather_indices, [batch_size*num_rois, 2])

                    if self._is_gpu_inference:
                        gather_indices = tf.cast(gather_indices, dtype=tf.int32)

                    mask_outputs = tf.reshape(mask_outputs, [batch_size*num_rois, self._num_classes, self._mrcnn_resolution, self._mrcnn_resolution])
                    mask_outputs = tf.gather_nd(mask_outputs, gather_indices)
                    mask_outputs = tf.reshape(mask_outputs, [batch_size, num_rois, self._mrcnn_resolution, self._mrcnn_resolution])
