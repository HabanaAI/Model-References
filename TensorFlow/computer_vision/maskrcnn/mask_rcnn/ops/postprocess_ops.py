...
def nms_multiclass_wa(pre_nms_boxes, pre_nms_scores, mopc, P, nms_thr, score_thr):
    n_batch, n_boxes, n_class, _ = pre_nms_boxes.get_shape().as_list()

    boxes_t = tf.transpose(pre_nms_boxes, [0,2,1,3])
    boxes_t_r = tf.reshape(boxes_t, [n_batch*n_class, n_boxes, 1, 4])

    scores_t = tf.transpose(pre_nms_scores, [0,2,1])
    scores_t_r = tf.reshape(scores_t, [n_batch*n_class, n_boxes, 1])

    post_nms_boxes, post_nms_scores, post_nms_classes, \
    post_nms_num_valid_boxes = tf.image.combined_non_max_suppression(
        boxes_t_r, scores_t_r, max_output_size_per_class=mopc, max_total_size=P,
        iou_threshold=nms_thr, score_threshold=score_thr, pad_per_class=False)

    post_nms_scores_r = tf.reshape(post_nms_scores, [n_batch, n_class*P])
    top_scores, top_p_indices = tf.nn.top_k(post_nms_scores_r, P)

    box_indices = top_p_indices + tf.expand_dims(tf.range(n_batch)*n_class*P, 1)
    box_indices_r = tf.reshape(box_indices, [n_batch*P])
    post_nms_boxes_r = tf.reshape(post_nms_boxes, [n_batch*n_class*P, 4])
    top_boxes = tf.gather(post_nms_boxes_r, box_indices_r)
    top_boxes = tf.reshape(top_boxes, [n_batch, P, 4])

    top_classes = tf.math.floordiv(top_p_indices, P)
    valid_boxes = tf.reshape(post_nms_num_valid_boxes, [n_batch, n_class])
    valid_boxes = tf.math.reduce_sum(valid_boxes, axis=1)
    valid_boxes = tf.minimum(valid_boxes, P)

    return top_boxes, top_scores, top_classes, valid_boxes
...
def generate_detections_gpu(class_outputs,
                            ...
                            nms_threshold=0.3,
                            bbox_reg_weights=(10., 10., 5., 5.),
                            nms_multiclass_workaround=False
                            ):
    ...
    with tf.name_scope('generate_detections'):
        ...
        if not nms_multiclass_workaround:
            ...
            logging.info ("Using Multiclass NMS HPU workaround for validation")
            post_nms_boxes, post_nms_scores, post_nms_classes, post_nms_num_valid_boxes = \
                nms_multiclass_wa(pre_nms_boxes, pre_nms_scores, pre_nms_num_detections,
                post_nms_num_detections, nms_threshold, 0.0)
