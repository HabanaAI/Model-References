...
def _propose_rois(scores,
    ...
    if rpn_nms_threshold > 0:
        ...
        if not nms_on_hpu:
            logging.info("[ROI OPs] Using Batched NMS on CPU/GPU with %d input boxes" % num_boxes)
            ...
            logging.info("[ROI OPs] Using Batched NMS on HPU with %d input boxes" % num_boxes)
            boxes, scores, _, _ = tf.image.combined_non_max_suppression(
                pre_nms_boxes,
                pre_nms_scores,
                max_output_size_per_class=topk_limit,
...
def multilevel_propose_rois(scores_outputs,
    ...
    topk_before_nms: a boolean indicating whether to use Topk before NMS.
    nms_on_hpu: a boolean indicating whether NMS is executed on HPU device.
