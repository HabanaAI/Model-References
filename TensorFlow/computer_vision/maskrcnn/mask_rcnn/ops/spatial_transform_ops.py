...
###############################################################################
# Changes:
# - removed casts to float32 for AddN (Gaudi supports AddN also on int32)
...
def nearest_upsampling(data, scale):
  ...
  with tf.name_scope('nearest_upsampling'):
      ...
      output = tf.reshape(output, [bs, h, w * scale, c])
...
def multilevel_crop_and_resize(features,
  ...
  if 'habana' in custom_roi:
      # calculate roi_level
      level_keys = features.keys()
      min_level = min(level_keys)
      max_level = max(level_keys)
      feature_list = []
      for level in range(min_level,max_level+1):
         feature_list.append(features[level])
      _, min_level_height, min_level_width, _ = features[min_level].get_shape().as_list()
      ...
      with tf.name_scope('KEEP_FP32_PRECISION'):
          ...
          roi_level = tf.floor(tf.math.log(areas_sqrt / 224.0 + 1e-6) / tf.math.log(2.0))
          roi_level = tf.cast(roi_level, dtype=tf.int32) + 4
          roi_level = tf.minimum(max_level, tf.maximum(roi_level, min_level))
          roi_level = tf.subtract(roi_level, 2)
          roi_level = tf.expand_dims(roi_level, axis=2)
          # get num_boxes: create constant for static shapes
          batch_size, num_boxes, _ = boxes.get_shape().as_list()
          num_boxes = tf.constant([[num_boxes]] * batch_size, dtype=tf.int32)
          sampling_ratio=1
          if 'bf16' not in custom_roi:
              # invoke Habana custom op
              logging.debug("[ROI Align] Using custom op HPU (FP32) implementation")
              from TensorFlow.common.library_loader import habana_ops
              pooled_feature = habana_ops.pyramid_roi_align(feature_list, num_boxes, boxes, roi_level,
                            output_size=[output_size, output_size], sampling_ratio=sampling_ratio,
                            use_abs_coords=True, scale_to_level=True)
      if 'bf16' in custom_roi:
          # invoke Habana custom op
          logging.debug("[ROI Align] Using custom op HPU (BF16) implementation")
          from TensorFlow.common.library_loader import habana_ops
          pooled_feature = habana_ops.pyramid_roi_align(feature_list, num_boxes, boxes, roi_level,
                        output_size=[output_size, output_size], sampling_ratio=sampling_ratio,
                        use_abs_coords=True, scale_to_level=True)
  ...
  with tf.name_scope('KEEP_FP32_PRECISION'):
      logging.debug("[ROI Align] Using NV-DL implementation")
