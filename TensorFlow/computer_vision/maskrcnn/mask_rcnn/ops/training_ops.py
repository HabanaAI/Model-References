...
def get_mask_targets_habana(fg_boxes, fg_proposal_to_label_map, fg_box_targets, mask_gt_labels, output_size=28):
    """Crop and resize on multilevel feature pyramid. Optimized for HPU.
    ...
    _, num_levels, max_feature_height, max_feature_width = mask_gt_labels.get_shape().as_list()
    batch_size, num_boxes, _ = fg_boxes.get_shape().as_list()

    mask_gt_labels_list = tf.split(mask_gt_labels, num_or_size_splits=num_levels, axis=1)
    for i,mask_gt_labels in enumerate(mask_gt_labels_list):
        mask_gt_labels_list[i] = tf.expand_dims(tf.squeeze(mask_gt_labels, 1), -1)
    ...
    y1_transform = (bb_y_min - gt_y_min) * valid_feature_height / (gt_y_max - gt_y_min + _EPSILON) + 2
    x1_transform = (bb_x_min - gt_x_min) * valid_feature_width / (gt_x_max - gt_x_min + _EPSILON) + 2
    ...
    y2_transform = y1_transform + h_transform
    x2_transform = x1_transform + w_transform

    boxes = tf.concat([y1_transform, x1_transform, y2_transform, x2_transform], -1)
    num_boxes = tf.constant([[num_boxes]] * batch_size, dtype=tf.int32)
    levels = tf.expand_dims(levels, -1)

    num_boxes = tf.reshape(num_boxes, [batch_size,1])
    from habana_frameworks.tensorflow import habana_ops
    features_per_box = habana_ops.pyramid_roi_align(mask_gt_labels_list, num_boxes, boxes, levels,
        output_size = [output_size, output_size], sampling_ratio = 1, use_abs_coords=True,
