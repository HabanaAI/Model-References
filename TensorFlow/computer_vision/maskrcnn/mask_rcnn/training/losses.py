...
def _huber_loss(y_true, y_pred, weights, delta):
    ...
    # Comment below line to avoid generating int64 reduce_sum ops
    #num_non_zeros = tf.math.count_nonzero(weights, dtype=tf.float32)

    # Implement count_nonzero op with float32 reduce_sum op instead of
    # int64 reduce_sum used in tf.math.count_nonzero
    num_non_zeros = tf.reduce_sum(tf.cast(tf.math.not_equal(tf.cast(weights, tf.float32),
        tf.constant(0, dtype=tf.float32)), tf.float32))
...
def _softmax_cross_entropy(onehot_labels, logits):
    ...
    # Implement count_nonzero op with float32 reduce_sum op instead of
    # int64 reduce_sum used in tf.math.count_nonzero
    num_non_zeros = tf.reduce_sum(tf.cast(tf.math.not_equal(tf.cast(onehot_labels, tf.float32),
        tf.constant(0, dtype=tf.float32)), tf.float32))
