...
DOWNLOADED_WEIGHTS = '/software/data/tf/data/mask_rcnn/resnet50_weights_tf_dim_ordering_tf_kernels_notop.h5'
############################################################
#  Utility Functions
############################################################
    ...
    def call(self, inputs):
        with tf.device(self.dev_str):
            ...
            def nms(boxes, scores):
                with tf.device(self.dev_str if 'HPU' in self.dev_str else "/device:CPU:0"):
...
def pyramid_roi_align_with_gemm(feature_maps, boxes, roi_size, sampling_ratio,
        valid_boxes_count = None, levels = None, use_crop_and_resize_sampling_grid = False):
    with tf.name_scope('pyramid_roi_align_with_gemm_{}x{}'.format(roi_size[0],roi_size[1])):
        level_keys = feature_maps.keys()
        min_level = min(level_keys)
        max_level = max(level_keys)
        (batch_size, min_level_height, min_level_width, num_filters) = feature_maps[min_level].get_shape().as_list()
        _, num_boxes, _ = boxes.get_shape().as_list()
        input_image_height = (2**min_level) * min_level_height
        input_image_width = (2**min_level) * min_level_width

        # Assign boxes to the right level.
        box_height = boxes[:, :, 2] - boxes[:, :, 0]
        box_width = boxes[:, :, 3] - boxes[:, :, 1]
        areas_sqrt = tf.sqrt(box_height * box_width)
        image_area_sqrt = tf.sqrt(float(input_image_width * input_image_width))
        if levels == None:
            levels = tf.floor(tf.math.log(areas_sqrt / (224.0 / image_area_sqrt) + 1e-6) / tf.math.log(2.0))
            levels = tf.cast(levels, dtype=tf.int32) + 4
            levels = tf.minimum(max_level, tf.maximum(levels, min_level))

        features_per_box = tf.zeros(
            [batch_size, num_boxes, roi_size[0], roi_size[1], num_filters],
            dtype=feature_maps[min_level].dtype)

        for level in range(min_level, max_level + 1):
          with tf.name_scope('L{}_kernel'.format(level)):
            features_level = feature_maps[level]
            (_, feature_height, feature_width, _) = features_level.get_shape().as_list()

            # compute x & y coordinates on sample grid
            # box_grid_y/x shape is [batch_size, num_boxes, roi_size[0/1] * sampling_ratio, 1]
            box_coords = boxes * tf.constant([feature_height-1,feature_width-1,feature_height-1,feature_width-1],dtype=tf.float32)
            box_y = tf.slice(box_coords, [0,0,0], [batch_size,num_boxes,1])
            box_x = tf.slice(box_coords, [0,0,1], [batch_size,num_boxes,1])
            box_h = tf.slice(box_coords, [0,0,2], [batch_size,num_boxes,1]) - box_y
            box_w = tf.slice(box_coords, [0,0,3], [batch_size,num_boxes,1]) - box_x
            bin_h = tf.math.multiply(box_h, 1.0/roi_size[0])
            bin_w = tf.math.multiply(box_w, 1.0/roi_size[1])
            if use_crop_and_resize_sampling_grid:
                bin_h2 = tf.math.multiply(box_h, 1.0/(2*roi_size[0]-1))
                bin_w2 = tf.math.multiply(box_w, 1.0/(2*roi_size[1]-1))
                c_range_roi_size_0_2 = tf.constant(list(range(2*roi_size[0])),dtype=tf.float32)
                c_range_roi_size_1_2 = tf.constant(list(range(2*roi_size[1])),dtype=tf.float32)
                box_grid_y = tf.reshape(tf.math.add(box_y, tf.math.multiply(bin_h2, c_range_roi_size_0_2)),
                                [batch_size,num_boxes,roi_size[0]*sampling_ratio,1])
                box_grid_x = tf.reshape(tf.math.add(box_x, tf.math.multiply(bin_w2, c_range_roi_size_1_2)),
                                [batch_size,num_boxes,roi_size[1]*sampling_ratio,1])
            else:
                c_range_roi_size_0 = tf.constant(list(range(roi_size[0])),dtype=tf.float32)
                c_range_roi_size_1 = tf.constant(list(range(roi_size[1])),dtype=tf.float32)
                c_sampling_ratio_grid = tf.constant([(2.*v+1.)/(2.*sampling_ratio) for v in range(sampling_ratio)],dtype=tf.float32)
                box_grid_y = tf.reshape(
                        tf.add(
                            tf.reshape(
                                tf.math.add(box_y, tf.math.multiply(bin_h, c_range_roi_size_0)),
                                [batch_size,num_boxes,roi_size[0],1]),
                            tf.math.multiply(tf.reshape(bin_h, [batch_size,num_boxes,1,1]), c_sampling_ratio_grid)),
                        [batch_size,num_boxes,roi_size[0]*sampling_ratio,1])
                box_grid_x = tf.reshape(
                        tf.add(
                            tf.reshape(
                                tf.math.add(box_x, tf.math.multiply(bin_w, c_range_roi_size_1)),
                                [batch_size,num_boxes,roi_size[1],1]),
                            tf.math.multiply(tf.reshape(bin_w, [batch_size,num_boxes,1,1]), c_sampling_ratio_grid)),
                        [batch_size,num_boxes,roi_size[1]*sampling_ratio,1])

            # calculate weight filters for gemm
            box_grid_y0 = tf.maximum(0., tf.floor(box_grid_y))
            box_grid_x0 = tf.maximum(0., tf.floor(box_grid_x))
            box_grid_y0y1 = tf.reshape(tf.stack([
                    tf.minimum(box_grid_y0  , feature_height),
                    tf.minimum(box_grid_y0+1, feature_height)], axis=3),
                [batch_size, num_boxes, roi_size[0], sampling_ratio * 2])
            box_grid_x0x1 = tf.reshape(tf.stack([
                    tf.minimum(box_grid_x0  , feature_width),
                    tf.minimum(box_grid_x0+1, feature_width)], axis=3),
                [batch_size, num_boxes, roi_size[1], sampling_ratio * 2])
            box_grid_y0y1 = tf.cast(box_grid_y0y1, dtype=tf.int32)
            box_grid_x0x1 = tf.cast(box_grid_x0x1, dtype=tf.int32)
            box_ly = box_grid_y - box_grid_y0
            box_lx = box_grid_x - box_grid_x0
            box_hy = 1. - box_ly
            box_hx = 1. - box_lx
            box_ky = tf.reshape(tf.stack([box_hy, box_ly], axis=3),
                        [batch_size, num_boxes, roi_size[0], sampling_ratio * 2, 1]) * (1. / sampling_ratio)
            box_kx = tf.reshape(tf.stack([box_hx, box_lx], axis=3),
                        [batch_size, num_boxes, roi_size[1], sampling_ratio * 2, 1]) * (1. / sampling_ratio)
            box_grid_y_weight = tf.reduce_sum(tf.multiply(tf.one_hot(box_grid_y0y1, feature_height), box_ky), axis=-2)
            box_grid_x_weight = tf.reduce_sum(tf.multiply(tf.one_hot(box_grid_x0x1, feature_width), box_kx), axis=-2)

          with tf.name_scope('L{}_gemm'.format(level)):
            # shape is [batch_size, num_boxes, roi_size[0], width, feature]
            y_outputs = tf.matmul(
                    tf.reshape(box_grid_y_weight, [batch_size,num_boxes*roi_size[0],feature_height]),
                    tf.reshape(features_level, [batch_size,feature_height,feature_width*num_filters]))
            y_outputs = tf.reshape(y_outputs,[batch_size,num_boxes,roi_size[0],feature_width,num_filters])

            # shape is [batch_size, num_boxes, roi_size[0], roi_size[1], feature]
            y_outputs_transposed = tf.transpose(
                    tf.reshape(y_outputs, [batch_size*num_boxes,roi_size[0],feature_width,num_filters]),
                    perm=[0,2,1,3])
            x_outputs_transposed = tf.matmul(
                    tf.reshape(box_grid_x_weight, [batch_size*num_boxes,roi_size[1],feature_width]),
                    tf.reshape(y_outputs_transposed, [batch_size*num_boxes,feature_width,roi_size[0]*num_filters]))
            x_outputs = tf.reshape(
                    tf.transpose(
                      tf.reshape(x_outputs_transposed, [batch_size*num_boxes,roi_size[1],roi_size[0],num_filters]),
                      perm=[0,2,1,3]),
                    [batch_size,num_boxes,roi_size[0],roi_size[1],num_filters])

          with tf.name_scope('L{}_select'.format(level)):
            level_equal = tf.equal(levels, level)
            if valid_boxes_count != None:
                valid_boxes_mask = tf.less(tf.constant([list(range(num_boxes))]*batch_size, dtype=tf.int32), valid_boxes_count)
                level_equal = tf.math.logical_and(level_equal, valid_boxes_mask)
            mask = tf.tile(
                        tf.cast(
                            tf.reshape(level_equal, [batch_size, num_boxes, 1]),
                            dtype=tf.float32),
                        [1, 1, roi_size[0] * roi_size[1] * num_filters])
            outputs = tf.multiply(tf.reshape(mask, [-1]), tf.reshape(x_outputs, [-1]))
            outputs = tf.reshape(outputs, [batch_size, num_boxes, roi_size[0], roi_size[1], num_filters])
          with tf.name_scope('sum'):
            features_per_box += outputs
...
class PyramidROIAlign(KE.Layer):
    ...
    def __init__(self, pool_shape, dev_str, num_boxes=None, B=None, R=None, C=None, custom_op=0, **kwargs):
    ...
    def call(self, inputs):
        ...
        if self.custom_op == 1:
            assert 'HPU' in self.dev_str, 'custom_roi=1 not supported for CPU'
            roi_level = tf.subtract(roi_level, 2)
            roi_level = tf.expand_dims(roi_level, axis=2)
            from TensorFlow.common.library_loader import habana_ops
            pooled_feature = habana_ops.pyramid_roi_align(feature_maps, self.num_boxes, boxes, roi_level,
                            output_size = [self.pool_shape[0], self.pool_shape[1]], sampling_ratio = 2)
            return pooled_feature
        elif self.custom_op == 2:
            features = {}
            for level, feature in enumerate(feature_maps):
                features[2+level] = feature
            pooled_feature = pyramid_roi_align_with_gemm(features,
                                boxes, self.pool_shape, 2,
                                valid_boxes_count = self.num_boxes, levels = roi_level, use_crop_and_resize_sampling_grid = True)
            return pooled_feature
        elif self.custom_op == 3:
            features = {}
            for level, feature in enumerate(feature_maps):
                features[2+level] = feature
            pooled_feature = pyramid_roi_align_with_gemm(features,
                                boxes, self.pool_shape, 2,
                                valid_boxes_count = self.num_boxes, levels = None, use_crop_and_resize_sampling_grid = False)
            return pooled_feature
        else:
...
def fpn_classifier_graph(dev_str, rois, feature_maps, image_meta, num_boxes,
    ...
    with tf.device(dev_str):
        pooled_features = build_roi_pool_fn([pool_size, pool_size], dev_str, num_boxes=num_boxes, name="roi_align_classifier")([rois, image_meta] + feature_maps)
...
def build_fpn_mask_graph(dev_str, num_classes, image_meta ,rois=None, feature_maps=None, num_boxes=None,
                         pool_size=None,pooled_features=None, train_bn=True, use_bn=True,
    ...
    with tf.device(dev_str):
        if pooled_features is not None:
            ...
            x = build_roi_pool_fn([pool_size, pool_size], dev_str, num_boxes=num_boxes, name="roi_align_mask")([rois, image_meta] + feature_maps)
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks, dev_str):
    ...
    with tf.device(dev_str if 'HPU' in dev_str else "/device:CPU:0"):
...
class MaskRCNN():
    ...
    def build(self, dev_str, mode, config):
        ...
        with tf.device(dev_str):
            ...
            if 'kapp_' in config.BACKBONE:
                ...
                if config.BACKBONE=="kapp_ResNet50" and os.path.exists(DOWNLOADED_WEIGHTS):
                    weights = DOWNLOADED_WEIGHTS
        ...
        if mode == "training":
            ...
            if not rpn_only:
                ...
                if config.USE_VALID_BOXES and config.PYRAMID_ROI_CUSTOM_OP:
                    num_boxes = tf.cast(tf.fill([config.IMAGES_PER_GPU,1], tf.shape(rois)[0]), tf.int32)
                else:
                    num_boxes = tf.cast(tf.fill([config.IMAGES_PER_GPU,1], config.TRAIN_ROIS_PER_IMAGE), tf.int32)
                mrcnn_class_logits, mrcnn_class, mrcnn_bbox =\
                    fpn_classifier_graph(dev_str, rois, mrcnn_feature_maps, input_image_meta, num_boxes,
                ...
                mrcnn_mask = build_fpn_mask_graph(dev_str, config.NUM_CLASSES, input_image_meta,rois, mrcnn_feature_maps, num_boxes,
            ...
            if not rpn_only:
                ...
                mask_loss = KL.Lambda(lambda x: mrcnn_mask_loss_graph(*x, dev_str), name="mrcnn_mask_loss")(
            ...
            if config.USE_VALID_BOXES and config.PYRAMID_ROI_CUSTOM_OP:
                num_boxes = tf.cast(tf.fill([1,1], tf.shape(rpn_rois)[0]), tf.int32)
            else:
                num_boxes = tf.cast(tf.fill([1,1], config.POST_NMS_ROIS_INFERENCE), tf.int32)
            build_roi_pool_fn = partial_class(build_roi_pool_fn,B=self.config.IMAGES_PER_GPU,
                                R=self.config.POST_NMS_ROIS_INFERENCE,C=self.config.TOP_DOWN_PYRAMID_SIZE,
                                custom_op=self.config.PYRAMID_ROI_CUSTOM_OP)
            _, mrcnn_class, mrcnn_bbox =\
                fpn_classifier_graph(dev_str, rpn_rois, mrcnn_feature_maps, input_image_meta, num_boxes,
                                     config.POOL_SIZE, config.NUM_CLASSES,
                                     train_bn=config.TRAIN_BN,use_bn=config.BN_ON_DETECTOR,
                                     fc_layers_size=config.FPN_CLASSIF_FC_LAYERS_SIZE,train=False,build_roi_pool_fn=build_roi_pool_fn)
            ...
            if config.USE_VALID_BOXES and config.PYRAMID_ROI_CUSTOM_OP:
                num_boxes = tf.cast(tf.fill([1,1], tf.shape(detections)[0]), tf.int32)
            else:
                num_boxes = tf.cast(tf.fill([1,1], config.DETECTION_MAX_INSTANCES), tf.int32)

            build_roi_pool_fn = partial_class(build_roi_pool_fn,B=self.config.IMAGES_PER_GPU,
                                R=self.config.DETECTION_MAX_INSTANCES,C=self.config.TOP_DOWN_PYRAMID_SIZE,
                                custom_op=self.config.PYRAMID_ROI_CUSTOM_OP)

            # Create masks for detections
            detection_boxes = KL.Lambda(lambda x: x[..., :4])(detections)
            mrcnn_mask = build_fpn_mask_graph(dev_str, config.NUM_CLASSES, input_image_meta,
                            detection_boxes, mrcnn_feature_maps, num_boxes,
                            config.MASK_POOL_SIZE, train_bn=config.TRAIN_BN,use_bn=config.BN_ON_MASK,build_roi_pool_fn=build_roi_pool_fn)
    ...
    def train(self, dev_str, train_dataset, val_dataset, learning_rate, epochs, layers,
              ...
              loss_weights=None, dump_tf_timeline=False, disable_validation=False):
        """Train the model.
        ...
        with tf.device(dev_str):
            ...
            if not disable_validation and not dump_tf_timeline:
