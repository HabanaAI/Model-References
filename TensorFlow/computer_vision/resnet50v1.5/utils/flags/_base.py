...
def define_base(data_dir=True, model_dir=True, clean=False, train_epochs=False,
  ...
  if display_steps:
    flags.DEFINE_integer(
        name="display_steps", short_name="dis", default=1,
        help=help_wrap("How many steps should pass between displaying results "
                       "output"))
    key_flags.append("display_steps")

  # default is None to not save any checkpoints
  # Tip: when setting save_checkpoint_steps, set also model_dir,
  # otherwise model, events etc. will be saved in /tmp/
