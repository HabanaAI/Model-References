...
def get_worker_index():
    """ Parse the worker's index from process's name
    """
    process_name = re.search(r'[0-9]+', mp.current_process().name)
    return int(process_name.group())-1


def set_tf_config(worker_index):
    """ Makes a TensorFlow cluster information and sets it to TF_CONFIG environment variable.
    """
    tf_config = {
        "cluster": {
            "worker": [f"localhost:{BASE_TF_SERVER_PORT + index}" for index in range(args.num_workers)]
        },
...
def train_mnist(barier, use_hpu: bool, batch_size: int, use_bfloat: bool, num_epochs: int):
    """ Train the distributed model on MNIST Dataset.
    """
    # Get the current worker's index
    worker_index = get_worker_index()
    # Set TF_CONFIG.
    set_tf_config(worker_index)
...
if __name__ == "__main__":
    args = parse_args()
    barier = mp.Barrier(parties=args.num_workers)
    processes = [mp.Process(target=train_mnist, args=(
        barier, args.use_hpu, args.batch_size, args.use_bfloat, args.epochs)) for i in range(args.num_workers)]
    for p in processes:
        p.start()
