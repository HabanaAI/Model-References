...
# To support TensorFlow Dataset operations, 'dynpatch_prf_remote_call.so' has to be preloaded and environment variable HBN_TF_REGISTER_DATASETOPS=1 set.
...
PYTHONPATH="$PYTHONPATH:$SCRIPT_PATH" \
    mpirun -np $NUM_WORKERS --tag-output python3 -m mnist_keras "$@"
set +x
