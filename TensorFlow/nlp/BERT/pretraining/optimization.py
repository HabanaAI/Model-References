...
def create_optimizer(loss, init_lr, num_train_steps, num_warmup_steps, manual_fp16=False, use_fp16=False, num_accumulation_steps=1,
  ...
  if num_accumulation_steps > 1:
      ...
      local_step = tf.cond(pred=reset_step, true_fn=lambda:local_step.assign(tf.ones_like(local_step)), false_fn=lambda:local_step.assign_add(1))
      ...
      all_are_finite = tf.reduce_all(input_tensor=[tf.reduce_all(input_tensor=tf.math.is_finite(g)) for g in grads]) if manual_fp16 or use_fp16 else tf.constant(True, dtype=tf.bool)
      batch_finite = tf.cond(pred=reset_step,
        true_fn=lambda: batch_finite.assign(tf.math.logical_and(tf.constant(True, dtype=tf.bool), all_are_finite)),
      ...
      def update(accum_vars):
          if allreduce_post_accumulation and hvd is not None:
              assert False, "TODO: support hvd.allreduce"
              accum_vars = [hvd.allreduce(tf.convert_to_tensor(value=accum_var), compression=Compression.fp16 if use_fp16 or manual_fp16 else Compression.none) if isinstance(accum_var, tf.IndexedSlices)
      ...
      update_op = tf.cond(pred=update_step,
