...
import os
import time
from TensorFlow.nlp.albert import classifier_utils
...
def main(_):
  ...
  if FLAGS.do_train:
    is_horovod_enabled = hvd is not None and hvd.is_initialized()
    if FLAGS.deterministic_run and not is_horovod_enabled and FLAGS.input_file :
      per_worker_filenames= [os.path.join(FLAGS.input_file, "train.tf_record")]
    if not tf.gfile.Exists(per_worker_filenames[worker_id]):
      classifier_utils.file_based_convert_examples_to_features(
          train_examples[start_index:end_index], label_list, FLAGS.max_seq_length, tokenizer,
    ...
    with dump_callback():
      estimator.train(input_fn=train_input_fn, max_steps=FLAGS.train_step, hooks=train_hooks)
  ...
  if FLAGS.do_eval:
    ...
    for ext in ["meta", "data-00000-of-00001", "index"]:
      ...
      tf.io.gfile.rename(
          os.path.join(model_dir, src_ckpt),
