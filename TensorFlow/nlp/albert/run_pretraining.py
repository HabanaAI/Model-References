...
flags.DEFINE_string(
    "eval_file", None,
    "Eval TF example files (can be a glob or comma separated).")
...
def main(_):
  ...
  if horovod_enabled():
    FLAGS.output_dir = FLAGS.output_dir if hvd.rank() == 0 else os.path.join(FLAGS.output_dir, "worker_" + str(hvd.rank()))
