...
def main(_):
  ...
  if FLAGS.do_train:
    ...
    if FLAGS.input_file:
      ...
      if tf.gfile.Exists(per_worker_filenames_temp[hvd.local_rank() if hvd is not None and hvd.is_initialized() else worker_id]):
        per_worker_filenames = per_worker_filenames_temp

    if not tf.gfile.Exists(per_worker_filenames[hvd.local_rank() if hvd is not None and hvd.is_initialized() else worker_id]):
      train_writer = squad_utils.FeatureWriter(
          filename=per_worker_filenames[hvd.local_rank() if hvd is not None and hvd.is_initialized() else worker_id], is_training=True)
