...
class BertPretrainingOverfit(TrainingRunHWConfig):
    ...
    def build_command(self):
        try:
            ...
            init_checkpoint_path = Path(self.args.init_checkpoint_path).joinpath(f"{self.args.model_variant}").joinpath("model.ckpt-0.meta")
            init_checkpoint = str(Path(self.args.init_checkpoint_path).joinpath(f"{self.args.model_variant}"))
