    ...
    if args.action == 'download':
        ...
        if args.dataset == 'bookscorpus':
            ...
            if args.skip_wikiextractor == 0:
                ...
                wikiextractor_command = 'python3 ' + path_to_wikiextractor_in_container + ' ' + directory_structure['download'] + '/' + args.dataset + '/wikicorpus_en.xml ' + '-b 100M --processes ' + str(args.n_processes) + ' -o ' + directory_structure['extracted'] + '/' + args.dataset
            ...
            if args.skip_wikiextractor == 0:
                path_to_wikiextractor_in_container = working_dir + '/wikiextractor/WikiExtractor.py'
                wikiextractor_command = 'python3 ' + path_to_wikiextractor_in_container + ' ' + directory_structure['download'] + '/' + args.dataset + '/wikicorpus_zh.xml ' + '-b 100M --processes ' + str(args.n_processes) + ' -o ' + directory_structure['extracted'] + '/' + args.dataset
                print('WikiExtractor Command:', wikiextractor_command)
                wikiextractor_process = subprocess.run(wikiextractor_command, shell=True, check=True)
        ...
        def create_record_worker(filename_prefix, shard_id, output_format='tfrecord', split='training'):
            bert_preprocessing_command = 'python3 ' + working_dir + '/create_pretraining_data.py'
