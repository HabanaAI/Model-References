...
def main(args):
    ...
    tfrecord_folder_prefix = "/seq_len_" + str(args.max_seq_length)
    hdf5_folder_prefix = "/lower_case_" + str(args.do_lower_case) + "_seq_len_" + str(args.max_seq_length) \
    ...
    directory_structure = {
        ...
        'tfrecord' : working_dir + '/tfrecord' + tfrecord_folder_prefix,
        'hdf5': working_dir + '/hdf5'+ hdf5_folder_prefix,
    }
    ...
    if args.action == 'download':
        ...
        if args.dataset == 'bookscorpus':
            ...
            if args.skip_wikiextractor == 0:
                ...
                path_to_wikiextractor_in_container = base_dir + '/wikiextractor/WikiExtractor.py'
                wikiextractor_command = f'{sys.executable} ' + path_to_wikiextractor_in_container + ' ' + directory_structure['download'] + '/' + args.dataset + '/wikicorpus_en.xml ' + '-b 100M --processes ' + str(args.n_processes) + ' -o ' + directory_structure['extracted'] + '/' + args.dataset
            ...
            if args.skip_wikiextractor == 0:
                ...
                path_to_wikiextractor_in_container = base_dir + '/wikiextractor/WikiExtractor.py'
                wikiextractor_command = f'{sys.executable} ' + path_to_wikiextractor_in_container + ' ' + directory_structure['download'] + '/' + args.dataset + '/wikicorpus_zh.xml ' + '-b 100M --processes ' + str(args.n_processes) + ' -o ' + directory_structure['extracted'] + '/' + args.dataset
                print('WikiExtractor Command:', wikiextractor_command)
                wikiextractor_process = subprocess.run(wikiextractor_command, shell=True, check=True)
        ...
        def create_record_worker(filename_prefix, shard_id, output_format='tfrecord', split='training'):
            bert_preprocessing_command = f'{sys.executable} ' + base_dir + '/create_pretraining_data.py'
