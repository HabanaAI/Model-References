...
BERT_ARGPARSER_HELP = ''
DEFAULT_BF16_CONFIG_PATH = os.fspath(Path(os.path.realpath(__file__)).parents[1].joinpath("bert/bf16_config/bert.json"))
  ...
  def pretraining(self, args):
    ...
    if args.test_set == "bookswiki":
      ...
      bookswiki_runner = bert_pretraining_bookswiki_main.BertPretrainingBooksWiki(
          ...
          args, e, p1, w1, b1, gbs1, s1, p2, w2, b2, gbs2, s2, self.pretrained_model, args.enable_scoped_allocator,args.enable_packed_data_mode,args.avg_seq_per_pack,
          args.resume_p1,args.resume_p2,args.init_checkpoint_path)
  ...
  def finetuning(self, args):
    ...
    if args.test_set == "mrpc":
      ...
      squad_runner = bert_squad_main.BertFinetuningSQUAD(hw_config_use_horovod, hw_config_num_workers_per_hls, "HLS1", args.kubernetes_run,
                                                         args, p, e, b, s, self.pretrained_model, args.enable_scoped_allocator)
      squad_runner.run()
    else:
  ...
  def download_pretrained_model(self, horovod_run):
    try:
      ...
      if horovod_run and is_valid_multi_node_config():
        ...
        run_cmd_as_subprocess(f"{sys.executable} {str(download_pretrained_model_path)} {self.pretrained_url} {self.pretrained_model} True")
    except Exception as exc:
      raise RuntimeError(f"Error in {self.__class__.__name__} download_pretrained_model()") from exc
...
class BertArgparser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument(
        '-i', '--iters', metavar='<val1[,val2]>', help='Number of steps for each phase (phase 2 is optional). Default: 7038,782 for bookswiki and 200 for overfit.')
    ...
    self.add_argument('--iterations_per_loop', metavar='<for_finetuning>', required=False, type=int,
                      default=-1, help='Defaults to -1 (This value is configured dynamically). How many steps to make in each estimator call.')
    ...
    self.add_argument('--init_checkpoint_path', metavar='<overfit_init_checkpoint_path>', required=False,
                      type=str, default='', help='Init checkpoint path for use w/wo resume')
    ...
    self.add_argument('--enable_packed_data_mode', type=bool, default=False, help='Whether to enable training with packed data. If enabled, packed data should be provided to input_files_dir.')
    self.add_argument('--avg_seq_per_pack', default=1.0, metavar='<val1[,val2]>', help='Number of avg_seq_per_pack for each phase of pretraining. Should be: 1.2,2 for bookswiki.')
    self.add_argument('--resume_p1', type=bool, default=False, help="Whether to resume training from init_checkpoint for phase 1. If enabled, global vars would be set from init_checkpoint.")
    self.add_argument('--resume_p2', type=bool, default=False, help="Whether to resume training from init_checkpoint for phase 2. If enabled, global vars would be set from init_checkpoint.")
    self.add_argument('--profile', type=str, default="", help="Profile Steps range X-Y (e.g. --profile 7,10)")
...
def check_packed_mode_setting(args):
  # The bf16 data-type is run as a mixed-precision fp32+bf16 training using the 'bert' config
  if args.enable_packed_data_mode and (type(args.avg_seq_per_pack) != str or len(args.avg_seq_per_pack.split(','))==1):
    raise Exception("In  packed mode avg_seq_per_pack should be set to val1,val2. For bookswiki it should be: 1.2,2")
...
def main():
  try:
    ...
    check_packed_mode_setting(args)
