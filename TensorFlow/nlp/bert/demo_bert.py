  ...
  def download_pretrained_model(self, horovod_run):
    try:
      if horovod_run and is_valid_multi_node_config():
        ...
        run_per_ip(f"python3 {str(download_pretrained_model_path)} {self.pretrained_url} {self.pretrained_model} False", [
  ...
  def pretraining(self, args):
    ...
    if args.test_set == "bookswiki":
      ...
      bookswiki_runner = bert_pretraining_bookswiki_main.BertPretrainingBooksWiki(
          args, e, p1, w1, b1, s1, p2, w2, b2, s2, self.pretrained_model)
      ...
      overfit_runner = bert_pretraining_overfit_main.BertPretrainingOverfit(
          args, e, p1, w1, b1, s1, self.pretrained_model)
      overfit_runner.run()
  ...
  def finetuning(self, args):
    ...
    if args.test_set == "mrpc":
      ...
      if args.max_seq_length:
        ...
        s = 128
      print(
          f"running MRPC on BERT {args.model_variant} {self.pretrained_model} epochs: {e} BF16: {os.environ.get('TF_ENABLE_BF16_CONVERSION')} BS: {b} T: {s}")
      mrpc_runner = bert_mrpc_main.BertFinetuningMRPC(args, e, b, s, self.pretrained_model)
      mrpc_runner.run()
      ...
      if args.max_seq_length:
        ...
        s = 384
      print(
          f"running SQUAD on BERT {args.model_variant} {self.pretrained_model} epochs: {e} BF16: {os.environ.get('TF_ENABLE_BF16_CONVERSION')} BS: {b} T: {s}")
      squad_runner = bert_squad_main.BertFinetuningSQUAD(args, e, b, s, self.pretrained_model)
      squad_runner.run()
    else:
  ...
  def download_pretrained_model(self, horovod_run):
    try:
      if horovod_run and is_valid_multi_node_config():
        download_pretrained_model_path = Path(__file__).parent.joinpath(
            'download').joinpath('download_pretrained_model.py')
        run_per_ip(f"python3 {str(download_pretrained_model_path)} {self.pretrained_url} {self.pretrained_model} True", [
                   'MULTI_HLS_IPS', 'PYTHONPATH'], False)
      else:
...
class BertArgparser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument('--bert-config-dir', required=False, metavar='<bert_config_dir>', type=str,
                      help="Path to directory containing bert config files needed for choosen training type. If not specified the zip file will be downloaded.")
    ...
    self.add_argument('--hls_type', metavar='<hls_type>', default='HLS1', choices=['HLS1', 'HLS1-H'], help='HLS Type')
    self.add_argument('-t', '--test_set', metavar='<test_set>', default='bookswiki', choices=[
                      'bookswiki', 'overfit', 'mrpc', 'squad'], help='Benchmark dataset, possible finetuning values: mrpc, squad; possible pretraining values: bookswiki, overfit')
    ...
    self.add_argument(
        '-i', '--iters', metavar='<val1[,val2]>', help='Number of steps for each phase of pretraining. Default: 900000,400000 for bookswiki and 200 for overfit')
    self.add_argument('-w', '--warmup', metavar='<val1[,val2]>',
                      help='Number of warmup steps for each phase of pretraining. Default: 10000,4000 for bookswiki and 10 for overfit')
...
def check_data_type_and_tf_bf16_conversion(args):
  # The bf16 data-type is run as a mixed-precision fp32+bf16 training using the 'bert' recipe
  if args.data_type == 'bf16':
...
def main():
  try:
    ...
    elif args.use_horovod is not None:
