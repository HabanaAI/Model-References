...
class Bert(object):
  ...
  def pretraining(self, args):
    ...
    if args.test_set == "bookswiki":
      ...
      print(f"running Books & Wiki on BERT {args.model_variant} {self.pretrained_model} epochs: {e}; p1: {p1} steps, {w1} warmup, {b1} batch, {s1} max_seq_length; "
            f"p2: {p2} steps, {w2} warmup, {b2} batch, {s2} max_seq_length; BF16: {os.environ.get('TF_BF16_CONVERSION')}")
      bookswiki_runner = bert_pretraining_bookswiki_main.BertPretrainingBooksWiki(
          hw_config_use_horovod, hw_config_num_workers_per_hls, args.hls_type, args.kubernetes_run,
          args, e, p1, w1, b1, s1, p2, w2, b2, s2, self.pretrained_model, args.enable_scoped_allocator)
  ...
  def finetuning(self, args):
    ...
    if args.test_set == "mrpc":
      ...
      mrpc_runner = bert_mrpc_main.BertFinetuningMRPC(hw_config_use_horovod, hw_config_num_workers_per_hls, args.hls_type, args.kubernetes_run,
      ...
      squad_runner = bert_squad_main.BertFinetuningSQUAD(hw_config_use_horovod, hw_config_num_workers_per_hls, args.hls_type, args.kubernetes_run,
                                                         args, e, b, s, self.pretrained_model, args.enable_scoped_allocator)
      squad_runner.run()
    else:
...
class BertArgparser(argparse.ArgumentParser):
  ...
  def __init__(self):
    ...
    self.add_argument('--hls_type', metavar='<hls_type>', default='HLS1', choices=['HLS1', 'HLS1-H'], help='HLS Type, possible values: HLS1, HLS1-H')
