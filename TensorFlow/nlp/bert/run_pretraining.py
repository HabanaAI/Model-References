...
def init_flags():
  ...
  flags.DEFINE_bool("enable_packed_data_mode", False, "Whether to enable training with packed data. "
                                                      "If enabled, packed data should be provided to input_files_dir. [DEPRECATED] This flag will be removed in future release.")
  flags.DEFINE_float("avg_seq_per_pack", 1.0, "Average number of sequences per packed sample. [DEPRECATED] This flag will be removed in future release.")
...
def main(_):
  ...
  if FLAGS.enable_packed_data_mode:
    logger.warning(f"Flag enable_packed_data_mode is deprecated and will be removed in future release.")
    if FLAGS.avg_seq_per_pack != 1.0: #  user overrides the flag
      logger.warning(f"Flag avg_seq_per_pack is deprecated and will be removed in future release.")
      avg_seq_per_pack = FLAGS.avg_seq_per_pack
    elif os.path.exists(metadata_file_path): # user does not override the flag; try to read from the metadata file
      avg_seq_per_pack = read_avg_seq_per_pack(metadata_file_path)
    else:
      raise IOError("Packed dataset metadata file not accessible. Please rerun the packing algorithm and make sure file is properly generated.")
  else:
    if os.path.exists(metadata_file_path): # file exists, so this is a packed directory
      avg_seq_per_pack = read_avg_seq_per_pack(metadata_file_path)
      FLAGS.enable_packed_data_mode = True
    else: # file does not exist, so this is unpacked directory
      avg_seq_per_pack = 1.0
...
#    config.gpu_options.per_process_gpu_memory_fraction = 0.7
  ...
  if FLAGS.do_train:
    ...
    global_batch_size = global_batch_size * avg_seq_per_pack if FLAGS.enable_packed_data_mode else global_batch_size
    batch_size_per_node = batch_size_per_node * avg_seq_per_pack if FLAGS.enable_packed_data_mode else batch_size_per_node
