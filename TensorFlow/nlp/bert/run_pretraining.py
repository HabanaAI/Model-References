...
def init_flags():
  ...
  flags.DEFINE_bool("enable_packed_data_mode", True, "Whether to enable training with packed data. "
                                                      "If enabled, packed data should be provided to input_files_dir.")
  flags.DEFINE_float("avg_seq_per_pack", 2.0, "Average number of sequences per packed sample.")
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if FLAGS.verbose_logging:
        ...
        for var in tvars:
          ...
          tf.compat.v1.logging.info("  %d :: name = %s, shape = %s%s", hvd.rank() if hvd is not None and hvd.is_initialized() else 0, var.name, var.shape,
                          init_string)
...
def main(_):
  ...
  if hvd is not None and hvd.is_initialized() and comm_local_rank() != 0:
...
#    config.gpu_options.per_process_gpu_memory_fraction = 0.7
  ...
  if FLAGS.do_train:
    ...
    global_batch_size = global_batch_size * FLAGS.avg_seq_per_pack if FLAGS.enable_packed_data_mode else global_batch_size
    batch_size_per_node = batch_size_per_node * FLAGS.avg_seq_per_pack if FLAGS.enable_packed_data_mode else batch_size_per_node
