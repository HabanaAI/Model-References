...
class BertModel(object):
  ...
  def __init__(self,
    ...
    with tf.compat.v1.variable_scope(scope, default_name="distilbert", custom_getter=get_custom_getter(compute_type)):
...
def get_assignment_map_from_checkpoint(tvars, init_checkpoint):
  ...
  pre_suffix = 'model/'
  pos_suffix = '/.ATTRIBUTES/VARIABLE_VALUE'
  for x in init_vars:
    (name, var) = (x[0], x[1])
    temp_name = name
    if temp_name.endswith(pos_suffix):
      temp_name = temp_name[:-len(pos_suffix)]
    if temp_name.startswith(pre_suffix):
      temp_name = temp_name[len(pre_suffix):]
