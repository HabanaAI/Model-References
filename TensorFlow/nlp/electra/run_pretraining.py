...
def train_one_step(config, model, optimizer, features, accumulator, first_step, take_step, clip_norm=1.0):
    ...
    if first_step or take_step:
        ...
        allreduced_accumulated_gradients = accumulator.gradients
...
def main(e2e_start_time):
    ...
    parser.add_argument('--hmp', action='store_true', help="Habana mixed precision training")

    parser.add_argument("--vocab_file", type=str, default="./vocab/vocab.txt", help="Path to vocab file.")
    parser.add_argument("--dump_config", type=str, default="", help="Dump config file path (when using test_sbs).")
    parser.add_argument("--test_sbs", action='store_true', help="Use this flag to run with sbs testing.")
    ...
    while int(checkpoint.step) <= config.num_train_steps:
        ...
        if config.test_sbs:
            with dump_callback(config.dump_config):
        ...
        if (step % args.log_freq == 0) and (local_step % args.gradient_accumulation_steps == 0):
            ...
            log('Step:{step:6d}, Loss:{total_loss:10.6f}, Gen_loss:{masked_lm_loss:10.6f}, Disc_loss:{disc_loss:10.6f}, Gen_acc:{masked_lm_accuracy:6.2f}, '
                ...
                loss_scale=optimizer.loss_scale if config.hmp else 1,
