...
def parse_args():
    ...
    parser.add_argument("--use_hpu", type=str, default="false", help="Use hpu for training")
    parser.add_argument("--bfloat16", type=str, default="false", help="Train with bfloat16 precision (when using hpu)")
    parser.add_argument("--train_iter_per_epoch", default=-1.0, type=float,help="Total number of training iterations Per Epoch.(For Perf Short Run )")
...
@tf.function
def train_step(model, inputs, loss, hmp, opt, init, v2=False, loss_class=None, fp16=False, clip_norm=1.0):
...
def main():
    ...
    # Adding the hpu support
    if args.use_hpu.lower() == "true":
        from habana_frameworks.tensorflow import load_habana_module
        # Op override must be disabled due to https://github.com/tensorflow/tensorflow/issues/30253
        log_info_devices = load_habana_module(allow_op_override=False)
        print(f"Devices:\n {log_info_devices}")
        #if args.bfloat16.lower() == "true":
        if args.hmp:
            os.environ['TF_BF16_CONVERSION'] = '1'
        else:
            os.environ['TF_BF16_CONVERSION'] = 'false'
    ...
    train_log_dir = os.path.join("logs_squad", current_time,
                                 'train_squad_' + str(get_rank()) + '_of_' + str(get_world_size()))
        ...
        if args.do_train:
            ...
            for iter, inputs in enumerate(epoch_iterator):
                ...
                if args.max_steps > 0 and (epoch * train_steps_per_epoch + iter) > args.max_steps:
                    break
                if args.train_iter_per_epoch != -1 and iter > args.train_iter_per_epoch:
                    break
                iter_start = time.time()
                # Optimize the model
                loss_value = train_step(model, inputs, loss, USE_HMP, opt, (iter == 0 and epoch == 0),
                                        v2=args.version_2_with_negative, loss_class=loss_class, fp16=USE_HMP)
                ...
                if iter % args.log_freq == 0:
                    if is_main_process():
                        log("\nEpoch: {:03d}, Step:{:6d}, Loss:{:12.8f}, Perf:{:5.0f}, loss_scale:{}, opt_step:{}".format(epoch, iter, loss_value,
                                                                                              epoch_perf_avg.result() * get_world_size(), opt.loss_scale if config.hmp else 1,
                                                                                              int(opt.iterations)))
                    ...
                    with train_summary_writer.as_default():
                        train_perf_val = float( epoch_perf_avg.result().numpy() * get_world_size()) #Perf Val
                        loss_val = float(loss_value.numpy())
                        tf.summary.scalar('train_perf', train_perf_val, iter)
                        tf.summary.scalar('loss', loss_val , iter)
    ...
    if is_main_process() and args.do_train and args.do_eval:
        ...
        # Saving Scalar Data
        with train_summary_writer.as_default():
            log("Adding TF events for exact_match and F1")
            tf.summary.scalar('exact_match', exact_match ,1 )
