...
def squad_convert_examples_to_features(
    ...
    features = [
        annotate_(example) for example in tqdm(
            examples,
            total=len(examples),
            desc="convert squad examples to features",
            mininterval=5,
            disable=get_rank() not in [-1, 0]
        )
    ]
    new_features = []
    ...
    for example_features in tqdm(features, total=len(features), desc="add example index and unique id",
                                 mininterval=5, disable=get_rank() not in [-1, 0]):
...
class SquadProcessor(DataProcessor):
    ...
    def get_examples_from_dataset(self, dataset, evaluate=False):
        ...
        examples = []
        for tensor_dict in tqdm(dataset, mininterval=5, disable=get_rank() not in [-1, 0]):
    ...
    def _create_examples(self, input_data, set_type):
        ...
        examples = []
        for entry in tqdm(input_data, mininterval=5, disable=get_rank() not in [-1, 0]):
            title = entry["title"]
