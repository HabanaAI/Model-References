...
###############################################################################
# Changes:
# - Added support for HabanaLayerNorm
...
from habana_frameworks.tensorflow.ops.layer_norm import *
...
def layer_norm(x,
  ...
  with tf.variable_scope(
    ...
    if use_fused_layer_norm:
        out, _, _ = habana_ops.habana_layer_norm(
                x=x,
                beta=bias,
                gamma=scale,
                axes=tensor_util.make_tensor_proto(len(x.shape)-1),
                epsilon=tensor_util.make_tensor_proto(epsilon)
        )
    else:
        out = layer_norm_compute(x, epsilon, scale, bias,
