  ...
  def _fast_decode_tpu(self,
    ...
    def preprocess_targets(targets, i):
      ...
      if positional_encoding is not None:
        targets += tf.gather(positional_encoding, [i], axis=1)
      return targets
    ...
    def symbols_to_logits_tpu_fn(ids, i, cache):
      ...
      bias = tf.gather(decoder_self_attention_bias, [i], axis=2)
    ...
    ret = fast_decode_tpu(
        ...
        top_k=top_k,
        use_top_k_with_unique=not tf.flags.FLAGS.use_fast_inference)
    if partial_targets is not None:
