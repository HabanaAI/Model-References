...
# Changes:
# - updated imports
# - added 'to_float' helper
# - added support for fast inference on HPU
# - replaced tf.slice with tf.gather
# - replaced tf.python.ops.alias_inplace_update with tf.add + tf.scatter_nd
...
def fast_tpu_gather(params, indices, name=None):
  ...
  with tf.name_scope(name):
    ...
    if tf.flags.FLAGS.use_fast_inference or dtype.is_integer:
...
def compute_topk_scores_and_seq(sequences,
  ...
  """
  if not tf.flags.FLAGS.use_fast_inference and not use_tpu:
...
def beam_search(symbols_to_logits_fn,
  ...
  if tf.flags.FLAGS.use_fast_inference or use_tpu:
  ...
  def grow_alive(curr_seq, curr_scores, curr_log_probs, curr_finished, states):
    ...
    return compute_topk_scores_and_seq(curr_seq, curr_scores, curr_log_probs,
                                       ...
                                       "grow_alive", states, use_tpu=use_tpu, use_top_k_with_unique=use_top_k_with_unique)
  ...
  def grow_topk(i, alive_seq, alive_log_probs, states):
    ...
    if (tf.flags.FLAGS.use_fast_inference or use_tpu) and states:
      flat_ids = tf.reshape(
        tf.gather(alive_seq, [i], axis=2), [batch_size * beam_size, -1])
    else:
    ...
    elif tf.flags.FLAGS.use_fast_inference or use_tpu:
    ...
    if not tf.flags.FLAGS.use_fast_inference and not use_tpu:
      ...
      # While TPU originally uses alias_inplace_update it's unavailable on HPU.
      # Due to a bug in TensorScatterUpdate we're currently using add + scatter_nd
      topk_seq = tf.add(topk_seq, tf.scatter_nd([[i + 1]], tf.expand_dims(topk_ids, 0), topk_seq.shape))
  ...
  def _is_not_finished(i, unused_alive_seq, alive_log_probs,
    ...
    Returns:
      Bool.
    """
    max_length_penalty = tf.pow(((5. + to_float(decode_length)) / 6.), alpha)
