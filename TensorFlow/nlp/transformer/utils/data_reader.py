...
# Changes:
# - updated imports
# - changed to_int32 to cast
# - removed '_summarize_features'
# - remove mlperf logs
# - removed tpu code
# - added dataset padding
# - added support for deterministic_dataset hparam
...
def input_fn(dataset,
  ...
  if hasattr(hparams, 'deterministic_dataset') and hparams.deterministic_dataset:
    num_threads = 1
  ...
  if not batch_size_means_tokens:
    ...
    if config and config.use_tpu:
      ...
      if force_fixed_batch_size:
        cur_batching_scheme["batch_sizes"] = [fixed_batch_size, fixed_batch_size]
      ...
      if force_fixed_batch_size and is_training:
        def _force_shape(example):
          for _, t in six.iteritems(example):
            ...
            t.get_shape().assert_is_fully_defined()
          return example
        dataset = dataset.map(_force_shape, num_parallel_calls=num_threads)
