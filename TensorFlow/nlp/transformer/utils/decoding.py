...
# Changes:
# - updated imports
# - disabled is_text2class
# - removed mlperf logs
# - added padding for HPU
# - added support for horovod
# - added support for fast inference on HPU
# - added full batch padding
...
from TensorFlow.nlp.transformer.data_generators import problem as problem_lib
...
from TensorFlow.nlp.transformer.utils.mpi import MPI_barrier
...
def log_decode_results(inputs,
  ...
  if is_image and save_images:
    ...
    if decoded_inputs and log_results and not is_video:
  ...
  if decoded_inputs and targets is not None and log_results and not is_video:
...
def decode_from_file(estimator,
  ...
  if tf.flags.FLAGS.use_fast_inference or estimator.config.use_tpu:
    def encode(line):
      if has_input:
        return inputs_vocab.encode(line.strip()) + [1]
      else:
        return targets_vocab.encode(line)
    def input_fn(params):
      encoded_inputs = list(map(encode, sorted_inputs))
      length = max(len(inp) for inp in encoded_inputs)
      batch_ids = []
      ...
      # Pad to a full batch
      batch_ids.extend([[text_encoder.EOS_ID] * length] * (len(batch_ids) % batch_size))
      ...
      if hparams.use_horovod:
          dataset = dataset.shard(num_shards=hparams.hvd_size, index=hparams.hvd_worker_id)
      return dataset
  else:
    use_static_shapes = hparams.use_hpu and hparams.use_static_shapes
    def input_fn():
      input_gen = _decode_batch_input_fn(
          num_decode_batches, sorted_inputs,
          inputs_vocab, decode_hp.batch_size,
          decode_hp.max_input_size,
          task_id=decode_hp.multiproblem_task_id, has_input=has_input, use_static_shapes=use_static_shapes)
      gen_fn = make_input_fn_from_generator(input_gen, use_static_shapes=use_static_shapes)
      example = gen_fn()
  ...
  for elapsed_time, result in timer(result_iter):
    if decode_hp.return_beams:
      ...
      for k, beam in enumerate(output_beams):
        ...
        decoded_inputs, decoded_outputs, _ = log_decode_results(
            result["inputs"],
            beam,
            problem_name,
            None,
      ...
      if decode_hp.write_beam_scores:
        ...
        decoded_outputs = "\t".join(beam_decodes)
    else:
      decoded_inputs, decoded_outputs, _ = log_decode_results(
          result["inputs"],
          result["outputs"],
          ...
          skip_eos_postprocess=decode_hp.skip_eos_postprocess)
    num_sentences += 1
    translations[decoded_inputs] = decoded_outputs
  ...
  decode_filename_base = decode_to_file if decode_to_file else filename
  if not decode_to_file:
    decode_filename = _decode_filename(decode_filename_base, problem_name, decode_hp)
  else:
    decode_filename = _add_shard_to_filename(decode_filename_base, decode_hp)
  part_suffix = '.part.json'
  decode_filename = decode_filename + part_suffix
  tf.logging.info("Writing partial decodes into %s" % decode_filename)
  with tf.gfile.Open(decode_filename, 'w') as outfile:
    json.dump(translations, outfile)

  MPI_barrier()

  if not decode_hp.use_horovod or decode_hp.hvd_worker_id == 0:
      translation_dict = {}
      for shard in range(decode_hp.hvd_size if decode_hp.use_horovod else 1):
        part = _add_shard_to_filename(decode_filename_base, decode_hp, shard) + part_suffix
        tf.logging.info("Reading partial decodes from %s" % part)
        with tf.gfile.Open(part, "r") as f:
          translation_dict.update(json.load(f))

      tf.logging.info("Merging partial decodes into %s" % decode_filename_base)
      with tf.gfile.Open(decode_filename_base, 'w') as f:
        for idx in range(len(sorted_inputs)):
          inp = sorted_inputs[sorted_keys[idx]]
          assert inp in translation_dict
          print(translation_dict[inp], file=f)
...
def _add_shard_to_filename(filename, decode_hp, shard_id=None):
  if not decode_hp.use_horovod:
    return filename
  if shard_id is None:
    shard_id = decode_hp.hvd_worker_id
  if decode_hp.shard_google_format:
    return filename + "-{0:05d}-of-{1:05d}".format(shard_id, decode_hp.hvd_size)
  else:
    return filename + ("%.3d" % shard_id)
...
def make_input_fn_from_generator(gen, use_static_shapes=False):
...
def _decode_batch_input_fn(num_decode_batches, sorted_inputs, vocabulary,
                           ...
                           task_id=-1, has_input=True, use_static_shapes=False):
  """Generator to produce batches of inputs."""
  tf.logging.info(" batch %d" % num_decode_batches)

  if use_static_shapes:
      global_max_length = max([len(vocabulary.encode(i)) for i in sorted_inputs]) + 1

  for b in range(num_decode_batches):
    tf.logging.info(f"Decoding batch {b+1}/{num_decode_batches}")
    batch_inputs = []
    ...
    final_batch_inputs = []
    if use_static_shapes:
        batch_length = global_max_length
    else:
        batch_length = max([len(ids) for ids in batch_inputs])

    for input_ids in batch_inputs:
      assert len(input_ids) <= batch_length
      x = input_ids + [0] * (batch_length - len(input_ids))
      final_batch_inputs.append(x)
    full_batch_padding = [[text_encoder.EOS_ID] * batch_length] * (batch_size - len(final_batch_inputs))
    final_batch_inputs = final_batch_inputs + full_batch_padding
...
def _decode_input_tensor_to_features_dict(feature_map, hparams, decode_hp):
  ...
  features["decode_length"] = (
      ...
      decode_hp.extra_length if tf.flags.FLAGS.use_fast_inference else tf.constant(decode_hp.extra_length))
  features["inputs"] = x
  # Save inputs to "partial_targets" when prepending inputs to targets. Also
  # keep "inputs" as some models crash if they don't exist.
  if getattr(hparams, "prepend_mode", "none") != "none":
