...
# Changes:
# - updated imports
# - use_hpu argument for setting device name in data_parallelism
...
def data_parallelism(daisy_chain_variables=True,
  ...
  if no_data_parallelism:
    ...
    tf.logging.warn(
        ...
        schedule)
    device_fmt = "/device:HPU:%d" if use_hpu else "gpu:%d"
    datashard_devices = [device_fmt % d for d in _gpu_order(worker_gpu)]
    if worker_gpu < 1:
