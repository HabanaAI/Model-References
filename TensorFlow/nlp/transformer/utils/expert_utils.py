...
from TensorFlow.nlp.transformer.layers.vq_discrete import DiscreteBottleneck
...
def cv_squared(x):
  ...
  """
  epsilon = 1e-10
  float_size = to_float(tf.size(x)) + epsilon
...
def _gates_to_load(gates):
  ...
  """
  return tf.reduce_sum(to_float(gates > 0), 0)
...
def vq_gating(x,
  ...
  with tf.variable_scope(name, reuse=tf.AUTO_REUSE):
    ...
    if hparams.use_scales:
      scale_loss = tf.reduce_mean(to_float(count_per_expert) * scales)
...
def local_moe_tpu(inputs,
  ...
  if isinstance(length, int):
    ...
    expert_capacity = tf.minimum(
        length, tf.to_int32(
            to_float(length) * 2 * overhead / num_experts))
  expert_capacity_f = to_float(expert_capacity)

  # This is the learned gating function.
  gates = tf.nn.softmax(
      to_float(common_layers.dense(inputs, num_experts, name="logits")))
  ...
  mask_1 *= to_float(tf.less(position_in_expert_1, expert_capacity_f))
  # [batch, 1, num_experts]
  ...
  use_rate = (mask_1_count + 1.0) / to_float(length)
  ...
  expected_exhausted = to_float(tf.greater(
      tf.reshape(to_float(tf.range(length)), [1, length, 1]),
  ...
  mask_2 *= to_float(tf.less(position_in_expert_2, expert_capacity_f))
  ...
  miss_rate_1 = 1.0 - tf.reduce_sum(mask_1_count) / to_float(batch * length)
  miss_rate_2 = 1.0 - tf.reduce_sum(mask_2_count) / to_float(batch * length)
