...
# Changes:
# - updated imports
# - removed hparams.summarize_vars, hparams.summarize_grads
# - remove unused optmimizers
# - added horovod support
# - added check if optimizer has get_gradients defined
...
from TensorFlow.nlp.transformer.utils import misc_utils
from TensorFlow.nlp.transformer.utils import multistep_optimizer
...
class ConditionalOptimizer(tf.train.Optimizer):
  ...
  def compute_gradients(self, loss, var_list=None, **kwargs):  # pylint: disable=arguments-differ
    if contrib.is_tf2 and getattr(self._opt, 'get_gradients', None) is not None:
