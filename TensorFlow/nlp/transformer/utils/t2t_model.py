...
# Changes:
# - updated imports
# - added call to write_hparams_v1
...
from TensorFlow.nlp.transformer.data_generators.problem import problem_hparams_to_features
from TensorFlow.nlp.transformer.layers import common_layers
from TensorFlow.nlp.transformer.layers import modalities
from TensorFlow.nlp.transformer.layers.common_attention import mixed_precision_is_enabled
...
from TensorFlow.nlp.transformer.utils import expert_utils as eu
from TensorFlow.nlp.transformer.utils import hparams_lib
from TensorFlow.nlp.transformer.utils import learning_rate
from TensorFlow.nlp.transformer.utils import metrics
from TensorFlow.nlp.transformer.utils import optimize
from TensorFlow.nlp.transformer.utils import quantization
from TensorFlow.nlp.transformer.utils import registry
from TensorFlow.nlp.transformer.utils import scheduled_sampling
...
class T2TModel(base.Layer):
  ...
  def __init__(self,
    ...
    if self._problem_hparams:
      for feature_name, modality in six.iteritems(
        ...
        if (modality in (modalities.ModalityType.IDENTITY_SYMBOL,
  ...
  def summarize_hparams(self):
    def create_hparams_summary(hparams, name):
      # Some params are not str/bool/int/float so we filter them out
      #  return {f'{name}{k}': v for k, v in hparams.values().items()}
      ret = {}
    ...
    model_hparams = create_hparams_summary(self._hparams, '')
    problem_hparams = {}
    if self._problem_hparams:
      problem_hparams = create_hparams_summary(
        self._problem_hparams, 'problem/')

    write_hparams_v1(self._hparams.model_dir,
      {**model_hparams, **problem_hparams})
        ...
        for name, value in sorted(six.iteritems(kwargs)):
          if name.startswith("ScalarSummary"):
            ...
            contrib.summary().scalar(
                name, tf.reduce_mean(tf.cast(value, tf.float32)), step=gs)
