...
###############################################################################
# Changes:
# - function tensorflow_transform.tf_metadata.dataset_schema.from_feature_spec replaced with
#   tensorflow_transform.tf_metadata.schema_utils.schema_from_feature_spec for TF2.4+ compatibility
# - add possibility to change DATA_BUCKET_FOLDER and LOCAL_DATA_TFRECORDS_DIR by env variables
# - disable remaining examples preprocessing in _transform_to_tfrecords_from_reslice function
#   for validation samples by keeping TEST_SET_MODE = False
...
DATA_BUCKET_FOLDER = os.getenv("DATA_BUCKET_FOLDER", "/outbrain/orig/")
SPARK_TEMP_FOLDER = "/tmp/spark/spark-temp/"
LOCAL_DATA_TFRECORDS_DIR = os.getenv("LOCAL_DATA_TFRECORDS_DIR", "/outbrain/tfrecords")

# Keep this value as False, due to an issue in remaining examples preprocessing
TEST_SET_MODE = False
...
def make_spec(output_dir, batch_size=None):
    ...
    metadata = dataset_metadata.DatasetMetadata(schema_from_feature_spec(spec))
