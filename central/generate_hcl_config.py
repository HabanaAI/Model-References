...
from central.multi_node_utils import is_hccl_api, run_cmd_as_subprocess
...
def generate_hcl_config_r(file_path, devices_per_hls, hls_type="HLS1"):
    ...
    if hcl_config_path:
        ...
        try:
            ...
            hcl_config_path = _get_default_hcl_config_path(
                file_path, num_workers_total)
    ...
    if hcl_config_path:
        ...
        if not is_hccl_api():
            try:
                ...
                raise Exception(
                    f"{host_name}: Error in {__file__} generate_hcl_config_unless_hccl({file_path}, {devices_per_hls}, {hls_type})") from exc
        else:
            return None
