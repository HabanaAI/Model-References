...
import subprocess
from central.habana_model_runner_utils import HabanaEnvVariables, get_canonical_path, get_canonical_path_str, is_valid_multi_node_config, get_multi_node_config_nodes
from central.multi_node_utils import run_cmd_as_subprocess, run_per_ip, generate_mpi_hostfile, print_file_contents
import central.generate_hcl_config as generate_hcl_config

class TrainingRunHWConfig():
    def __init__(self, args, output_dir="demo_bert_log"):
        self.args = args
    ...
    def create_single_hls_setup(self, tmp_dir):
        ...
        hcl_config_path = generate_hcl_config.generate_hcl_config_r(str(tmp_dir), self.num_workers_per_hls, self.args.hls_type)
    ...
    def create_multi_hls_setup(self, tmp_dir):
        ...
        # Multi-HLS Mode
        #
        if os.environ.get('MPI_TCP_INCLUDE'):
            mpi_tcp_include = os.environ.get('MPI_TCP_INCLUDE')
        else:
            mpi_tcp_include = "enp3s0"
        print(f"mpi_tcp_include = {mpi_tcp_include}")

        gen_hcl_path = Path(__file__).parent.joinpath('generate_hcl_config.py')
        # Create HCL config on each remote IP.
        run_per_ip(f"python3 {str(gen_hcl_path)} {str(tmp_dir)} {self.num_workers_per_hls} {self.args.hls_type}", ['MULTI_HLS_IPS', 'PYTHONPATH'], False)
        ...
        mpi_hostfile_path = generate_mpi_hostfile(str(tmp_dir), self.num_workers_per_hls)
    ...
    def create_multi_worker_setup(self):
        ...
        output_file_name = str(tmp_dir.joinpath(self.output_dir))
