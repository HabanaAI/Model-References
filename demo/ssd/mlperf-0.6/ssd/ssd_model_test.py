...
class SSDModelTest(test.TestCase):
  ...
  def testIOU(self):
    ...
    with tf.Session() as sess, tf.device("/device:CPU:0"):
      ...
      iou = sess.run(iou_fn, { boxes: boxes_np })
  ...
  def testNonMaxSuppressionOp(self):
    ...
    with tf.Session() as sess, tf.device("/device:CPU:0"):
      ...
      (scores1, boxes1) = ssd_model._filter_scores(scores, boxes)
      (scores2, boxes2) = ssd_architecture.non_max_suppression_padded(
          scores=tf.cast(scores1, tf.float32),
          boxes=tf.cast(boxes1, tf.float32),
          max_output_size=12,
          iou_threshold=0.5)
      ...
      (scores_result, boxes_result) = sess.run([scores2, boxes2], inputs)

    scores_output = np.array(scores_result)
    boxes_output = np.array(boxes_result)
    self.assertAllClose(
        boxes_output,
