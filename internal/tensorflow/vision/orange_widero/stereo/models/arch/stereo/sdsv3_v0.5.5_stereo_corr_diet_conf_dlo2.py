...
from menta.src.layers import ZeroPadding2D, Cropping2D,\
    Maximum, Concatenate, Slice, Clamp, Identity, SplitLayer
...
def corr_scores(I_cntr, I_srnd_0, I_srnd_1, I_srnd_2, T_cntr_srnd, origin_l2, focal_l2, steps,
                batch_norm, training, bn_first, regularizer):
    images = Concatenate(axis=0)([I_cntr, I_srnd_0, I_srnd_1, I_srnd_2])
    corr_features = CorrFeaturesLayer(name='corr_features', batch_norm=batch_norm, training=training,
                                      bn_first=bn_first, regularizer=regularizer, activation_and_bn_output=False)(images)
    (corr_features_cntr_l2, corr_features_srnd_0_l2,
     corr_features_srnd_1_l2, corr_features_srnd_2_l2) = SplitLayer(4, axis=0)(corr_features)
    ...
    return Maximum(name='corr_scores_l2')([corr_scores_srnd_0_l2, Maximum()([corr_scores_srnd_1_l2,
                                                                             corr_scores_srnd_2_l2])])
...
def arch(I_cntr, I_srnd_0, I_srnd_1, I_srnd_2, T_cntr_srnd, focal_l2, origin_l2,
    ...
    corr_scores_l2 = corr_scores(I_cntr, I_srnd_0, I_srnd_1, I_srnd_2, T_cntr_srnd, origin_l2, focal_l2, steps,
                                 batch_norm, training, bn_first, regularizer)
    ...
    pred_inv_depth = Clamp(min_val=1/max_Z, max_val=1/min_Z)(unet_up_l0)
