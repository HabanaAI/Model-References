...
def arch(I_cntr, I_srnd_0, I_srnd_1, I_srnd_2, T_cntr_srnd, focal, origin, steps,
    ...
    # T_cntr_srnd = reshape(T_cntr_srnd, [-1, 3, 3])
    # focal = reshape(focal, [-1, 2])
    # origin = reshape(origin, [-1, 2])
    # steps = reshape(steps, [-1, 45])
    ...
    unet_up_l0 = UpConvLayer(ch_out=6,   name='unet_up_l0', batch_norm=batch_norm, training=training,
                             bn_first=bn_first, regularizer=regularization)([unet_up_l1, to_skip_l0])
