...
from tensorflow.keras.backend import name_scope, ones_like, zeros_like
...
def arch(I_cntr, I_srnd_0, I_srnd_1, T_cntr_srnd, focal, origin, steps,
    ...
    corr_scores_l2 = maximum([corr_scores_srnd_0_l2, corr_scores_srnd_1_l2], name='corr_scores_l2')
    ...
    if error_branch_layers == 1:
        conf_up_l0 = UpConvLayer(ch_out=1, name='conf_up_l0', batch_norm=False, training=training,
                                 activation='relu', regularizer=regularization)([unet_up_l1, to_skip_l0])
    elif error_branch_layers == 2:
        conf_up_l1 = UpConvLayer(ch_out=12, name='conf_up_l1', batch_norm=batch_norm, training=training,
                                 bn_first=bn_first, regularizer=regularization)([unet_up_l2, to_skip_l1])
        conf_up_l0 = UpConvLayer(ch_out=1, name='conf_up_l0', batch_norm=False, training=training,
                                 activation='relu', regularizer=regularization)([conf_up_l1, to_skip_l0])
    elif error_branch_layers == 3:
        conf_up_l2 = UpConvLayer(ch_out=24, name='conf_up_l2', batch_norm=batch_norm, training=training,
                                 bn_first=bn_first, regularizer=regularization)([unet_up_l3, to_skip_l2])
        conf_up_l1 = UpConvLayer(ch_out=12, name='conf_up_l1', batch_norm=batch_norm, training=training,
                                 bn_first=bn_first, regularizer=regularization)([conf_up_l2, to_skip_l1])
        conf_up_l0 = UpConvLayer(ch_out=1, name='conf_up_l0', batch_norm=False, training=training,
                                 activation='relu', regularizer=regularization)([conf_up_l1, to_skip_l0])
    pred_inv_depth = unet_up_l0
    pred_error = conf_up_l0

    pred_inv_depth = minimum([maximum([pred_inv_depth, 1/max_Z*ones_like(pred_inv_depth)]),
                             1/min_Z*ones_like(pred_inv_depth)])  # this should be called clipped_l0

    out = unpad_image(pred_inv_depth, cropping)
    out_conf = unpad_image(pred_error, cropping)
    _ = identity(out, name='out')
    _ = identity(out_conf, name='out_conf')
    return {'out': out, 'out_conf': out_conf}

    # out = concatenate([pred_inv_depth, pred_error], axis=3)
    # out = unpad_image(out, cropping)
    # _ = identity(out[..., 0, None], name='out_inv_z')
    # _ = identity(out[..., 1, None], name='out_conf')
    # return out
