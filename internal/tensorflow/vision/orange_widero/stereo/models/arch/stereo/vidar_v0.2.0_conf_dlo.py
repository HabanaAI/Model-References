...
from menta.src.layers import Add, Concatenate, Slice, Identity, Clamp
...
def arch(I_cntr, I_srnd_0, I_srnd_1, I_srnd_2, T_cntr_srnd, focal_l2, origin_l2,
         min_Z=None, max_Z=None, training=False, regularization=None, reg_constant=0.01, corr_steps_kwargs=None):
    ...
    corr_features_func = CorrFeaturesLayer(name='corr_features', regularizer=regularizer, activation_and_bn_output=False)
    ...
    weight_scores_srnd_0 = tf.Variable(1.0, name='weight_scores_srnd_0')
    weight_scores_srnd_1 = tf.Variable(1.0, name='weight_scores_srnd_1')
    weight_scores_srnd_2 = tf.Variable(1.0, name='weight_scores_srnd_2')

    corr_scores_srnd_0_l2 = correlation_map(corr_features_cntr_l2, corr_features_srnd_0_l2, origin_l2, focal_l2,
                                            T_cntr_srnd0, steps, name='corr_scores_srnd_0_l2') * weight_scores_srnd_0
    corr_scores_srnd_1_l2 = correlation_map(corr_features_cntr_l2, corr_features_srnd_1_l2, origin_l2, focal_l2,
                                            T_cntr_srnd1, steps, name='corr_scores_srnd_1_l2') * weight_scores_srnd_1
    corr_scores_srnd_2_l2 = correlation_map(corr_features_cntr_l2, corr_features_srnd_2_l2, origin_l2, focal_l2,
                                            T_cntr_srnd2, steps, name='corr_scores_srnd_2_l2') * weight_scores_srnd_2
    corr_scores_l2 = Add(name='corr_scores_l2')([corr_scores_srnd_0_l2, corr_scores_srnd_1_l2, corr_scores_srnd_2_l2])
    ...
    unet_up_l5 = UpConvLayer(ch_out=144, name='unet_up_l5', regularizer=regularizer)([unet_dn_l6, to_skip_l5])
    unet_up_l4 = UpConvLayer(ch_out=96,  name='unet_up_l4', regularizer=regularizer)([unet_up_l5, to_skip_l4])
    unet_up_l3 = UpConvLayer(ch_out=48,  name='unet_up_l3', regularizer=regularizer)([unet_up_l4, to_skip_l3])
    unet_up_l2 = UpConvLayer(ch_out=24,  name='unet_up_l2', regularizer=regularizer)([unet_up_l3, to_skip_l2])
    unet_up_l1 = UpConvLayer(ch_out=16,  name='unet_up_l1', regularizer=regularizer)([unet_up_l2, to_skip_l1])
    unet_up_l0 = UpConvLayer(ch_out=8,   name='unet_up_l0', regularizer=regularization)([unet_up_l1, to_skip_l0])
    ...
    # confidence branch
    conf_up_l1 = UpConvLayer(ch_out=16, name='conf_up_l1', regularizer=regularizer, concat_skip_last=False)([unet_up_l2, to_skip_l1])
    conf_up_l0 = UpConvLayer(ch_out=8, name='conf_up_l0', regularizer=regularizer, concat_skip_last=False)([conf_up_l1, to_skip_l0])
    conf_up_l0_ch1 = Conv2DWrap(filters=1, kernel_size=3, name='conf_up_l0_ch1', strides=[1, 1], padding="same",
                                data_format='channels_last', kernel_initializer=glorot_normal(),
                                kernel_regularizer=regularization)(conf_up_l0)

    pred_inv_depth = Clamp(min_val=1 / max_Z, max_val=1 / min_Z)(unet_up_l0_ch1)
    pred_error = conf_up_l0_ch1
    out = Identity(name='out')(pred_inv_depth)
    out_conf = Identity(name='out_conf')(pred_error)
