...
from menta.src.layers import Add, Concatenate, Slice, Identity, Clamp, Reshape
...
from stereo.models.layers.keras_layers import Conv2DWrap, glorot_normal, Ones
...
def arch(I_cntr, I_srnd_0, I_srnd_1, I_srnd_2, T_cntr_srnd, focal_l2, origin_l2,
    ...
    corr_scores_shape = corr_scores_srnd_0_l2.shape.as_list()
    corr_scores_concat = Concatenate(axis=3, name='corr_scores_concat')(
        [
            Reshape((corr_scores_shape[1] * corr_scores_shape[2], corr_scores_shape[3], 1))(corr_scores_srnd_0_l2),
            Reshape((corr_scores_shape[1] * corr_scores_shape[2], corr_scores_shape[3], 1))(corr_scores_srnd_1_l2),
            Reshape((corr_scores_shape[1] * corr_scores_shape[2], corr_scores_shape[3], 1))(corr_scores_srnd_2_l2)
        ])
    corr_scores_conv = Conv2DWrap(filters=1, kernel_size=(1, 1), name='corr_scores_conv', padding="same",
                                  data_format='channels_last', kernel_initializer=Ones(),
                                  kernel_regularizer=regularizer)(corr_scores_concat)
    corr_scores_l2 = Reshape((corr_scores_shape[1], corr_scores_shape[2], corr_scores_shape[3]))(
        corr_scores_conv)
    ...
    _ = tf.identity(pred_inv_depth, name='out')
    _ = tf.identity(pred_error, name='out_conf')
