...
def arch(inp, origin=[350, 150], num_of_categories=3, back_frames=1, batch_norm=False, training=False, bn_first=True,
         regularization=None):
    """
    ...
    frames = tf.split(inp, back_frames, -1)
    corr_features = []
    mono_features_l2, to_skip_l0, to_skip_l1 = MonoFeaturesLayer(name='mono_features_l2', batch_norm=batch_norm,
                                                                 ...
                                                                 regularizer=regularization)(frames[0])
    ...
    for frame in frames:
        corr_features.append(corr_features_func(frame))

    if back_frames > 1:
        unet_dn_l2 = concatenate(corr_features, axis=3, name='concat_l2')
    else:
        unet_dn_l2 = corr_features[0]
    ...
    unet_up_l0 = UpConvLayer(ch_out=num_of_categories, name='unet_up_l0', batch_norm=batch_norm, training=training,
                             deconv_kernel_sz=4,
    ...
    for i in range(back_frames):
        _ = identity(tf.slice(inp, [0, 0, 0, i], [-1, -1, -1, 1]), name='%s_frames_back' % i)
    return out
