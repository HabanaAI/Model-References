...
def loss(out, I_cntr, T_cntr_srnd, focal, origin, photo_loss_im_mask,
         measure='ssim', radius=1, reg_constant=1.0, stop_gradient_depth=False, max_Z=None):

    invZ = out['out']
    if stop_gradient_depth:
        invZ = tf.stop_gradient(invZ)
    ...
    if max_Z is not None:
        photo_loss_im_mask = 1 - (1 - photo_loss_im_mask) * tf.cast((invZ**-1) < max_Z, tf.float32)
