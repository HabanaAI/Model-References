...
@tfmpl.figure_tensor
def mpl_get_boxes_image(boxes, confidence, inp_resolution, origin, focal, confidence_thresh=0.0, label_image=None):
    if label_image is None:
        label_image = np.zeros(inp_resolution, dtype=np.float32)
    ...
    for i, box in enumerate(boxes):
        ...
        color = float(confidence[i]) #np.array(3*[confidence[i]] + [1])
        fillPoly(image, pts=[box_pixels], color=color)
    fig = tfmpl.create_figure(figsize=(5, 12))
    ax = fig.add_subplot(111)
    ax.axis('off')
    # ax.imshow(label_image, cmap='rainbow')
...
def get_boxes_to_labels_loss(pred_boxes, labels):
    ...
    labels = five_box_to_six_box(labels)  # shape: (?, 50, 6)
    broadcast_labels = tf.expand_dims(labels, 2)  # shape: (?, 50, 1, 6)
    _, out_length, out_width, _ = pred_boxes.shape.as_list()
    broadcast_boxes = tf.reshape(pred_boxes, (-1, 1, out_length * out_width, 6))  # shape: (?, 1, 40X16, 6)
    diff_boxes = np.abs(broadcast_labels - broadcast_boxes)  # shape: (?, 50, 40X16, 6)
    num_of_labels = labels.shape.as_list()[1]
    reshaped_all_diff = tf.reshape(diff_boxes, (-1, num_of_labels, out_length, out_width, 6))  # shape: (?, 50, 40, 16, 6)
    return reshaped_all_diff


def five_box_to_six_box(five_box):
...
def loss(out, label, top_view_cam, vehicles_boxes, inp_resolution=(700, 300), origin=(300, 150), input_focal=10.0,
    ...
    boxes_params_preds, confidence_preds = out[:, :, :, :6], out[:, :, :, 6]
    out_resolution = boxes_params_preds.shape.as_list()[1:3]  # [40, 16]
    default_boxes = get_default_boxes(out_resolution, inp_resolution, origin, input_focal)  # shape: (40, 16, 6)
    ...
    # Locations loss
    pred_boxes = five_box_to_six_box(default_boxes) + boxes_params_preds  # shape: (?, 40, 16, 6)
    all_boxes_loss = get_boxes_to_labels_loss(pred_boxes, vehicles_boxes)  # shape: (?, 50, 40, 16, 6)
    matching_boxes_loss = tf.expand_dims(matching_labels, -1) * all_boxes_loss  # shape: (? ,50, 40, 16, 6)
    locations_loss = tf.reduce_sum(input_tensor=matching_boxes_loss, axis=[1, 2, 3]) / (tf.reduce_sum(input_tensor=matching_labels) + 1e-6)  # shape: (?, 6)
    ...
    _ = identity(tf.reduce_mean(input_tensor=locations_loss[:, 4]), "scalar_loss_cos")
