...
def motion(inp1, inp2, Z_inv_base, Z_inv_curr, RTcurr2base, focal, origin, x, y, batch_norm=1, training=True):
        ...
        with tf.compat.v1.name_scope("unet_phase_a"):
            ...
            with tf.compat.v1.name_scope("down_convs"):
                ...
                with tf.compat.v1.name_scope("conv_1"):
                    conv_1_ = conv2d(conv_0, 10, down_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_1_0")
                    conv_1 = conv2d(conv_1_, 20, down_conv_width, strides=(2, 2), batch_norm=batch_norm, training=training, name="conv_1_1")
                with tf.compat.v1.name_scope("conv_2"):
                    conv_2_ = conv2d(conv_1, 20, down_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_2_0")
                    conv_2 = conv2d(conv_2_, 40, down_conv_width, strides=(2, 2), batch_norm=batch_norm, training=training, name="conv_2_1")
                with tf.compat.v1.name_scope("conv_3"):
                    conv_3_ = conv2d(conv_2, 40, down_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_3_0")
                    conv_3 = conv2d(conv_3_, 80, down_conv_width, strides=(2, 2), batch_norm=batch_norm, training=training, name="conv_3_1")
                with tf.compat.v1.name_scope("conv_4"):
                    conv_4_ = conv2d(conv_3, 80, down_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_4_0")
                    conv_4 = conv2d(conv_4_, 160, down_conv_width, strides=(2, 2), batch_norm=batch_norm, training=training, name="conv_4_1")
                with tf.compat.v1.name_scope("conv_5"):
                    conv_5_ = conv2d(conv_4, 160, down_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_5_0")
                    conv_5 = conv2d(conv_5_, 320, down_conv_width, strides=(2, 2), batch_norm=batch_norm, training=training, name="conv_5_1")
                with tf.compat.v1.name_scope("conv_6"):
                    conv_6_ = conv2d(conv_5, 320, down_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_6_0")
                    conv_6 = conv2d(conv_6_, 640, down_conv_width, strides=(2, 2), batch_norm=batch_norm, training=training, name="conv_6_1")
            ...
            with tf.compat.v1.name_scope("up_convs"):
                ...
                with tf.compat.v1.name_scope("deconv_6"):
                    deconv_6 = deconv2d(conv_6, up_deconv_width, conv_6_.shape[1:3], int(conv_5.shape[3]),
                                        int(conv_6.shape[3]),
                                        strides=up_deconv_strides, batch_norm=batch_norm, training=training, name="deconv_6")
                    deconv_6 = tf.concat([deconv_6, conv_6_], axis=3)
                    deconv_6 = conv2d(deconv_6, conv_5.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_6_0")
                    deconv_6 = conv2d(deconv_6, conv_5.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_6_1")
                with tf.compat.v1.name_scope("deconv_5"):
                    deconv_5 = deconv2d(deconv_6, up_deconv_width, conv_5_.shape[1:3], int(conv_4.shape[3]),
                                        int(conv_5.shape[3]),
                                        strides=up_deconv_strides, batch_norm=batch_norm, training=training, name="deconv_5")
                    deconv_5 = tf.concat([deconv_5, conv_5_], axis=3)
                    deconv_5 = conv2d(deconv_5, conv_4.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_5_0")
                    deconv_5 = conv2d(deconv_5, conv_4.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_5_1")
                with tf.compat.v1.name_scope("deconv_4"):
                    deconv_4 = deconv2d(deconv_5, up_deconv_width, conv_4_.shape[1:3], int(conv_3.shape[3]),
                                        int(conv_4.shape[3]),
                                        strides=up_deconv_strides, batch_norm=batch_norm, training=training, name="deconv_4")
                    deconv_4 = tf.concat([deconv_4, conv_4_], axis=3)
                    deconv_4 = conv2d(deconv_4, conv_3.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_4_0")
                    deconv_4 = conv2d(deconv_4, conv_3.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_4_1")
                with tf.compat.v1.name_scope("deconv_3"):
                    deconv_3 = deconv2d(deconv_4, up_deconv_width, conv_3_.shape[1:3], int(conv_2.shape[3]),
                                        int(conv_3.shape[3]),
                                        strides=up_deconv_strides, batch_norm=batch_norm, training=training, name="deconv_3")
                    deconv_3 = tf.concat([deconv_3, conv_3_], axis=3)
                    deconv_3 = conv2d(deconv_3, conv_2.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_3_0")
                    deconv_3 = conv2d(deconv_3, conv_2.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_3_1")
                with tf.compat.v1.name_scope("deconv_2"):
                    deconv_2 = deconv2d(deconv_3, up_deconv_width, conv_2_.shape[1:3], int(conv_1.shape[3]),
                                        int(conv_2.shape[3]),
                                        strides=up_deconv_strides, batch_norm=batch_norm, training=training, name="deconv_2")
                    deconv_2 = tf.concat([deconv_2, conv_2_], axis=3)
                    deconv_2 = conv2d(deconv_2, conv_1.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_2_0")
                    deconv_2 = conv2d(deconv_2, conv_1.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_2_1")
                with tf.compat.v1.name_scope("deconv_1"):
                    deconv_1 = deconv2d(deconv_2, up_deconv_width, conv_1_.shape[1:3], int(conv_0.shape[3]),
                                        int(conv_1.shape[3]),
                                        strides=up_deconv_strides, batch_norm=batch_norm, training=training, name="deconv_1")
                    deconv_1 = tf.concat([deconv_1, conv_1_], axis=3)
                    deconv_1 = conv2d(deconv_1, conv_0.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_1_0")
                    deconv_1 = conv2d(deconv_1, conv_0.shape[3], up_conv_width, strides=(1, 1), batch_norm=batch_norm, training=training, name="deconv_1_1")

        with tf.compat.v1.name_scope("argmax_phase_a"):
            argmax = conv2d(deconv_1, 20, 1, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_arg1")
            argmax = conv2d(argmax, 10, 1, strides=(1, 1), batch_norm=batch_norm, training=training, name="conv_arg2")
            argmax = conv2d(argmax, 3, 1, strides=(1, 1), batch_norm=False, training=training, name="conv_arg3", activation=None)

        # Note that this output needs to be multiplied by 5 to get the actual values
        out = tf.identity(argmax, name="motion_output")
