...
from stereo.models.MOVE_utils import loss_geom_motion, loss_phot_motion, object_consistency_descriptor_loss
...
def loss(out, curr_inp, prev_inp, next_inp, prev_curr2frame, next_curr2frame, curr_T_cntr_srnd,
    ...
    with tf.compat.v1.name_scope("Loss_"):
        ...
        loss_geo_X, loss_geo_Y, loss_geo_Z, Z_inv2_warp, Z_inv2, Z_diff_im, X_diff_im, Y_diff_im = loss_geom_motion(
            ...
            x, y, mask2=curr_im_mask, return_diff_ims=True)
        ...
        motion_masked_no_y = tf.concat(
            [tf.expand_dims(curr_motion[:, :, :, 0], 3), tf.zeros_like(tf.expand_dims(curr_motion[:, :, :, 0], 3)),
             curr_motion[:, :, :, 2:]], axis=3)

        loss_object, object_loss_Z, direct_loss_X, \
        direct_loss_Z, sum_direct, des_pull_loss, des_push_loss = object_consistency_descriptor_loss(motion_masked_no_y,
                                                                                          curr_instance_mask,
                                                                                          curr_instance_ids,
                                                                                          curr_seg_im,
                                                                                          curr_vcl, min_des_loss=True,
                                                                                                     with_static=True)


        direct_loss = direct_loss_X*6. + direct_loss_Z

        des_loss = des_pull_loss + 20.*des_push_loss

        loss_geo_motion = 30 * loss_geo_Z + .25 * loss_geo_X + 0. * loss_geo_Y
        ...
        loss_motion_ = loss_geo_motion + loss_photo_motion * .5 + static_motion_loss * 2.5

        reg_losses = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES)
        reg_loss = reg_constant * sum(reg_losses)

        # loss_ = loss_motion_ * 10.
        loss_ = direct_loss * 2. + des_loss + static_motion_loss * 5.
                # + static_motion_loss + loss_object
        ...
        _ = tf.reverse(tf.expand_dims(curr_motion[:,:,:,3], 3), axis=[1], name='des_1')
        _ = tf.reverse(tf.expand_dims(curr_motion[:,:,:,4], 3), axis=[1], name='des_2')
        _ = tf.reverse(tf.expand_dims(curr_motion[:,:,:,5], 3), axis=[1], name='des_3')
        ...
        _ = tf.identity(des_loss, name="des_loss")
        _ = tf.identity(des_push_loss, name="des_push_loss")
        _ = tf.identity(des_pull_loss, name="des_pull_loss")
