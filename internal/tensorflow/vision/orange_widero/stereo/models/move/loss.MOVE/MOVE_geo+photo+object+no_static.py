...
from stereo.models.MOVE_utils import loss_geom_motion, loss_phot_motion, object_consistency_loss, open_bit_mask
...
def loss(out, curr_inp, prev_inp, next_inp, prev_curr2frame, next_curr2frame, curr_T_cntr_srnd,
         ...
         next_instance_ids, x, y, reg_constant=1.0):
    ...
    with tf.compat.v1.name_scope("Loss_"):
        ...
        loss_geo_X, loss_geo_Y, loss_geo_Z, Z_inv2_warp, Z_inv2, Z_diff_im, X_diff_im, Y_diff_im = loss_geom_motion(Z_inv1_cntr, Z_inv2_cntr,
                                                                                   motion_masked_no_y, curr_focal, curr_origin,
                                                                                   tf.linalg.inv(prev_curr2frame),
                                                                                   x, y, mask2=curr_im_mask, return_diff_ims=True)

        loss_object, object_loss_Z = object_consistency_loss(curr_motion, curr_instance_mask, curr_instance_ids)
        ...
        # static_motion_loss = tf.reduce_mean(curr_motion * tf.tile((1 - curr_im_mask), [1, 1, 1, 3]))
        static_motion_loss = 0.
        control_loss *= 10.

        loss_motion_ = loss_geo_motion + loss_photo_motion * .5 + static_motion_loss * 2.5 + loss_object * 0.
        ...
        _ = tf.reverse(tf.expand_dims(open_mask[:,:,:,0], 3), axis=[1], name='open_mask1')
        _ = tf.reverse(tf.expand_dims(open_mask[:,:,:,1], 3), axis=[1], name='open_mask2')
