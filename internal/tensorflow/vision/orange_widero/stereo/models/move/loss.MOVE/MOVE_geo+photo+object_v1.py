...
from stereo.models.MOVE_utils import loss_geom_motion, loss_phot_motion, object_consistency_loss
...
def loss(out, curr_inp, prev_inp, next_inp, prev_curr2frame, next_curr2frame, curr_T_cntr_srnd,
    ...
    with tf.compat.v1.name_scope("Loss_"):
        ...
        loss_photo_motion, I_warp_motion, motion_loss_im = loss_phot_motion(I1_cntr, I2_cntr, Z_inv2_cntr, curr_motion,
                                                                            curr_focal, curr_origin, prev_curr2frame,
                                                                            x, y, mask1=curr_im_mask)

        loss_geo_X, loss_geo_Y, loss_geo_Z, Z_inv2_warp, Z_inv2 = loss_geom_motion(Z_inv1_cntr, Z_inv2_cntr,
                                                                                   curr_motion, curr_focal, curr_origin,
                                                                                   tf.linalg.inv(prev_curr2frame),
                                                                                   x, y, mask2=curr_im_mask)

        loss_object = object_consistency_loss(curr_motion, curr_instance_mask, curr_instance_ids)


        loss_geo_motion = 30 * loss_geo_Z + .25 * loss_geo_X + .1 * loss_geo_Y

        static_motion_loss = tf.reduce_mean(input_tensor=curr_motion * tf.tile((1 - curr_im_mask), [1, 1, 1, 3]))
        control_loss *= 10.

        loss_motion_ = loss_geo_motion + loss_photo_motion * .5 + static_motion_loss * 2.5 + loss_object * 10
