...
def loss(out, curr_inp, prev_inp, next_inp, prev_curr2frame, next_curr2frame, curr_T_cntr_srnd,
    ...
    with tf.compat.v1.name_scope("Loss_"):
        ...
        _ = tf.identity(tf.reduce_mean(input_tensor=curr_motion[:,:,360:,0]), name="mean_x_right")
        _ = tf.identity(tf.reduce_mean(input_tensor=curr_motion[:,:,:360,0]), name="mean_x_left")
