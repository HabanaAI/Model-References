...
class ProposalLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        def nms(boxes, scores):
            ...
            ## the following lines adds Reshape op after Pad
            ## this is to call off dynamic shape after nms
            shape = [self.proposal_count, 4]
            proposals = tf.reshape(proposals, shape)
            return proposals
...
class PyramidROIAlign(KE.Layer):
    ...
    constructor.
    """
    def __init__(self, pool_shape,B=None,R=None,C=None, **kwargs):
    ...
    def _pyramid_pools_by_level(self,roi_level,boxes,feature_maps):
        ...
        for i, level in enumerate(range(2, 6)):
            ix = tf.cast(tf.where(tf.equal(roi_level, level)),tf.int32)
            ...
            feats = self._roi_align(feature_maps[i], level_boxes, box_indices)
            pooled.append(feats)
    ...
    def _epiluge(self,pooled,box_to_level):
        def _get_reorder_rois(box_to_level):
        ...
        reorder_ids = _get_reorder_rois(box_to_level)
        out_shape = (self.B,self.R)+ self.pool_shape + (self.C,)
        reordered_pooled = tf.gather(pooled, reorder_ids)
        # Re-add the batch dimension
        return tf.reshape(reordered_pooled, out_shape)
...
class PyramidROIAlignV2(PyramidROIAlign):
    def __init__(self, *args, **kwargs):
        super(PyramidROIAlignV2, self).__init__(*args,**kwargs)

    def _pyramid_pools_by_level(self,roi_level,boxes,feature_maps):
        # Loop through levels and apply ROI pooling to each. P2 to P5.
        final_feat_shape = (self.B, self.R, self.pool_shape[0], self.pool_shape[1], self.C)
        final_feats = tf.zeros(final_feat_shape)
        for i, level in enumerate(range(2, 6)):
            ix = tf.cast(tf.where(tf.equal(roi_level, level)),tf.int32)
            # Box indices for crop_and_resize.
            # Keep track of which box is mapped to which level
            ...
            # Result: [batch * num_boxes, pool_height, pool_width, channels]
            feats = self._roi_align(feature_maps[i], level_boxes, box_indices)
            final_feats = tf.tensor_scatter_nd_update(final_feats, ix, feats)

        return final_feats,None

    def _epiluge(self,pooled,box_to_level=None):
        return tf.reshape(pooled,(self.B,self.R,self.pool_shape[0],self.pool_shape[1],self.C))

class PyramidROIAlignOffload(PyramidROIAlign):
    """Implements ROI Pooling on multiple levels of the feature pyramid.
    ...
    def __init__(self, *args, **kwargs):
        super(PyramidROIAlignOffload, self).__init__(*args,**kwargs)
    ...
    def _pyramid_pools_by_level(self,roi_level,boxes,feature_maps):
        ...
        box_to_level = []
        # for crop&resize
        ## hack to gather on all boxes
        flattend_boxes = tf.reshape(boxes,(self.B*self.R, 4))
        box_ids = tf.repeat(tf.range(self.B),self.R,0)
        final_feat_shape = (self.B, self.R, self.pool_shape[0], self.pool_shape[1], self.C)
        ...
        for i, level in enumerate(range(2, 6)):
            # Crop and Resize
            # Result: [batch * num_boxes, pool_height, pool_width, channels]
            feats = tf.image.crop_and_resize(
                feature_maps[i], flattend_boxes, box_ids, [j*2 for j in self.pool_shape],
                method="bilinear")
            feats = tf.nn.avg_pool2d(feats, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
            all_box_roi_feats.append(tf.reshape(feats,final_feat_shape))

        # Pack pooled features into one tensor this part should be offloaded to host
        final_feats = tf.zeros(final_feat_shape)
        #[batch, num_boxes_lvl_i, pool_height, pool_width, channels]
        for i, level in enumerate(range(2, 6)):
            box_to_level.append(tf.cast(tf.where(tf.equal(roi_level, level)),tf.int32))
            level_feats = tf.gather_nd(all_box_roi_feats[i], box_to_level[i])
            final_feats = tf.tensor_scatter_nd_update(final_feats,box_to_level[i],level_feats)
            # Result final_feats is updated with the features from level i so we don't need to run sort
...
class PyramidROIAlignOffloadV2(PyramidROIAlignOffload):
    """Implements ROI Pooling on multiple levels of the feature pyramid.
    ...
    def __init__(self, *args, **kwargs):
        super(PyramidROIAlignOffloadV2, self).__init__(*args,**kwargs)
    ...
    def _pyramid_pools_by_level(self,roi_level,boxes,feature_maps):
        ...
        ## calculate the inverse permutation to for gathering rois in correct order, using map_fn since there is no
        # batched version for this op
        order = tf.map_fn(tf.math.invert_permutation, order_ids_to_invert)

        ## reorder ids (instead of data)
        stacked_level_mapping_reordered = tf.gather(stacked_lvl_roi_mapping, order,batch_dims=1)

        ##  collect ROIs in order, this is static and can be done on device:
...
# ## from tensorpack:
# #todo check this code for errors, training on proposals fails to converge
...
#     ret = avgpool(ret, [1, 2, 2, 1], [1, 2, 2, 1], padding='SAME')
#     return ret
...
def fpn_classifier_graph(rois, feature_maps, image_meta,
    ...
    pooled_features = build_roi_pool_fn([pool_size, pool_size], name="roi_align_classifier")([rois, image_meta] + feature_maps)
...
def build_fpn_mask_graph(num_classes, image_meta ,rois=None, feature_maps=None,pool_size=None,pooled_features=None,
    ...
    if pooled_features is not None:
        ...
        x = build_roi_pool_fn([pool_size, pool_size], name="roi_align_mask")([rois, image_meta] + feature_maps)
...
def rpn_bbox_loss_graph(config, target_bbox, rpn_match, rpn_bbox):
    ...
    if AVOID_LOSS_SWITCH:
        loss = tf.reduce_sum(loss) / tf.maximum(tf.cast(tf.size(loss),tf.float32), 1.0)
        #normalizer = tf.cast(tf.reduce_prod(tf.shape(target_bbox)[:2]),tf.float32)
...
def mrcnn_class_loss_graph(target_class_ids, pred_class_logits,
    ...
    # Loss
    #tf.Print([target_class_ids],tf.shape(target_class_ids))
...
def mrcnn_bbox_loss_graph(target_bbox, target_class_ids, pred_bbox):
    ...
    if AVOID_LOSS_SWITCH:
        ...
        #non_zeros_count_per_image = tf.math.count_nonzero(tf.reduce_sum(pred_bbox,axis=-1),1)
        # non_zero_mask = tf.cast(tf.reduce_sum(tf.abs(boxes), axis=1), tf.bool)
...
def load_image_gt(dataset, config, image_id, augment=False, augmentation=None,
    ...
    if DEBUG:
        ...
        # visualize.display_instances(image, box, mask, class_ids, class_names=[str(i) for i in range(80)],
        #           scores=None, title="",
...
class DataGenerator(keras.utils.Sequence):
    ...
    def __getitem__(self, idx):
        ...
        # buffer_for_loading_errors=max(min(len(self.image_ids)-idx_stop,5),0)
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir, hvd=None):
        """
    ...
    def build(self, mode, config):
        ...
        if self.config.PYRAMID_ROI_IMP == 3:
            build_roi_pool_fn = PyramidROIAlignOffloadV2
        elif self.config.PYRAMID_ROI_IMP == 2:
            build_roi_pool_fn = PyramidROIAlignOffload
        elif self.config.PYRAMID_ROI_IMP == 1:
            build_roi_pool_fn = PyramidROIAlignV2
        else:
        ...
        if mode == "training" and not self.dynamic_anchors:
            ...
            #anchors = KL.Lambda(lambda x: tf.stop_gradient(tf.Variable(anchors, name="anchors")))(input_image)
            anchors = tf.broadcast_to(K.constant(anchors), (config.BATCH_SIZE,) + anchors.shape)
        else:
        ...
        if mode == "training":
            ...
            if not rpn_only:
                ...
                # if config.APPROX_JOINT:
                #     print('using APPROXIMATE JOINT TRAINING (ROI COORDINATES GRADINET STOPPING)')
                #     target_rois= KL.Lambda(lambda x: tf.stop_gradient(x) , name="target_rois_no_grad")(target_rois)
                ...
                mrcnn_mask = build_fpn_mask_graph(config.NUM_CLASSES, input_image_meta,rois, mrcnn_feature_maps,
            ...
            mrcnn_mask = build_fpn_mask_graph(config.NUM_CLASSES, input_image_meta,
    ...
    def compile(self, learning_rate, momentum, loss_weights=None):
        ...
        if self.config.OPTIMIZER == 'SGD':
            opt_kwargs = dict(lr=learning_rate, momentum=momentum)
        ...
        #self.keras_model.metrics_names.clear()
        #self.keras_model.metrics_tensors.clear()
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers,
        ...
        #         shapes.append(tf.TensorShape(i.shape))
        #         types.append(getattr(tf,f'{i.dtype}'))
        #     #import pdb;pdb.set_trace()
        #     train_generator = tf.data.Dataset.from_generator(train_generator.__iter__, output_types=(tuple(types),tuple()),
        #                         output_shapes=(tuple(shapes),tuple(tf.TensorShape([])))).make_one_shot_iterator()
        #     val_generator = tf.data.Dataset.from_generator(val_generator.__iter__, output_types=(tuple(types),tuple()),
        #         output_shapes=(tuple(shapes),tuple(tf.TensorShape([])))).make_one_shot_iterator()
        ...
        self.compile(learning_rate, self.config.LEARNING_MOMENTUM,loss_weights=loss_weights)
