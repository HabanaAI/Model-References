...
if __name__ == '__main__':
    ...
    parser.add_argument('--backbone', required=False,default='resnet101',
                        metavar="/path/to/weights.h5",
                        help="Path to weights .h5 file or 'coco'")
    ...
    parser.add_argument('--roi_impl', required=False,
                        default=1,type=int,choices=[0,1,2,3],
                        metavar="0- vanilla, 1- no sort, 2 - host offload, 3 offload2",
                        help='specify roi pool implementation')
...
############################################################
    ...
    if args.using_horovod and args.command == "train":
        ...
        ## WARNING: there is a segnificant slowdown when using xla!
        # sess_config.graph_options.optimizer_options.global_jit_level = tf.OptimizerOptions.ON_1
        # instead set TF_XLA_FLAGS="--tf_xla_auto_jit=1 --tf_xla_cpu_global_jit"
    ...
    class CocoConfig(Config):
        ...
        PYRAMID_ROI_IMP = args.roi_impl # 0 - vanilla, 1 - no sort, 2 - offload
        if args.xl_inputs:
    ...
    model = modellib.MaskRCNN(mode=mode, config=config, model_dir=args.logs, hvd=hvd)
    ...
    if args.command == "train":
        ...
        #         iaa.Sometimes(0.5,iaa.Crop(percent=(0.25, 0.25))),
